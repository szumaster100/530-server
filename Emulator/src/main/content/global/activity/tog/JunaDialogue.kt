...
class JunaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if(!isQuestComplete(player, Quests.TEARS_OF_GUTHIX) && getQuestStage(player, Quests.TEARS_OF_GUTHIX) < 1) {
            npc(FaceAnim.OLD_DEFAULT,"Tell me... a story...").also { stage = 3 }
        } else if(getQuestStage(player, Quests.TEARS_OF_GUTHIX) >= 1) {
            options("Okay...", "A story?", "Not now", "You tell me a story").also { stage = 19 }
        } else if(getQuestStage(player, Quests.TEARS_OF_GUTHIX) >= 1 && inInventory(player, Items.STONE_BOWL_4704)) {
            npc("Before you can collect the Tears of Guthix you must", "make a bowl out of the stone in the cave on the south", "of the chasm.").also { stage = 25 }
        }
        return true
    }
        ...
        when (stage) {
            0 -> {
                ...
                if (BarcrawlManager.getInstance(player).isFinished) {
                    questDialogue.add(0, "I completed my bar crawl card and earned access to the Barbarian Outpost agility course.")
                    questDialogue.add(1, "...")
                }
                ...
                if (isQuestComplete(player, Quests.COOKS_ASSISTANT)) {
                    questDialogue.add(0, "...And, in the end, I found all the ingredients, so the Duke of Lumbridge had a birthday cake after all.")
                    questDialogue.add(1, "Ah, a happy ending. It would not be good for such an anniversary to go unmarked.")
                }
                if (isQuestComplete(player, Quests.DEMON_SLAYER)) {
                    questDialogue.add(0, "...so I destroyed the demon Delrith and saved Varrock!")
                    questDialogue.add(1, "I remember Delrith. A most unpleasant character; I am glad he has been dispatched.")
                }
                ...
                if (isQuestComplete(player, Quests.DRAGON_SLAYER)) {
                    questDialogue.add(0, "...so with Elvarg the dragon dead, the master of the Champions' Guild let me in, and I was able to wear rune plate armour!")
                    questDialogue.add(1, "I remember Delrith. A most unpleasant character; I am glad he has been dispatched.")
                }
                if (isQuestComplete(player, Quests.DORICS_QUEST)) {
                    questDialogue.add(0, "...so once I had got all the ores he wanted, Doric let me use his anvils.")
                ...
                if (isQuestComplete(player, Quests.DWARF_CANNON)) {
                    ...
                    questDialogue.add(1, "So, war still rages in the world above? Will you never tire of creating machines of destruction?")
                }
                if (isQuestComplete(player, Quests.ERNEST_THE_CHICKEN)) {
                    questDialogue.add(0, "...So, once I had found all the parts for the machine, poor Ernest could be himself once more.")
                    questDialogue.add(1, "That was a good deed. It is a terrible thing to be locked out of one's natural form.")
                }
                ...
                if (isQuestComplete(player, Quests.FISHING_CONTEST)) {
                    questDialogue.add(0, "...And after I had won the fishing contest, the Dwarves let me go under White Wolf Mountain.")
                    questDialogue.add(1, "Fishing? A strange test of worthiness to pass through an underground tunnel!")
                }
                if (isQuestComplete(player, Quests.THE_FREMENNIK_TRIALS)) {
                    questDialogue.add(0, "...And that was how I became an honourary member of the Fremennik, and was given my Fremennik name, ${getAttribute(player, "fremennikname", "fremmyname")}.")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.GERTRUDES_CAT)) {
                    ...
                    questDialogue.add(1, "Cats are one of the most mysterious creatures of Guthix. I hope you take your responsibility seriously.")
                }
                if (isQuestComplete(player, Quests.GOBLIN_DIPLOMACY)) {
                    questDialogue.add(0, "...and so the goblins ended up wearing the armour colour they had to start off with!")
                    questDialogue.add(1, "Poor, silly goblins! Their race had such potential; if only they could rise above their petty squabbles.")
                }
                if (isQuestComplete(player, Quests.IMP_CATCHER)) {
                    questDialogue.add(0, "...took some time, but I finally got all four beads back, and Mizgog gave me my reward.")
                    questDialogue.add(1, "Imps! I remember the age of great war, when armies of Zamorak's imps bloodied the ankles of the other gods' creatures.")
                }
                if (isQuestComplete(player, Quests.JUNGLE_POTION)) {
                    questDialogue.add(0, "...and once I had gathered all the herbs, Trufitus Shakaya was able to commune with his gods.")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.LOST_CITY)) {
                    questDialogue.add(0, "...And when I entered the door carrying the Dramen Staff, I was transported to a whole new world - a world populated by magical fairies!")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.THE_LOST_TRIBE)) {
                    questDialogue.add(0, "...Sigmund was dismissed, and the duke and the ruler of the cave goblins signed a peace treaty.")
                    questDialogue.add(1, "The Dorgueshuun goblins have been good neighbours during my vigil here. They are a timid race, but not cowardly, and I am glad they have the peace they desire.")
                }
                if (isQuestComplete(player, Quests.MERLINS_CRYSTAL)) {
                    questDialogue.add(0, "...and when I told King Arthur that I had single-handedly freed Merlin from his crystal prison, he made me a knight of the Round Table!")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.MONKS_FRIEND)) {
                    ...
                    questDialogue.add(1, "Mirth should not be taken to excess. These Saradominists have never understood the importance of balance.")
                }
                if (isQuestComplete(player, Quests.NATURE_SPIRIT)) {
                    questDialogue.add(0, "...and that was how I helped Filliman Tarlock transform into a nature spirit.")
                ...
                if (isQuestComplete(player, Quests.PIRATES_TREASURE)) {
                    questDialogue.add(0, "...And when I dug in the middle of the park in Falador, I found the pirate's treasure!")
                    questDialogue.add(1, "Such is ever the folly of pirates, to bury their loot in the ground so that another can dig it up.")
                }
                ...
                if (isQuestComplete(player, Quests.ROMEO_JULIET)) {
                    questDialogue.add(0, "...I gave the message to Romeo, but he misunderstood, so they never were together.")
                ...
                if (isQuestComplete(player, Quests.RUNE_MYSTERIES)) {
                    questDialogue.add(0, "...So, I brought Aubury's notes to Sedridor the head wizard and, from then on, I was able to mine rune essence.")
                ...
                if (isQuestComplete(player, Quests.SHEEP_SHEARER)) {
                    questDialogue.add(0, "Out of sheer hard work, I managed to shear some sheep for farmer Fred. Then I got in a spin and created some balls of wool.")
                    questDialogue.add(1, "...")
                }
                ...
                if (isQuestComplete(player, Quests.THE_TOURIST_TRAP)) {
                    questDialogue.add(0, "...Ana wasn't too happy to be cooped up in that barrel! But at least I got her out of the mining camp.")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.VAMPIRE_SLAYER)) {
                    questDialogue.add(0, "...And once the vampyre was dead, the people of Draynor no longer lived in fear.")
                    questDialogue.add(1, "...")
                }
                ...
                if (isQuestComplete(player, Quests.WHAT_LIES_BELOW)) {
                    questDialogue.add(0, "...but after I defeated King Roald, Rat was able to remove Surok's mind control spell from him.")
                    questDialogue.add(1, "I see the servants of Zamorak still play their part in balancing the world between good and evil.")
                }
                ...
                if (isQuestComplete(player, Quests.WITCHS_POTION)) {
                    questDialogue.add(0, "...And once I got her all the ingredients, Hetty's potion increased my magical power!")
                ...
                if (isQuestComplete(player, Quests.BIG_CHOMPY_BIRD_HUNTING)) {
                    questDialogue.add(0, "...poor Rantz was so clumsy he couldn't shoot anything, but I managed to kill the Chompy bird.")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.CREATURE_OF_FENKENSTRAIN)) {
                    questDialogue.add(0, "...But in the end I stopped Fenkenstrain from continuing his horrible experiments.")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.ENLIGHTENED_JOURNEY)) {
                    questDialogue.add(0, "...we landed the balloon in Taverley and Auguste thanked me for helping him get his idea off the ground.")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.GARDEN_OF_TRANQUILLITY)) {
                    ...
                    questDialogue.add(1, "It is good that you can take time from slaying and adventuring to grow a beautiful garden.")
                }
                ...
                if (isQuestComplete(player, Quests.SHEEP_SHEARER)) {
                    questDialogue.add(0, "...I would never have thought that three scorpions would take that long to find!")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.TEMPLE_OF_IKOV)) {
                    questDialogue.add(0, "...Lucien said that the Staff of Armadyl I gave him had made him more powerful.")
                    questDialogue.add(1, "...")
                }
                if (isQuestComplete(player, Quests.TEMPLE_OF_IKOV) && getAttribute(player, GameAttributes.QUEST_IKOV_SELECTED_END, 0) == 1) {
                ...
                if (isQuestComplete(player, Quests.THE_DIG_SITE)) {
                    questDialogue.add(0, "...And the examiner was very impressed that I had discovered an ancient altar of Zaros.")
                    questDialogue.add(1, "Zaros? I had not heard that name for a thousand years, even before the start of my sojourn here.")
                }
                if (isQuestComplete(player, Quests.TOWER_OF_LIFE)) {
                    questDialogue.add(0, "...So, it turned out that the alchemists had created a Homunculus - a mixture of logic and magic.")
                    questDialogue.add(1, "Ah, that is good. New forms of life should always be created according to a balance between different principles.")
                }
                if (isQuestComplete(player, Quests.TREE_GNOME_VILLAGE)) {
                    questDialogue.add(0, "...And King Bolren thanked me for defeating the warlord and returning the orbs to the gnome people.")
                    questDialogue.add(1, "...")
                }
            ...
            21 -> npc(FaceAnim.OLD_DEFAULT,"Your stories have entertained me. I will let you into", "the cave for a short time.").also { stage++ }
            22 -> if(!inInventory(player, Items.STONE_BOWL_4704)) {
                npc(FaceAnim.OLD_DEFAULT, "But first you will need to make a bowl in which to", "collect the tears.").also { stage++ }
            } else {
                player("I have a bowl.").also { stage++ }
            }
            ...
            23 -> {
                ...
                npc(FaceAnim.OLD_DEFAULT, "There is a cave on the south side of the chasm that is", "similarly infused with the power of Guthix. The stone in" ,"that cave is the only substance that can catch the Tears", "of Guthix.").also { stage++ }
            }
            24 -> {
                npc(FaceAnim.OLD_DEFAULT,"Mine some stone from that cave, make it into a bowl,", "and bring it to me, and then I will let you catch the tears.")
                player.questRepository.setStageNonmonotonic(player.questRepository.forIndex(120), 1)
                resetCamera(player)
                stage = END_DIALOGUE
            }
            25 -> player("I have a bowl.").also { stage++ }
            26 -> npc(FaceAnim.OLD_DEFAULT,"I will keep your bowl for you, so that you may collect", "the tears many times in the future.").also { stage++ }
            27 -> {
                end()
                finishQuest(player, Quests.TEARS_OF_GUTHIX)
                setVarbit(player, Vars.VARBIT_QUEST_TEARS_OF_GUTHIX_PROGRESS_451, 2)
            }
        }
        return true
    }
