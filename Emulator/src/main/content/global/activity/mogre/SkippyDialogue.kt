        ...
        when (getVarbit(player, SkippyUtils.SKIPPY_VARBIT)) {
            0 ->
                if (!inInventory(player, Items.BUCKET_OF_WATER_1929)) {
                    playerl(FaceAnim.HALF_GUILTY, "You know, I could shock him out of it if I could find some cold water...").also { stage = END_DIALOGUE }
                } else {
                    playerl(FaceAnim.HALF_GUILTY, "Well, I could dump this bucket of water over him. That would sober him up a little.").also { stage = 0 }
                }
        ...
        when (stage) {
            ...
            15 -> {
                if (!anyInInventory(player, NETTLE_TEA_BOWL, NETTLE_TEA_CUP, NETTLE_TEA_PORCELAIN_CUP)) {
                    playerl(FaceAnim.NEUTRAL, "No real reason. I just thought I would check up on you is all.").also { stage = 29 }
                } else {
