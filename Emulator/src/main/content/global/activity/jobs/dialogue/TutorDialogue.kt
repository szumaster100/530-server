...
class TutorDialogue : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            0 -> playerl(FaceAnim.ASKING, "Do you have any jobs for me?").also {
                stage = if (hasAnItem(player!!, Items.TASK_LIST_13464).exists()) {
                    if (JobManager.getInstance(player!!).hasJob()) 100 else 10
                } else 1
            }

            1 -> npcl("Well, first of all, I can see that you don't have a task list. Would you like one?").also { stage++ }
            2 -> options("Yes, please.", "No, thanks.").also { stage++ }
            3 -> when (buttonID) {
                1 -> when (addItem(player!!, Items.TASK_LIST_13464)) {
            ...
            10 -> npcl("Let me see if I've got any work for you.").also {
                if (playerJobManager.hasReachedJobLimit()) stage = 200 else {
            ...
            30 -> npcl("I'm sorry, I don't currently have any jobs that you're qualified for.").also {
                stage = END_DIALOGUE
            }
            ...
            100 -> {
                ...
                when (job.type) {
                    ...
                    JobType.BONE_BURYING -> {
                        ...
                        npcl("You're still working for $employerName. You need to bury $amount more $pluralized in ${job.buryArea.title}.")
                    }
