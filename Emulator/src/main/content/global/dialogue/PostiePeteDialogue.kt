...
class PostiePeteDialogue(player: Player? = null) : Dialogue(player) {
    ...
    private fun handleLaughingMiner() {
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "4 pints of your finest please.").also {
                sendChat(npc, "4 pints of your finest please.")
                stage++
            }

            1 -> sendNPCDialogue(player, NPCs.BARMAID_2178, "4 pints coming right up. Are you expecting guests?", FaceAnim.OLD_NORMAL).also {
            ...
            2 -> npcl(FaceAnim.OLD_NORMAL, "Just a few old friends.").also {
                sendChat(npc, "Just a few old friends.")
                stage++
            }

            3 -> sendNPCDialogue(player, NPCs.BARMAID_2178, "You want me to put it on your tab?", FaceAnim.OLD_NORMAL).also {
                findLocalNPC(player, NPCs.BARMAID_2178)?.let { sendChat(it, "You want me to put it on your tab?") }
                stage++
            }

            4 -> npcl(FaceAnim.OLD_NORMAL, "That would be great, thanks. Oh, cripes! I've left a fire burning! I've got to go!").also {
    ...
    private fun handlePortPhasmatys() {
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Hey Pete. Any news from the fleet?").also {
            ...
            3 -> npcl(FaceAnim.OLD_NORMAL, "Good good.").also {
                sendChat(npc, "Good good.")
                stage++
            }

            4 -> npcl(FaceAnim.OLD_NORMAL, "Well you know what to do if something goes wrong.").also {
            ...
            7 -> npcl(FaceAnim.OLD_NORMAL, "That's it brother, and we'll come a running!").also {
    ...
    private fun handleVarrockLibrary() {
        when (stage) {
            ...
            3 -> npc(FaceAnim.OLD_NORMAL, "Not much, just doing some research on Dwarvern", "Culture. Did you know that Keldagrim was originally", "constructed by a long lost clan who embraced the power", "of magic, and used it to aid with glorious buildings like")
            4 -> npc(FaceAnim.OLD_NORMAL, "the royal palace?")
            5 -> player(FaceAnim.HAPPY, "Really??")
            6 -> npc(FaceAnim.OLD_NORMAL, "Yes. They even stayed down there for 1000 years or", "more, in a time they call 'The age of Kings'.")
            7 -> player(FaceAnim.HAPPY, "WOW!")
            8 -> npc(FaceAnim.OLD_NORMAL, "Shhhh! That's enough history for one day I think!").also { stage = END_DIALOGUE }
            9 -> GameWorld.Pulser.submit(object : Pulse(1, npc) {
                var counter = 0
    ...
    private fun handleFaladorPartyRoom() {
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Hey Bro, how's the party business?").also {
            ...
            4 -> npcl(FaceAnim.OLD_NORMAL, "Nice. Well I'm here to talk to some white knights.").also {
            ...
            6 -> npcl(FaceAnim.OLD_NORMAL, "No thanks, I'm still dieting.").also {
                sendChat(npc, "No thanks, I'm still dieting.")
                stage++
            }

            7 -> npcl(FaceAnim.OLD_NORMAL, "Just seafood for me.").also {
    ...
    private fun handleIceMountain() {
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Well I'm back. And he said no. Again.").also {
            ...
            2 -> npcl(FaceAnim.OLD_NORMAL, "Lemons? What are you talking about?").also {
            ...
            4 -> npcl(FaceAnim.OLD_NORMAL, "I swear you get stranger every time.").also {
                sendChat(npc, "I swear you get stranger every time.")
                stage++
            }

            5 -> npcl(FaceAnim.OLD_NORMAL, "Ok, I'll ask again.").also {
    ...
    private fun handleWildernessBanditCamp() {
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Psssst! How is the plan going?").also {
            ...
            4 -> npcl(FaceAnim.OLD_NORMAL, "Till next time. Farewell.").also {
    ...
    private fun handleArdougneZoo() {
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Hi Pete!").also {
                sendChat(npc, "Hi Pete!")
                stage++
            }

            1 -> npcl(FaceAnim.OLD_NORMAL, "Mum wants to know if you're coming round for tea.").also {
            ...
            6 -> npcl(FaceAnim.OLD_NORMAL, "Got a taste for parrots now too does he?").also {
                sendChat(npc, "Got a taste for parrots now too does he?")
                stage++
            }

            7 -> npcl(FaceAnim.OLD_NORMAL, "*Sigh* Will nothing sacred be safe?").also {
