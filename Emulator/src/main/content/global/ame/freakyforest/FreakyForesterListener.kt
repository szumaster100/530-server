    ...
    override fun defineListeners() {
        ...
        on(freakNpc, IntType.NPC, "talk-to") { player, node ->
            if (inBorders(player, FreakyForesterUtils.freakArea)) {
                if (getAttribute(player, GameAttributes.RE_FREAK_TASK, -1) == -1) FreakyForesterUtils.giveFreakTask(
                    player
                )
        ...
        on(pheasants, IntType.NPC, "attack") { player, node ->
            ...
            } else if (inInventory(player, Items.RAW_PHEASANT_6178) || inInventory(player, Items.RAW_PHEASANT_6179) || getAttribute(player, GameAttributes.RE_FREAK_KILLS, false)) {
