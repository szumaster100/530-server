...
class MordautDialogue(
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        if (examComplete) {
            ...
            when (stage++) {
                0 -> npcl(FaceAnim.OLD_NORMAL, "WELL DONE! You've proven your aptitude for pattern recognition. You'll receive a prize when you leave the classroom.")
                1 -> npcl(FaceAnim.OLD_NORMAL, "To exit, use the $doortext of the school room.").also { stage = END_DIALOGUE }
            }
                    ...
                    when (stage++) {
                        0 ->
                            if (getAttribute(player!!, GameAttributes.RE_PATTERN_CORRECT, -1) == 1) {
                                npcl(FaceAnim.OLD_NORMAL, "Finally, a pupil using his brains rather than trying to eat them. Next question.")
                            } else {
                ...
                when (stage++) {
                    0 ->
                        if (getAttribute(player!!, GameAttributes.RE_PATTERN_CORRECT, -1) == 0) {
                            npcl(FaceAnim.OLD_NORMAL, "Remember, the goal here is to find the pattern. Look for objects that would have a connection to each other.")
                        } else {
            ...
            when (stage++) {
                0 -> npcl(FaceAnim.OLD_NORMAL, "Ah, It's you, ${player!!.username}. You've been slacking in your studies, so it's time for an exam.")
                1 -> npcl(FaceAnim.OLD_NORMAL, "There are two types of question. The first type requires you to find the next object in a pattern.")
                2 -> npcl(FaceAnim.OLD_NORMAL, "In the second type, I will present you with fifteen cards and a hint. Discover three cards related to the hint, select them and confirm.")
