        ...
        if (correctTask == 1) {
            ...
            when (stage) {
                ...
                2 -> {
                    lock(player!!, 3)
                    runTask(player!!, 3) {
                        Component.setUnclosable(player!!, player!!.dialogueInterpreter.sendDialogues(NPC(3508), FaceAnim.FRIENDLY, "Well, that's a good attempt, but it's just not right.")).also { stage = 3 }
                    }
                }
                3 -> npcl("Try looking in the coffins to get a better idea of who is in them, and then read the gravestones to find who needs to be in there.").also { stage = 4 }
                4 -> playerl("All right, I'll give it another shot.").also { stage = 5 }
                5 -> npcl("Don't forget to store any items that you don't need in the mausoleum. I'll take them to the bank while you work.").also { stage = END_DIALOGUE }
            ...
            when (stage) {
                ...
                3 -> {
                    lock(player!!, 3)
                    runTask(player!!, 3) {
                        Component.setUnclosable(player!!, player!!.dialogueInterpreter.sendDialogues(NPC(3508), FaceAnim.FRIENDLY, "Wonderful! That's taken care of all of them.")).also { stage = 4 }
                    }
                }
