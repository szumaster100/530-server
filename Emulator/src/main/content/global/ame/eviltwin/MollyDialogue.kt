        ...
        when (type) {
                    ...
                    3 -> {
                        ...
                        AntiMacro.rollEventLoot(player!!).forEach {
                            ...
                            sendItemDialogue(
                                player!!,
                                it.id,
                                "Molly's given you " + it.amount + " " + getItemName(it.id).lowercase() + "s.",
                            )
                        }
                        stage = END_DIALOGUE
                    }
            ...
            3 ->
                when (stage) {
                    0 ->
                        when {
                            !getAttribute(player!!, GameAttributes.RE_TWIN_DIAL, false) ->
                                npc(
                                    FaceAnim.SAD,
                                    "I'm sorry for abducting you like that, but I really need",
                                    "your help ${player!!.username}.",
                                ).also {
                                    setAttribute(player!!, "/save:${GameAttributes.RE_TWIN_DIAL}", true)
                                    stage =
                                        4
                                }
                            !getAttribute(
                                player!!,
                                GameAttributes.RE_TWIN_OBJ_DIAL,
                                false,
                            ) ->
                                npc(FaceAnim.ASKING, "Wait! Do you know what you're doing here?").also {
                                    setAttribute(player!!, "/save:${GameAttributes.RE_TWIN_OBJ_DIAL}", true)
                                    stage++
                                }
