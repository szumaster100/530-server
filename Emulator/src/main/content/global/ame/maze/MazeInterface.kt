...
class MazeInterface : InteractionListener, EventHook<TickEvent>, MapArea {
    companion object {
        ...
        fun initMaze(player: Player) {
            ...
            sendNPCDialogue(
                ...
                "You need to reach the maze center, then you'll be returned to where you were."
            )
        }
         ...
         */
        fun overrideScenery(
            wallScenery: core.game.node.scenery.Scenery,
            chestSceneryId: Int
    ...
    override fun defineListeners() {
        ...
        on(Scenery.CHEST_3635, IntType.SCENERY, "open") { player, node ->
            if (getAttribute(player, MAZE_ATTRIBUTE_TICKS_LEFT, 0) > 0 && getAttribute(
                    player,
                    MAZE_ATTRIBUTE_CHESTS_OPEN,
                    0
                ) < 10
            ) {
                animate(player, 536)
            ...
            queueScript(player, 0, QueueStrength.SOFT) { stage: Int ->
                when (stage) {
                    ...
                    2 -> {
                        teleport(
                            player,
                            getAttribute(player, MAZE_ATTRIBUTE_RETURN_LOC, Location.create(3222, 3218, 0))
                        )
