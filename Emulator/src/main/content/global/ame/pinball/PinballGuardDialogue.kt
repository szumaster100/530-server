        ...
        when (stage) {
            0 ->
                if (getVarbit(player!!, 2121) >= 10) {
                    ...
                    lock(player!!, 1000)
                    sendNPCDialogue(player!!, 3912, "You poke 10 flashing pillars, right? You NOT poke other pillars, right? Okay, you go play now.", FaceAnim.OLD_NORMAL)
                    stage += 3
                }

            1 -> sendNPCDialogue(player!!, 3912, "Yer, get going. We get break now.", FaceAnim.OLD_NORMAL).also { stage++ }
            2 -> end().also {
