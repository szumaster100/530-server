    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (patch?.patch?.type == PatchType.TREE_PATCH) {
            ...
            if (patch!!.isGrown() && !isTreeStump) {
                sendMessage(player, "You need to chop this tree down first.")
                stage = 1000
                return true
            }
        }
                ...
                1 -> {
                    ...
                    queueScript(player, 0, QueueStrength.WEAK) { stage: Int ->
                        when (stage) {
                            ...
                            2 -> {
                                if (patch?.patch?.type == PatchType.TREE_PATCH) {
                                    if (patch?.getCurrentState() == (
                                                patch?.plantable?.value
                                                    ?: 0
                                                ) + (
                                                patch?.plantable?.stages
                                                    ?: 0
                                                ) + 2 && patch?.isWeedy() != true && patch?.isEmptyAndWeeded() != true
                                    ) {
