...
class RaccoonDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (stage) {
            ...
            9 -> npcl(
                FaceAnim.CHILD_NORMAL,
                "Chitterchatter chatterchitter chitter. (You don't tell me nothin'. You'd best start openin' up ter me soon, " + if (player.isMale) "boy" else "girl" + ", or we are gonna fall out.)"
            ).also { stage = END_DIALOGUE }
            ...
            14 -> npcl(
                FaceAnim.CHILD_NORMAL,
                "Chatter chitter. (Don't ever refer to me as cute, though, " + if (player.isMale) "boy" else "girl" + ".)"
            ).also { stage++ }
