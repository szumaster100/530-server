...
class CrystalBallsSpace : InteractionListener {
    private enum class Staff(
        val staffId: Int, val start: Animation, val end: Animation, val cost: Item? = null, var type: StaffType
    ) {
        ...
        companion object {
            ...
            fun getProduct(type: StaffType, element: String): Int? {
    ...
    private val crystalBallObjects = intArrayOf(
        Scenery.CRYSTAL_BALL_13659, Scenery.ELEMENTAL_SPHERE_13660, Scenery.CRYSTAL_OF_POWER_13661
    )
    ...
    override fun defineListeners() {
        onUseWith(IntType.SCENERY, Staff.ALL_STAFFS, *crystalBallObjects) { player, staff, scenery ->
            ...
            if (scenery.id == Scenery.CRYSTAL_BALL_13659 && staffType == StaffType.MYSTIC || staffType == StaffType.BATTLE) {
    ...
    private fun handleElementSelection(player: Player, staff: Staff) {
