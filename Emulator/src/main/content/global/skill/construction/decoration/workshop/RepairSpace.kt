 ...
 */
@Initializable
class RepairSpace : UseWithHandler(
    /*
    ...
    override fun handle(event: NodeUsageEvent?): Boolean {
        ...
        when (event.usedWith.id) {
            Scenery.REPAIR_BENCH_13713 -> if (item.id == Items.BROKEN_ARROW_687 || item.id == Items.BROKEN_STAFF_689) {
                repair(player, item)
                return true
            }

            Scenery.WHETSTONE_13714 -> if (item.id == Items.BROKEN_ARROW_687 || item.id == Items.BROKEN_STAFF_689 || item.id == Items.RUSTY_SWORD_686) {
                repair(player, item)
                return true
            }

            Scenery.ARMOUR_REPAIR_STAND_13715 -> if (item.id in brokenItem) {
                repair(player, item)
            } else {
                var baseCost = 0.0
                ...
                val cost = ceil(((100.0 - (player.skills.getLevel(Skills.SMITHING) / 2.0)) / 100.0) * baseCost).toInt()
