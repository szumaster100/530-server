...
class AgilityBossDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npc(FaceAnim.CHILD_NORMAL, "Bring the stick back to the werewolf waiting at the end", "of the death slide to get your agility bonus.").also { stage++ }
            3 -> npc(FaceAnim.CHILD_NORMAL, "I will throw your stick as soon as you jump onto the", "first stone.").also { stage++ }
            4 -> {
                end()
