...
@JvmOverloads constructor(player: Player? = null) : AgilityCourse(player, 5, 0.0) {
    ...
    private fun handleStairs(player: Player, scenery: Scenery) {
    ...
    private fun handleLowWall(player: Player, scenery: Scenery) {
    ...
    private fun handleLedge(player: Player, scenery: Scenery) {
        val d = Direction.getLogicalDirection(player.location, getLedgeLocation(player, scenery))
        val dir = d
        val diff =
            if (scenery.rotation == 3 && dir == Direction.EAST) 1 else if (scenery.rotation == 3 && dir == Direction.WEST) 0 else if (d == Direction.EAST || (d == Direction.SOUTH && scenery.rotation != 0) || d == Direction.NORTH) 0 else 1
        val fail = player.getSkills().getLevel(Skills.AGILITY) < 75 && hasFailed(player)
    ...
    private fun handlePlank(player: Player, scenery: Scenery) {
    ...
    private fun handleJumpGap(player: Player, scenery: Scenery) {
        ...
        if (fail) {
            ...
            dest = if (!regular) {
                ...
                dest.transform(
                    if (dir == Direction.NORTH || dir == Direction.SOUTH) -1 else if (dir == Direction.WEST) 1 else 0,
                    if (dir == Direction.SOUTH) 1 else if (dir == Direction.WEST || dir == Direction.EAST) -1 else 0,
                    0
                )
            }
    ...
    private fun handleGapCross(player: Player, scenery: Scenery) {
        ...
        val mod = if (player.location == Location(3359, 2849, 2)) 0 else if (player.location == Location(
                3357,
                2841,
                2
            )
        ) 1 else if (player.location == Location(3367, 2832, 1)) 1 else if (player.location == Location(
                3372,
                2832,
                1
            )
        ) 0 else if (scenery.location == Location(
                3370,
                2831,
                1
            )
        ) 0 else if (rot == 1 && dir == Direction.EAST) 0 else if (rot == 3 && (dir == Direction.WEST || dir == Direction.EAST)) 1 else if (rot == 0 && dir == Direction.SOUTH) 1 else if (dir == Direction.WEST && rot != 3 || dir == Direction.EAST) 1 else 0
        ...
        if (fail) {
            var dest = scenery.location.transform(dir, 1)
            dest = if (rot == 1 && dir == Direction.EAST) dest.transform(
                1,
                2,
                0
            ) else if (rot == 1 && dir == Direction.WEST && player.location.y < 2841) dest.transform(
                0,
                -2,
                0
            ) else if (rot == 1 && dir == Direction.WEST) dest.transform(
                0,
                2,
                0
            ) else dest.transform(
    ...
    private fun handlePyramidTop(player: Player, scenery: Scenery) {
    ...
    private fun getLocationData(data: Array<Array<Location>>, location: Location): Array<Location>? {
        for (datum in data) {
            for (l in datum) {
                if (l == location) {
                    return datum
    ...
    private fun hasFailed(player: Player, mod: Double = 0.009000): Boolean {
    ...
    companion object {
        ...
        @JvmStatic
        fun addConfig(player: Player, objectId: Int, value: Int, save: Boolean) {
        ...
        @JvmStatic
        fun addConfig(player: Player, scenery: Scenery, value: Int, save: Boolean) {
