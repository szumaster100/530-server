...
class WerewolfCourse constructor(player: Player? = null) : AgilityCourse(player, 5, 0.0) {
    ...
    override fun handle(p: Player, node: Node, option: String): Boolean {
        ...
        when (n.id) {
            ...
            org.rs.consts.Scenery.HURDLE_5133, org.rs.consts.Scenery.HURDLE_5134, org.rs.consts.Scenery.HURDLE_5135 -> jumpHurdleObstacle(p, n)
            ...
            org.rs.consts.Scenery.ZIP_LINE_5139, org.rs.consts.Scenery.ZIP_LINE_5140, org.rs.consts.Scenery.ZIP_LINE_5141 -> zipLineObstacle(p, n)
        }
        return true
    }
    ...
    override fun configure() {
        ...
        for (i in intArrayOf(org.rs.consts.Scenery.HURDLE_5133, org.rs.consts.Scenery.HURDLE_5134, org.rs.consts.Scenery.HURDLE_5135)) {
        ...
        for (i in intArrayOf(org.rs.consts.Scenery.ZIP_LINE_5139, org.rs.consts.Scenery.ZIP_LINE_5140, org.rs.consts.Scenery.ZIP_LINE_5141)) {
    ...
    override fun getDestination(n: Node?, node: Node): Location? {
        ...
        if (scenery.id == org.rs.consts.Scenery.STEPPING_STONE_35996) {
            when (scenery.location.x) {
                ...
                3540 -> return if (scenery.location.y == 9877) scenery.location.transform(
                    -2,
                    0,
                    0
                ) else scenery.location.transform(0, -2, 0)
            }
        }
        if (scenery.location.y in intArrayOf(9893, 9896, 9899)) {
            when (scenery.id) {
                org.rs.consts.Scenery.HURDLE_5133, org.rs.consts.Scenery.HURDLE_5135 -> return scenery.location.transform(1, -1, 0)
                org.rs.consts.Scenery.HURDLE_5134 -> return if (scenery.location.x == 3543) scenery.location.transform(
                    -1,
                    -1,
                    0
                ) else scenery.location.transform(0, -1, 0)
            }
        }
        return null
    }
