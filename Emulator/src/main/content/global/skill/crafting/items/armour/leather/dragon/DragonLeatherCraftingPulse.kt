...
class DragonLeatherCraftingPulse(player: Player?, node: Item?, val leather: DragonLeather, var amount: Int) :
    ...
    override fun reward(): Boolean {
        ...
        if (removeItem(player, Item(leather.leather, leather.amount))) {
            if (leather.name.contains("VAMBS")) {
                ...
                sendMessage(
                    player,
                    "You make " + (if (StringUtils.isPlusN(getItemName(leather.product).lowercase())) "an" else "a") + " " + getItemName(
                        leather.product
                    ).lowercase() + "."
                )
            }
