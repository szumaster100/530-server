...
class EmptyOptionListener : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(EmptyItem.emptyItemList.toIntArray(), IntType.ITEM, "empty", "empty bowl", "empty dish") { player, node ->
            if (node.name.contains("brew") || node.name.contains("potion") || node.name.lowercase()
                    .contains("poison") || node.name.lowercase()
                    .contains("serum") || node.name.contains("cure") || node.name.contains("mix") || node.name.contains(
                    "balm"
                )
            ) {
    ...
    enum class EmptyItem(var fullId: Int, var emptyId: Int, var emptyMessage: String, var audioId: Int = -1) {
        ...
        companion object {
            var emptyItemMap: Int2IntMap = Int2IntOpenHashMap()
            var emptyMessageMap = HashMap<Int, String>()
            var emptyAudioMap: Int2IntMap = Int2IntOpenHashMap()
            var emptyItemList: IntArrayList = IntArrayList()
            ...
            init {
                ...
                for (item in ItemDefinition.getDefinitions().values) if (item.name.contains("potion") || item.name.contains(
                        "brew"
                    ) || item.name.contains("poison") || item.name.lowercase()
                        .contains("serum") || item.name.contains("cure") || item.name.contains("mix") || item.name.contains(
                        "balm"
                    )
                ) {
