        ...
        on(tabIDs, IntType.ITEM, "break") { player, node ->
            ...
            if (tabEnum != null && inInventory(player, tab)) {
                ...
                if (inInventory(player, tab)) {
                    ...
                    if (tab == Items.ASTRAL_ALTAR_TP_13611 && !hasRequirement(
                            player,
                    ...
                    if (tab == Items.COSMIC_ALTAR_TP_13605 && !isQuestComplete(player, Quests.LOST_CITY))
                        return@on true
                    if (tab == Items.DEATH_ALTAR_TP_13609 && !hasRequirement(player, QuestReq(QuestRequirements.MEP_2)))
                        return@on true
                    if (tab == Items.BLOOD_ALTAR_TP_13610 && !hasRequirement(
                            player,
