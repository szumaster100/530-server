    ...
    override fun handle(
        ...
        if (button == 2) {
            ...
            if (confirmedSkill == -1) {
                ...
                when (confirmedSkill) {
                    Skills.HERBLORE -> if (!checkHerblore(player)) sendMessage(
                        player,
                        "You need to have completed ${Quests.DRUIDIC_RITUAL} for this."
                    ).also { return true }

                    Skills.RUNECRAFTING -> if (!checkRunecrafting(player)) sendMessage(
                        player,
                        "You need to have completed Rune Mysteries for this."
                    ).also { return true }

                    Skills.SUMMONING -> if (!checkSummoning(player)) sendMessage(
                        player,
