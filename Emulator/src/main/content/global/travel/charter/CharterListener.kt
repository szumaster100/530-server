        ...
        on(PLANK, IntType.SCENERY, "cross") { player, node ->
            ...
            if (getUsedOption(player) == "cross") {
                when (node.id) {
                    ...
                    Scenery.GANGPLANK_2085 -> cross(player, ARDOUGNE[0]).also {
                        sendMessage(
                            player,
                    ...
                    Scenery.GANGPLANK_2087 -> cross(player, BRIMHAVEN[2]).also {
                        sendMessage(
                            player,
                    ...
                    Scenery.GANGPLANK_14304 -> cross(player, PORT_SARIM[6]).also {
                        sendMessage(player, "You board the ship.")
                    }

                    Scenery.GANGPLANK_14305 -> cross(player, PORT_SARIM[7]).also {
                        sendMessage(player, "You disembark the ship.")
                    }

                    Scenery.GANGPLANK_14306 -> cross(player, PEST_CONTROL[0]).also {
                        sendMessage(player, "You board the ship.")
                    }

                    Scenery.GANGPLANK_14307 -> cross(player, PEST_CONTROL[1]).also {
                        sendMessage(player, "You disembark the ship.")
                    }
                    ...
                    Scenery.GANGPLANK_17398 -> cross(player, KARAMJA[2]).also {
                        sendMessage(player, "You must speak to the Customs Officer before it will set sail.")
                    }

                    Scenery.GANGPLANK_17399 -> cross(player, KARAMJA[3]).also {
                        sendMessage(player, "You must speak to the Customs Officer before it will set sail.")
                    }
