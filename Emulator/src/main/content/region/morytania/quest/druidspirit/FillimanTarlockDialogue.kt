...
class FillimanTarlockDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when (questStage) {
            ...
            25 -> npcl(FaceAnim.HALF_GUILTY, "Oh, hello... Sorry, you've caught me at a bad time, it's just that I've had a sign you see and I need to find my journal.").also { stage = 7 }
            30 -> npcl(FaceAnim.HALF_GUILTY, "Thanks for the journal, I've been reading it. It looks like I came to a violent and bitter end but that's not really important. I just have to figure out what I am going to do now?").also { stage = 14 }
            ...
            45 -> {
                if (inInventory(player, Items.MORT_MYRE_FUNGUS_2970)) {
                    ...
                    playerl(FaceAnim.NEUTRAL, "Hello, I've been blessed but I don't know what to do now.").also { stage = 70 }
                }
            }

            50 -> npcl(FaceAnim.NEUTRAL, " Hello again! I don't suppose you've found out what the other components of the Nature spell are have you?").also { stage = 90 }
        ...
        when (stage) {
            ...
            7 -> {
                if (!inInventory(player, Items.JOURNAL_2967)) {
                    playerl(FaceAnim.NEUTRAL, "Where did you put it?").also { stage++; setQuest(25) }
            ...
            55 -> sendDialogue(*splitLines("The druid produces a small sheet of papyrus with some writing on it.")).also {
                addItemOrDrop(
                    player,
            ...
            62 ->
                if (inInventory(player, Items.DRUIDIC_SPELL_2968) || inBank(player, Items.DRUIDIC_SPELL_2968)) end()
                else playerl(FaceAnim.NEUTRAL, "Could I have another bloom scroll please?").also { stage++ }

            63 -> npcl(FaceAnim.NEUTRAL, "Sure, but please look after this one.").also { stage++ }
            64 -> sendDialogue("The spirit of Filliman Tarlock gives you another bloom spell.").also {
                addItemOrDrop(
                    player,
                    Items.DRUIDIC_SPELL_2968
                ); stage = END_DIALOGUE
            ...
            75 -> sendDialogue(
                ...
                "another bloom spell."
            ).also { addItem(player, Items.DRUIDIC_SPELL_2968); stage = END_DIALOGUE }
            ...
            1003 -> when (buttonId) {
                1 -> playerl(FaceAnim.NEUTRAL, "I'm wearing an amulet of ghost speak!").also {
                    stage = 1010; setQuest(20)
                }

                2 -> playerl(FaceAnim.NEUTRAL, "How long have you been a ghost?").also { stage = 1020; setQuest(20) }
                3 -> playerl(FaceAnim.NEUTRAL, "What's it like being a ghost?").also { stage = 1030; setQuest(20) }
            ...
            1016 -> npcl(FaceAnim.HALF_GUILTY, "Don't be silly now! That in no way reflects the truth!").also {
                stage = 1002
            }
