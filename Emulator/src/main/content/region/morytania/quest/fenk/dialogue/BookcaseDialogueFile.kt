...
class BookcaseWest : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (stage) {
            ...
            1 -> when (buttonID) {
                1 -> sendDialogue(player!!, "This book leaves you contemplating vegetarianism.").also {
                    stage = END_DIALOGUE
                }
                ...
                4 -> sendDialogue(
                    player!!,
                    "As you pull the book a hidden latch springs into place, and the bookcase swings open, revealing a secret compartment."
                ).also { stage++ }
            }
            ...
            2 -> {
                sendItemDialogue(
                    player!!,
                    Items.MARBLE_AMULET_4187,
                    "You find a marble amulet in the secret compartment."
                )
                end()
...
class BookcaseEastDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            START_DIALOGUE -> sendDialogueOptions(
                player!!,
                "Which book would you like to read?",
            ...
            1 -> when (buttonID) {
                1 -> sendDialogue(
                    player!!,
                    "You discover some fascinating insights into the mind of the $opposingGender kind."
                ).also { stage = END_DIALOGUE }
            ...
            2 -> {
                ...
                if (getQuestStage(player!!, Quests.CREATURE_OF_FENKENSTRAIN) == 2) {
                    sendItemDialogue(player!!, Items.OBSIDIAN_AMULET_4188, "You find an obsidian amulet in the secret compartment.")
