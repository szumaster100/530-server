...
class CanifisCitizenDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (inEquipment(player, Items.RING_OF_CHAROS_4202) || inEquipment(player, Items.RING_OF_CHAROSA_6465)) {
            when (randomWithRing) {
                0 -> npc(FaceAnim.HALF_GUILTY, "I haven't smelt you around here before...").also { stage = END_DIALOGUE }
                1 -> npc(FaceAnim.HALF_GUILTY, "If you catch anyone promise me you'll share.").also { stage = END_DIALOGUE }
                2 -> npc(FaceAnim.HALF_GUILTY, "You look to me like someone with a healthy taste for blood.").also { stage = END_DIALOGUE }
                3 -> npc(FaceAnim.HALF_GUILTY, "Fancy going up to the castle for a bit of a snack?").also { stage = END_DIALOGUE }
                4 -> npc(FaceAnim.HALF_GUILTY, "I have no interest in talking to a pathetic", "meat bag like yourself.").also { stage = END_DIALOGUE }
                5 -> npc(FaceAnim.HALF_GUILTY, "I bet you have wonderful paws.").also { stage = END_DIALOGUE }
                6 -> npc(FaceAnim.HALF_GUILTY, "Seen any humans around here?", "I'm v-e-r-y hungry.").also { stage = END_DIALOGUE }
                ...
                9 -> npc(FaceAnim.HALF_GUILTY, "A very miserable day, altogether...", "enjoy it while it lasts.").also { stage = END_DIALOGUE }
            }
        } else {
            when (randomWithoutRing) {
                10 -> npc(FaceAnim.HALF_GUILTY, "I have no interest in talking to a pathetic", "meat bag like yourself.").also { stage = END_DIALOGUE }
                ...
                14 -> npc(FaceAnim.HALF_GUILTY, "I don't have anything to give you so leave me alone,", "mendicant.").also { stage = END_DIALOGUE }
                15 -> npc(FaceAnim.HALF_GUILTY, "I don't have time for this right now.").also { stage = END_DIALOGUE }
                16 -> npc(FaceAnim.HALF_GUILTY, "Don't talk to me again if you value your life!").also { stage = END_DIALOGUE }
