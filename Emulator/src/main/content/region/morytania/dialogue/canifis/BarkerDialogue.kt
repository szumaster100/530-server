...
class BarkerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FaceAnim.HAPPY, "You are looking for clothes, yes? You look at my", "products! I have very many nice clothes, yes?").also { stage++ }
            ...
            4 -> npc(FaceAnim.HALF_GUILTY, "Unfortunate for you, yes?", "Many bargains, won't find elsewhere!").also { stage = END_DIALOGUE }
        }
        return true
    }
