...
class FlorinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            6 -> npcl(FaceAnim.HALF_GUILTY, "Not as far as I know, we have some basic amenities now which is so very useful, we're all incredibly grateful to you. I've never really seen or known a hero before, so I have nothing to compare one to, but I suspect in").also { stage++ }
            7 -> npcl(FaceAnim.HALF_GUILTY, "most people's judgement, you would be one! I for one am very pleased to have met you.").also { stage = 1 }
        }
        return true
    }
