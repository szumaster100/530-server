...
class DalcianFangDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npcl(FaceAnim.HALF_GUILTY, "Oh, Paterdomus is that rather impressive looking temple on the Salve... I'm sure you must have passed it on your way into Morytania.").also { stage = END_DIALOGUE }
            4 -> npcl(FaceAnim.HALF_GUILTY, "I'm hoping to get back to civilisation and see what the lands of Misthalin look like.").also { stage = END_DIALOGUE }
            5 -> npcl(FaceAnim.HALF_GUILTY, "Let's just say that I'm somewhat experienced and I'm happy to take on my fair share of bad guys. You don't need to worry about that.").also { stage = END_DIALOGUE }
            6 -> if (!player.familiarManager.hasFamiliar()) {
                npcl(FaceAnim.HALF_GUILTY, "Oh really? That would be great! I'll give you a small reward once we reach our destination.").also { stage++ }
            } else {
                end()
                npcl(FaceAnim.HALF_GUILTY, "Hmmm, it seems that you have a follower. I think it may get lost on this journey if it's left to wander on its own. Let's leave when your follower is safe.")
            }
