...
class VasileDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (getQuestStage(player, Quests.IN_AID_OF_THE_MYREQUE) < 1) {
            ...
            npcl(FaceAnim.HALF_GUILTY, "Hey it's the Hero! Wow! You're amazing, how on earth did you manage to tackle those two juvinates and Gadderanks? And the town is lovely, you've done a great job!")
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npcl(FaceAnim.HALF_GUILTY, "Huh! I'm Vasile. Ravaging after food is what I mostly do. Occasionally, I'll happen across a rat - and that's a good day - just like today!").also { stage = 1 }
            4 -> npcl(FaceAnim.HALF_GUILTY, "It's just perfect, thanks so much for your efforts.").also { stage = 1 }
            5 -> npcl(FaceAnim.HALF_GUILTY, "Oh, not much really, we're all still hiding from the vampyres and eking out a living from this terrible place, hopefully soon we'll be strong enough to make the journey to the temple on the Salve.").also { stage = 1 }
            6 -> npcl(FaceAnim.HALF_GUILTY, "Not as far as I know, we have some basic amenities now which is so very useful, we're all incredibly grateful to you. I've never really seen or known a hero before, so I have nothing to compare one to, but I suspect in most people's judgement, you would be one!").also { stage++ }
