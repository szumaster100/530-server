...
class RobinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            13 -> npcl(FaceAnim.FRIENDLY, "Oh charming. You just can't get the staff these days.").also {
                stage = END_DIALOGUE
            }

            14 -> npcl(FaceAnim.FRIENDLY, "Well it's about time. Run along now, I need some Robin time.").also {
                stage = END_DIALOGUE
            }
        }
        return true
