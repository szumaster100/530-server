...
class NecrovarusDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        when {
            isQuestComplete(player, Quests.GHOSTS_AHOY) -> player(
                "Told you I'd defeat you, Necrovarus. My advice to",
                "you is to pass over to the next world yourself with",
                "everybody else."
            ).also { stage = 4 }

            !isQuestComplete(player, Quests.GHOSTS_AHOY) -> openDialogue(player, NecrovarusDialogueFile())
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npc(FaceAnim.ANNOYED, "Speak to me again and I will rend the soul from your", "flesh.").also {
                stage = END_DIALOGUE
            }

            2 -> npc(FaceAnim.ANNOYED, "You dare to speak to me??? Have you lost your", "wits????").also {
                stage = END_DIALOGUE
            }
