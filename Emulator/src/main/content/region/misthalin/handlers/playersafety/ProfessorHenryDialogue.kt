        ...
        when (stage) {
            START_DIALOGUE -> {
                if (player.savedData.globalData.getTestStage() == 2 && inInventory(player, Items.TEST_PAPER_12626)) {
                    ...
                    npcl(FaceAnim.HAPPY, "Good job ${player.name}, you completed the test!").also { stage = END_DIALOGUE }
                    return true
                } else {
            ...
            14 -> npcl(FaceAnim.HAPPY, "Excellent! Allow me to reward you for your work. " + "I have these two old lamps that you may find useful.").also { stage++ }
            15 -> npc("Also, there is an old jail block connected to the cells", "below the training centre, which have been overrun with", "vicious creatures. If you search around the jail cells", "downstairs, you should find it easily enough.").also { stage++ }
            ...
            17 -> {
                ...
                if (freeSlots(player) >= 1) {
                    ...
                    npcl(FaceAnim.SAD, "You do not have space in your inventory for the rewards").also { stage = END_DIALOGUE }
                }
            }
            ...
            110 -> npcl(FaceAnim.HALF_GUILTY, "Here you can take part in the Player Safety test: " + "a set of valuable lessons to learn about staying safe " + "in ${ServerConfig.SERVER_NAME}.").also { stage++ }
            ...
            200 -> {
                if (freeSlots(player) == 0) {
                    ...
                    npcl(FaceAnim.HALF_GUILTY, "You already have a test, please fill it out and return it to me.").also { stage = END_DIALOGUE }
                } else {
                    player.savedData.globalData.setTestStage(1)
                    addItem(player, Items.TEST_PAPER_12626)
                    npcl(FaceAnim.HALF_GUILTY, "Right then. Here is the test paper. " + "When you have completed all the questions, bring it back to me for marking.").also { stage++ }
                }
            }
