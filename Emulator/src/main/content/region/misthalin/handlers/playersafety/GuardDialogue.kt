...
class GuardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npc(FaceAnim.NEUTRAL, "You must learn about player safety before entering the", "training centre.").also { stage++ }
            4 -> player(FaceAnim.HALF_ASKING, "Oh. How do I do that?").also { stage++ }
            5 -> npc(FaceAnim.NEUTRAL, "Each of these gublinches have been caught breaking the", "Rules of ${settings!!.name}. You should read the plaques on", "each of their cells to learn what they did wrong.").also { stage++ }
            6 -> player(FaceAnim.SAD, "Oh, right. I can enter the training centre once I have", "done that?").also { stage++ }
            7 -> npc(FaceAnim.NEUTRAL, "Yes. Once you have examined each of the plaques,", "come and speak to me and I will tell you about the", "Report Abuse function.").also { stage++ }
            8 -> npc(FaceAnim.NEUTRAL, "After that, I can let you into the training centre,", "upstairs.").also { stage++ }
            9 -> {
                end()
            ...
            20 -> npc(FaceAnim.HALF_GUILTY, "That's it! It really is that simple and it only takes a", "moment to do. Now you may enter the training", "centre. Good luck, citizen.").also { stage++ }
