...
class CradleOfLifeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            7 -> end()
            100 ->
                if (!inInventory(player, Items.FANCY_BOOTS_9005, 1) && !inBank(player, Items.FIGHTING_BOOTS_9006, 1) && !inEquipment(player, Items.FANCY_BOOTS_9005, 1) && !inInventory(player, Items.FIGHTING_BOOTS_9006, 1) && !inBank(player, Items.FIGHTING_BOOTS_9006, 1) && !inBank(player, Items.FIGHTING_BOOTS_9006, 1)) {
            ...
            200 -> when (buttonId) {
                1 -> playerl(FaceAnim.HALF_GUILTY, "Yes, I'd like the other pair instead please!").also { stage = 800 }
