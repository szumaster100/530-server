...
@Initializable
class HistorianEastDialogue(player: Player? = null) : Dialogue(player) {
...
class TalkAboutSnails : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            0 -> npcl(FaceAnim.HALF_GUILTY, "Snails move like worms by squishing up and stretching out, very, very slowly.").also { stage++ }
            1 -> npcl(FaceAnim.HALF_GUILTY, "They also make a lot of slime, in order to aid moving by reducing friction.").also { stage++ }
            2 -> npcl(FaceAnim.HALF_GUILTY, "They also use slime for protection. For instance, snails can use their slime to crawl over razor-blades without being hurt. It also helps keep away dangerous insects like ants.").also { stage++ }
            3 -> npcl(FaceAnim.HALF_GUILTY, "When they hide in their shells, snails secrete a special type of slime, which dries to cover the entrance of their shells like a 'trapdoor'.").also { stage++ }
            4 -> npcl(FaceAnim.HALF_GUILTY, "This is called an operculum. The snails of Morytania are the most malignant molluscs ever to have been studied.").also { stage++ }
            5 -> npcl(FaceAnim.HALF_GUILTY, "They are broken down into two distinct species: achatina acidia and achatina acidia giganteus or, as they are more commonly known, the acid-spitting snail and the giant").also { stage++ }
            6 -> npcl(FaceAnim.HALF_GUILTY, "acid-spitting snail. Both of these varieties are voracious carnivores, using their mutated mouthpieces to spit a glob of powerful acid to kill their foe.").also { stage++ }
            7 -> npcl(FaceAnim.HALF_GUILTY, "They then simply have to wait, whilst the digestive juices make short work of the poor creature. Then, they simply slurp up what remains.").also { stage++ }
            8 -> npcl(FaceAnim.HALF_GUILTY, "How these strange creatures came to be is still something of a mystery.").also { stage++ }
            9 -> npcl(FaceAnim.HALF_GUILTY, "The most prevalent theory suggests that they mutated, as a reaction to an 'as yet unknown' pollutant that has appeared in the swamps.").also { stage++ }
            10 -> npcl(FaceAnim.HALF_GUILTY, "The local populace has capitalised on the appearance of these strange species, using their shells to fashion a rudimentary helm that is fairly resistant to the snails acid.").also { stage++ }
            11 -> npcl(FaceAnim.HALF_GUILTY, "Other known uses of snail by-products include a tasty local delicacy and a fireproof oil.").also { stage++ }
            12 -> npcl(FaceAnim.HALF_GUILTY, "And this concludes my short lecture on snails. I hope you've enjoyed yourselves.").also { stage++ }
            13 -> {
                end()
                unlock(player!!)
...
class TalkAboutMonkeys : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        npc = NPC(NPCs.NATURAL_HISTORIAN_5967)
        when (stage) {
            0 -> npcl(FaceAnim.HALF_GUILTY, "A monkey is a member of either of two known groupings of simian primates.").also { stage++ }
            1 -> npcl(FaceAnim.HALF_GUILTY, "These two known groupings are the Karamja monkeys and the 'harmless' monkeys.").also { stage++ }
            2 -> npcl(FaceAnim.HALF_GUILTY, "Because of their similarity to monkeys, apes such as chimpanzees and gibbons are often called monkeys by accident.").also { stage++ }
            3 -> npcl(FaceAnim.HALF_GUILTY, "Though some natural historians don't consider them to be monkeys. Also, a few monkey species have the word 'ape' in their common name.").also { stage++ }
            4 -> npcl(FaceAnim.HALF_GUILTY, "The Karamja monkeys are rumoured to be fairly cunning and intelligent creatures, although rumours that they have learned human speech is anecdotal at best.").also { stage++ }
            5 -> npcl(FaceAnim.HALF_GUILTY, "In appearance, they stand much shorter than a human and tend to move in a hunched fashion.").also { stage++ }
            6 -> npcl(FaceAnim.HALF_GUILTY, "Karamja monkeys also sport a red mohawk, though it is unknown whether this is an affectation or not.").also { stage++ }
            7 -> npcl(FaceAnim.HALF_GUILTY, "They are very fond of bananas and bitternuts, eating them in huge quantities whenever they can get their paws on them.").also { stage++ }
            8 -> npcl(FaceAnim.HALF_GUILTY, "The harmless monkeys of Mos Le'Harmless are a very similar, but in some ways entirely different, breed.").also { stage++ }
            9 -> npcl(FaceAnim.HALF_GUILTY, "They stand roughly the same size but are a lighter colour.").also { stage++ }
            10 -> npcl(FaceAnim.HALF_GUILTY, "Interestingly, Karamaja monkeys have a deep dislike of seaweed, though this may stem from the actions of a number of irresponsible people.").also { stage++ }
