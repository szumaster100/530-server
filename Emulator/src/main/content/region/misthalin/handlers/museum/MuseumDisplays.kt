...
class MuseumDisplays : InterfaceListener, InteractionListener {

    val interfaceModel = "ifaces:534:model"
    val smallInterfaceModel = "small$interfaceModel"
    val displayItems = intArrayOf(
    ...
    val removedForCleaning = arrayOf(
    ...
    override fun defineListeners() {
        ...
        on(Scenery.DISPLAY_CASE_24627, IntType.SCENERY, "open") { player, _ ->
            ...
            if (inInventory(player, Items.STATUETTE_4618) || inBank(player, Items.STATUETTE_4618) || player.getAttribute("the-golem:placed-statuette", false)) {
        ...
        onUseWith(IntType.SCENERY, displayItems, Scenery.DISPLAY_CASE_24550) { player, used, with ->
            when(with.location) {
                Location.create(3262, 3448, 0) -> if(used.id == Items.OLD_SYMBOL_11182) {
                    removeItem(player, used.asItem(), Container.INVENTORY)
                    setVarbit(player, Vars.VARBIT_SCENERY_MUSEUM_DISPLAY_37_3646, 1, true)
                }
                Location.create(3255, 3453, 1) -> if(used.id == Items.DISPLAY_CABINET_KEY_4617) {
                    sendMessage(player, "You have already taken the statuette.")
                }
                Location.create(3259, 3448, 0) -> if(used.id == Items.OLD_COIN_11179) {
                    removeItem(player, used.asItem(), Container.INVENTORY)
                    setVarbit(player, Vars.VARBIT_SCENERY_MUSEUM_DISPLAY_45_3648, 1, true)
                }
                Location.create(3259, 3450, 0) -> if(used.id == Items.ANCIENT_COIN_11180) {
                    removeItem(player, used.asItem(), Container.INVENTORY)
                    setVarbit(player, Vars.VARBIT_SCENERY_MUSEUM_DISPLAY_44_3647, 1, true)
                }
                Location.create(3262, 3450, 0) -> if(used.id == Items.ANCIENT_SYMBOL_11181) {
                    removeItem(player, used.asItem(), Container.INVENTORY)
                    setVarbit(player, Vars.VARBIT_SCENERY_MUSEUM_DISPLAY_36_3645, 1, true)
                }
            }
            if(used.id != 4617) {
                sendMessages(player, "You carefully place the ${getItemName(used.id).lowercase()} in the display and update.", "the information.")
            }
