    ...
    override fun defineListeners() {
        ...
        on(Scenery.WALL_25161, IntType.SCENERY, "climb-over") { player, _ ->
            if (player.location.x >= 2847) {
                if (getQuestStage(player, Quests.DRAGON_SLAYER) == 40 && !inInventory(
                        player,
        ...
        onUseWith(IntType.ITEM, mapPieces, *mapPieces) { player, _, _ ->
            if (!inInventory(player, Items.MAP_PART_1537) || !inInventory(player, Items.MAP_PART_1536) || !inInventory(
                    player,
                    Items.MAP_PART_1535
                )
            ) {
                sendMessage(player, "You don't have all the map pieces yet.")
            } else if (removeItem(player, Items.MAP_PART_1537) && removeItem(player, Items.MAP_PART_1536) && removeItem(
                    player,
                    Items.MAP_PART_1535
                )
            ) {
