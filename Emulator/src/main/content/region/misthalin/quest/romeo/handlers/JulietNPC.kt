...
class JulietNPC : AbstractNPC {
    ...
    override fun isHidden(player: Player): Boolean {
        return player.getQuestRepository().getQuest(Quests.ROMEO_JULIET)
            .getStage(player) > 60 && player.getQuestRepository().getQuest(Quests.ROMEO_JULIET).getStage(player) < 100
    }
