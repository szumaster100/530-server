...
class ShieldOfArravPlugin : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        ...
        when (id) {
                ...
                "search" -> {
                    if (quest.getStage(player) == 70 && ShieldofArrav.isBlackArm(player)) {
                        if (!player.inventory.containsItem(ShieldofArrav.BLACKARM_SHIELD) &&
                            ...
                            sendItemDialogue(
                                player, ShieldofArrav.BLACKARM_SHIELD.id,
            ...
            2398 -> {
                if (quest.getStage(player) == 60 && ShieldofArrav.isBlackArmMission(player) && !inInventory(
                        player,
            ...
            2402 -> {
                if (quest.getStage(player) == 10 && !inInventory(player, ShieldofArrav.BOOK.id) && !inBank(
                        player,
