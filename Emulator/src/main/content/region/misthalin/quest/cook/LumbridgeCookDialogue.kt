...
class LumbridgeCookDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (getQuestStage(player, Quests.COOKS_ASSISTANT) <= 0) {
            ...
            if (getAttribute(player, "cooks_assistant:all_submitted", false) || (getAttribute(
                    player,
                    "cooks_assistant:milk_submitted",
                    false
                ) && getAttribute(player, "cooks_assistant:flour_submitted", false) && getAttribute(
                    player,
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (getQuestStage(player, Quests.THE_LOST_TRIBE) == 10) {
            when (stage) {
                ...
                4 -> npc("Thank you, ${player.name}! If you can convince the Duke", "I'm telling the truth then we can get to the bottom of", "this mystery.").also {
                    stage = 1000
        ...
        when (stage) {
            ...
            1 -> when (buttonId) {
                ...
                3 -> player(FaceAnim.NEUTRAL, "You don't look very happy.").also { stage = 30; }
                4 -> player(FaceAnim.HAPPY, "Nice hat!").also { stage = 40 }
            }

            10 -> npc(FaceAnim.SCARED, "Oh dear, oh dear, oh dear, I'm in a terrible terrible", "mess! It's the Duke's birthday today, and I should be", "making him a lovely big birthday cake.").also { stage++ }

            11 -> npc(FaceAnim.SAD, "I've forgotten to buy the ingredients. I'll never get", "them in time now. He'll sack me! What will I do? I have", "four children and a goat to look after. Would you help", "me? Please?").also { stage++ }
            ...
            14 -> npc(FaceAnim.SAD, "Fine. I always knew you Adventurer types were callous", "beasts. Go on your merry way!").also { stage = 1000 }

            20 -> npc(FaceAnim.SAD, "*sniff* Don't talk to me about cakes...").also { stage++ }
            21 -> player(FaceAnim.NEUTRAL, "What's wrong?").also { stage = 10 }
            30 -> npc(FaceAnim.SAD, "No, I'm not. The world is caving in around me - I am", "overcome by dark feelings of impending doom.").also { stage++ }
            ...
            33 -> npc(FaceAnim.SAD, "No, that's the worst thing I could do. I'd get in terrible", "trouble.").also { stage++ }

            34 -> player(FaceAnim.ASKING, "Well maybe you need to take a holiday...").also { stage++ }
            35 -> npc(FaceAnim.SAD, "That would be nice, but the Duke doesn't allow holidays", "for core staff").also { stage++ }

            36 -> player(FaceAnim.SUSPICIOUS, "Hmm, why not run away to the sea and start a new", "life as a Pirate?").also { stage++ }

            37 -> npc(FaceAnim.SAD, "My wife gets sea sick, and I have an irrational fear of", "eyepatches. I don't see it working myself.").also { stage++ }
            ...
            43 -> player(FaceAnim.HAPPY, "The whole hat, apron, stripy trousers ensemble. It", "works. It makes you looks like a real cook.").also { stage++ }

            44 -> npc(FaceAnim.ANGRY, "I AM a real cook! I haven't got time to be chatting", "about culinary fashion. I'm in desperate need of help!").also { stage = 21 }

            50 -> npc(FaceAnim.HAPPY, "Oh thank you, thank you. I need milk, an egg and", "flour. I'd be very grateful if you can get them for me.").also { player?.questRepository?.getQuest(Quests.COOKS_ASSISTANT)?.start(player!!); stage++ }
            ...
            61 -> when (buttonId) {
                1 -> npc(FaceAnim.NEUTRAL, "There is a Mill fairly close, go North and then West.", "Mill Lane Mill is just off the road to Draynor. I", "usually get my flour from there.").also { stage = 70 }
                2 -> npc(FaceAnim.NEUTRAL, "There is a cattle field on the other side of the river,", "just across the road from the Groats' Farm.").also { stage = 71 }
                3 -> npc(FaceAnim.NEUTRAL, "I normally get my eggs from the Groats' farm, on the", "other side of the river.").also { stage = 73 }
                4 -> player(FaceAnim.NEUTRAL, "Actually, I know where to find this stuff.").also { stage = 1000 }
            }

            70 -> npc(FaceAnim.SUSPICIOUS, "Talk to Millie, she'll help, she's a lovely girl and a fine", "Miler. Make sure you take a pot with you for the flour", "though, " + if (inInventory(player, EMPTY_POT, 1)) "you've got one on you already." else "there should be one on the table in here.").also { stage = 80 }

            71 -> npc(FaceAnim.SUSPICIOUS, "Talk to Gillie Groats, she looks after the DairyProduct cows -", "she'll tell you everything you need to know about", "milking cows!").also { stage++ }

            72 ->
                if (inInventory(player, EMPTY_BUCKET, 1)) {
                    npc(FaceAnim.NEUTRAL, "You'll need an empty bucket for the milk itself. I do see", "you've got a bucket with you already luckily!").also { stage = 80 }
                } else {
                    npc(FaceAnim.NEUTRAL, "You'll need an empty bucket for the milk itself. The", "general store just north of the castle will sell you one", "for a couple of coins.").also { stage = 80 }
                }
            ...
            100 ->
                if (!getAttribute(player, "cooks_assistant:milk_submitted", false) && inInventory(player, MILK, 1)) {
                    ...
                    if (gave) {
                        ...
                        if (getAttribute(player, "cooks_assistant:milk_submitted", false) && getAttribute(
                                player,
                                "cooks_assistant:flour_submitted",
                                false
                            ) && getAttribute(player, "cooks_assistant:egg_submitted", false)
                        ) {
                            npc(FaceAnim.HAPPY, "You've brought me everything I need! I am saved!", "Thank you!").also { setAttribute(player, "/save:cooks_assistant:all_submitted", true); stage = 200 }
                        } else {
                            npc(FaceAnim.WORRIED, "Thanks for the ingredients you have got so far, please get", "the rest quickly. I'm running out of time! The Duke", "will throw me into the streets!").also { stage = 151 }
                        }
                    } else {
                        if (!getAttribute(player, "cooks_assistant:submitted_some_items", false)) {
                            player(FaceAnim.NEUTRAL, "I haven't gotten any of them yet, I'm still looking.").also { stage = 155 }
                        } else {
            ...
            150 -> npc(FaceAnim.WORRIED, "Thanks for the ingredients you have got so far, please get", "the rest quickly. I'm running out of time! The Duke", "will throw me into the streets!").also { stage++ }
            ...
            155 -> npc(FaceAnim.WORRIED, "Please get the ingredients quickly. I'm running out of", "time! The Duke will throw me into the streets!").also { stage++ }
            ...
            161 -> when (buttonId) {
                ...
                2 -> player(FaceAnim.ASKING, "Can you remind me how to find these things again?").also {
                    stage = 60
                }
            }

            200 -> player(FaceAnim.HAPPY, "So do I get to go to the Duke's Party?").also { stage++ }
            201 -> npc(FaceAnim.SAD, "I'm afraid not, only the big cheeses get to dine with the", "Duke.").also { stage++ }

            202 -> player(FaceAnim.NEUTRAL, "Well, maybe one day I'll be important enough to sit on", "the Duke's table.").also { stage++ }
            ...
            330 -> npc(FaceAnim.HAPPY, "Go ahead! It's a very good range; it's better than most", "other ranges.").also { stage++ }

            331 -> npc(FaceAnim.NEUTRAL, "It's called the Cook-o-Matic 100 and it uses a combination", "of state-of-the-art temperature regulation and magic.").also { stage++ }
