...
class AvaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when (quest!!.getStage(player)) {
            0 -> npc("Hello there and welcome to my humble abode. It's sadly", "rather more humble than I'd like, to be honest, although", "perhaps you can help with that?")
            10 -> npc("Do you need a reminder what you are supposed to do?", "I know you must have shiny beads distracting you, but", "it is an important job you are doing for me.")
            20 -> npc("My spiritometric devices show that you have been in", "close contact with ghostly animals. Are we closer to", "success?")
            25 -> npc("Go and talk to the Witch next door. She'd talk the hind", "legs off a donkey but she can select the iron with which", "it is suitable for the chicken to interact.")
            27 ->
            ...
            29, 30 -> player("Well, I tried to hack the trees with my axe, but it just", "bounced off the trunk! It did all seem all-too-convenient", "to work on the first try.")
            ...
            100 -> npc("I'm busy with my newest research, so can't gossip too", "much. Are you after an upgrade to your device, or a", "new device, or some information, of would you like to", "see my goods for sale?")
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            ...
            20 -> when (stage) {
                ...
                5 -> {
                    player("Obvious, yes, but why on " + settings!!.name + " would you need", "an undead chicken when there are perfectly good live", "chickens just down the road?")
                    stage++
                }
                ...
                10 -> {
                    npc("We'll need a magnet next, one with purely natural", "fields and made from a carefully selected iron bar. A", "firm impact when the iron is parallel to " + settings!!.name + "'s", "field will stabilise this field in the rod.")
                    stage++
                }
            ...
            27 -> when (stage) {
                ...
                5 -> {
                    npc("The plan is that the chicken will operate the magnet to", "attract bits of iron and steel, maybe even your own", "recently fired arrows. There are plenty of totally lost", "arrowheads lying about in the Fields of " + settings!!.name + ", I")
                    stage++
                }
            ...
            100 -> when (stage) {
                ...
                17 -> {
                    if (!inInventory(player, Items.AVAS_ATTRACTOR_10498) && getStatLevel(player, Skills.RANGE) > 50) {
                        ...
                        npc(FaceAnim.HAPPY, "You are ready to upgrade. I'll take 75 steel arrows and", "the old device, if that's all fine with you?").also { stage++ }
                    }
                }
