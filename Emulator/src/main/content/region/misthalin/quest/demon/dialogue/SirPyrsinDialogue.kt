...
class SirPyrsinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            30 -> when (stage) {
                0 ->
                    if (!inInventory(player, DemonSlayerUtils.SILVERLIGHT.id) && !inBank(player, DemonSlayerUtils.SILVERLIGHT.id) && !inEquipment(player, DemonSlayerUtils.SILVERLIGHT.id)) {
                        ...
                        object : Pulse(1, player) {
                            ...
                            override fun pulse(): Boolean {
                                when (counter++) {
                                    ...
                                    12 -> {
                                        npc.transform(883)
                                        if (player.inventory.remove(DemonSlayerUtils.FIRST_KEY, DemonSlayerUtils.SECOND_KEY, DemonSlayerUtils.THIRD_KEY)) {
