...
class WoodcuttingTutorDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (getStatLevel(player, Skills.WOODCUTTING) >= 99) {
            npc(FaceAnim.HALF_GUILTY, "Wow! It's not often I meet somebody as accomplished", "as me in Woodcutting! Seeing as youre so skilled,", "maybe you are interested in buying a Skillcape of", "Woodcutting?").also { stage = 100 }
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            10 -> npc(FaceAnim.HALF_GUILTY, "My name is Wilfred and I'm the best woodsman in", "Asgarnia! I've spent my life studying the best methods for", "woodcutting. That's why I have this cape, the Skillcape of", "Woodcutting.").also { stage = END_DIALOGUE }
            20 -> npc(FaceAnim.HALF_GUILTY, "This is a Skillcape of Woodcutting. Only a person who has", "achieved the highest possible level in a skill can wear one.").also { stage = END_DIALOGUE }
            ...
            1000 -> {
                if (freeSlots(player) == 1) {
                    player(FaceAnim.HALF_GUILTY, "Sorry, I don't have enough inventory space.").also { stage = END_DIALOGUE }
                    return true
                }
                if (player.inventory.contains(Items.COINS_995, 99000)) {
                    player.inventory.remove(Item(Items.COINS_995, 99000))
                    player.inventory.add(Item(Items.WOODCUTTING_CAPE_9807 + (if (player.getSkills().masteredSkills > 1) 1 else 0)))
            ...
            511 -> when (buttonId) {
                ...
                3 -> sendItemDialogue(player, Items.MAGIC_LOGS_1513, "Magic trees are... magic. A difficult wood to work with, but worth it for the rewards. Find them in the areas south of Seers village or on the East side of the Mage arena.").also { stage = 5300 }
                4 -> sendItemDialogue(player, Items.BRONZE_AXE_1351, "Bronze axes are easy to get, simply go visit Bob in his shop in Lumbridge, or talk to me if you have mislaid yours.").also { stage = 5400 }
            ...
            5401 -> sendDoubleItemDialogue(player, Items.IRON_AXE_1349, Items.STEEL_AXE_1353, "As your woodcutting skill increases you will find yourself able to use better axes to chop trees faster.... anything up to steel you can buy from Bob's axe shop.").also { stage++ }
            5402 -> sendItemDialogue(player, Items.RUNE_AXE_1359, "Rune axes can be player made with very high level smithing and mining. They can also be obtained through killing one of the fearsome tree spirits, though this is very rare.").also { stage = 510 }
