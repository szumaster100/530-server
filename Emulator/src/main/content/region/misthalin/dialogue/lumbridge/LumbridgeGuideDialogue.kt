...
class LumbridgeGuideDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            52 -> npcl(FaceAnim.FRIENDLY, "Should you choose to step away from the path of the ironman now, you will not have the option to return.").also { stage++ }
            53 -> npcl(FaceAnim.FRIENDLY, "Now I ask you to make sure, are you sure you want to <b>permanently</b> remove ironman mode?").also { stage++ }
            54 -> showTopics(
            ...
            56 ->
                if (player.ironmanManager.mode == IronmanMode.HARDCORE) {
                    npcl(FaceAnim.WORRIED, "Oh, dear, it's just as I feared. You're a hardcore ironman! My apologies, but there's nothing I can do to help.").also { stage = END_DIALOGUE }
                } else {
                    npcl(FaceAnim.FRIENDLY, "Oh, wonderful. It appears everything is in order. Sit still for a moment...").also { stage++ }
                }
            ...
            58 -> {
                ...
                npcl(FaceAnim.HALF_ASKING, "There, I believe it is done. You should no longer be restricted from the wider world.")
                stage = END_DIALOGUE
            }
        }
        return true
