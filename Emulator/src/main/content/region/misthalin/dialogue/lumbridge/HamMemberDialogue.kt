...
class HamMemberDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npcl(FaceAnim.FRIENDLY, "I'm a strong believer in the non-monsters policy...we should really get rid of them...and if that means I have to live in a cave like a monster, so be it!").also { stage = 2 }
            5 -> npcl(FaceAnim.FRIENDLY, "We want a world without monsters, to live in safety and without fear of being attacked by these ferocious beasts.").also { stage++ }
            6 -> playerl(FaceAnim.HALF_GUILTY, "But there aren't that many ferocious beasts in the towns and cities.").also { stage++ }
            7 -> npcl(FaceAnim.FRIENDLY, "That's not enough, we want to get rid of them totally, we want to enjoy the surrounding lands and not worry about our children playing in caves and so on.").also { stage = 2 }
            8 -> npcl(FaceAnim.FRIENDLY, "Most of us came from small towns that had been attacked by monsters. We all got fed up with it and so decided to join this movement.").also { stage++ }
            9 -> npcl(FaceAnim.NEUTRAL, "We're hoping to return to the towns and cities when we've cleaned up the areas that these monsters live in.").also { stage = 2 }
        }
        return true
    }
