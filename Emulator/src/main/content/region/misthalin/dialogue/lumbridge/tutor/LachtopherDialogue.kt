...
class LachtopherDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npcl(FaceAnim.HALF_GUILTY, "Hello, I suppose. I'm Lachtopher. Could you lend me some money?").also { stage++ }
            1 -> playerl(FaceAnim.HALF_GUILTY, "Lend you money? I really don't think so. Don't you have any of your own?").also { stage++ }
            2 -> npcl(FaceAnim.HALF_GUILTY, "I spent it all and I can't be bothered to earn any more.").also { stage++ }
            3 -> playerl(FaceAnim.HALF_GUILTY, "Right, and you want my hard-earned money instead? No chance!").also { stage++ }
            4 -> npcl(FaceAnim.HALF_GUILTY, "You're just like my sister, Victoria. She won't give me any money.").also { stage++ }
            5 -> playerl(FaceAnim.HALF_GUILTY, "Your sister sounds like she has the right idea.").also { stage++ }
            6 -> npcl(FaceAnim.HALF_GUILTY, "Yeah, I've heard it all before. 'Oh,' she says, 'It's easy to make money: just complete Tasks for cash.'").also { stage++ }
            7 -> playerl(FaceAnim.HALF_GUILTY, "Well, if you want to make money...").also { stage++ }
            8 -> npcl(FaceAnim.HALF_GUILTY, "That's just it. I don't want to make money. I just want to have money.").also { stage++ }
            9 -> playerl(FaceAnim.HALF_GUILTY, "I've had it with you! I don't think I've come across a less worthwhile person.").also { stage++ }
            10 -> playerl(FaceAnim.HALF_GUILTY, "I think I'll call you Lazy Lachtopher, from now on.").also { stage = END_DIALOGUE }
