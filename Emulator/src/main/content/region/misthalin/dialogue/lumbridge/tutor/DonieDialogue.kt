...
class DonieDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npc(FaceAnim.HALF_GUILTY, "Aye, not too bad thank you. Lovely weather in", settings!!.name + " this fine day.").also { stage++ }
            5 -> player(FaceAnim.HALF_GUILTY, "Weather?").also { stage++ }
            6 -> npc(FaceAnim.HALF_GUILTY, "Yes weather, you know.").also { stage++ }
            7 -> npc(FaceAnim.HALF_GUILTY, "The state or condition of the atmosphere at a time and", "place, with respect to variables such as temperature,", "moisture, wind velocity, and barometric pressure.").also { stage++ }
