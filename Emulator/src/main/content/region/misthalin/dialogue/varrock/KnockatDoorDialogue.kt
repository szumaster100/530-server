...
class KnockatDoorDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            3 -> when (buttonID) {
                1 -> player(FaceAnim.NEUTRAL, "I'm " + player!!.username + ". Please let me in.").also { stage = 10 }
            ...
            10 -> sendNPCDialogue(player!!, npcId, "No. Staff only beyond this point. You can't come in here.", FaceAnim.ANNOYED).also { stage = END_DIALOGUE }
            20 -> sendNPCDialogue(player!!, npcId, "Boo who?").also { stage++ }
            21 -> player(FaceAnim.NEUTRAL, "There's no need to cry!").also { stage++ }
            22 -> sendNPCDialogue(player!!, npcId, "What? I'm not... oh, just go away!", FaceAnim.ANNOYED).also { stage = END_DIALOGUE }
            30 -> sendNPCDialogue(player!!, npcId, "Kanga who?").also { stage++ }
            31 -> player(FaceAnim.NEUTRAL, "No, 'kangaroo'.").also { stage++ }
            32 -> sendNPCDialogue(player!!, npcId, "Stop messing about and go away!", FaceAnim.ANNOYED).also { stage = END_DIALOGUE }
            ...
            50 -> sendNPCDialogue(player!!, npcId, "Doctor. wh.. hang on, I'm not falling for that one again! Go away.", FaceAnim.ANNOYED).also { stage = END_DIALOGUE }
        }
    }
}