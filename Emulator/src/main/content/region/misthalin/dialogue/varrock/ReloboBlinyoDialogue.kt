...
class ReloboBlinyoDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> when (buttonId) {
                1 -> playerl(FaceAnim.HALF_GUILTY, "You look like you've travelled a fair distance.").also { stage = 3 }
            ...
            4 -> player(FaceAnim.HALF_GUILTY, "I don't mean to be rude, but the face paint and", "hair, for startes.").also { stage++ }
            5 -> npc(FaceAnim.HALF_GUILTY, "Ah, yes I'm from Shilo Village on Karamja. It's a style", "I've had since I was little.").also { stage++ }
            6 -> playerl(FaceAnim.HALF_GUILTY, "Then tell me, why are you so far from home?").also { stage++ }
            7 -> npc(FaceAnim.HALF_GUILTY, "This Grand Exchange! Isn't it marvellous I've never seen", "anything like it in my life. My people were not pleased to", "see me break traditions to visit such a place. But i hope", "to make some serious profit. then they'll see I was right!").also { stage++ }
            8 -> player(FaceAnim.HALF_GUILTY, "So, what are you selling?").also { stage++ }
            9 -> npc(FaceAnim.HALF_GUILTY, "Logs! of all kinds! That's my plan, at least. Nature", "is one thing my people understand very well.").also { stage = END_DIALOGUE }
