        ...
        when (stage) {
            ...
            1 -> when (buttonId) {
                ...
                3 -> if (inInventory(player, Items.FROG_TOKEN_6183)) {
                    npc(FaceAnim.FRIENDLY, "That entitles you to a free costume!", "Do you want a frog mask or a " + (if (player.isMale) "prince" else "princess") + " outfit?").also { stage = 52 }
                }
            }

            10 -> npc(FaceAnim.HAPPY, "I have a number of fine pieces of clothing on sale or,", "if you prefer, I can offer you an exclusive", "total clothing makeover?").also { stage++ }
            ...
            21 -> npc(FaceAnim.HAPPY, "Here at Thessalia's fine clothing boutique, we offer a", "unique service where we will totally revamp your outfit", "to your choosing, for... wait for it...").also { stage++ }
            22 -> npc(FaceAnim.AMAZED, "A fee of only 500 gold coins! Tired of always wearing", "the same old outfit, day in, day out? This is the service", "for you!").also { stage++ }
            23 -> npc(FaceAnim.ASKING, "So what do you say? Interested? We can change either", "your top, or your legwear for only 500 gold an item!").also { stage++ }
            ...
            30 -> {
                if (player.equipment.isEmpty) {
                    ...
                    npc(FaceAnim.HAPPY, "Just select what style and colour you would like from", "this catalogue, and then give me the 1000 gold when", "you've picked.").also { stage++ }
                } else {
                    npc(FaceAnim.WORRIED, "You can't try them on while wearing armour. Take", "it off and speak to me again.").also { stage = END_DIALOGUE }
                }
            }
