...
class MurkyMattDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npc(FaceAnim.HALF_GUILTY, "Nay! Now, what is it that ye be wantin?", "I can tell ye all about the prices of runes, I can.").also { stage++ }
            ...
            4 -> when (buttonId) {
                ...
                3 -> player(FaceAnim.HALF_GUILTY, "I got to go, erm, swab some decks! Yarr!").also { stage = END_DIALOGUE }
            }
            ...
            7 -> npc(FaceAnim.HALF_GUILTY, "No? Then what's that smell? The smell o'", "someone spent too long at sea without a bath!").also { stage++ }
            8 -> player(FaceAnim.HALF_GUILTY, "I think that's probably you.").also { stage++ }
            9 -> npc(FaceAnim.HALF_GUILTY, "Har har har!").also { stage++ }
            10 -> npc(FaceAnim.HALF_GUILTY, "We've got a stern landlubber 'ere'! Well, let", "me tell ye, I'm here for the Grand Exchange!").also { stage++ }
            11 -> player(FaceAnim.HALF_GUILTY, "Don't you just want to sell it in a shop or trade", "it to someone specific?").also { stage++ }
            12 -> npc(FaceAnim.HALF_GUILTY, "By my wave-battered bones! Not when I can sell to", "the whole world from this very spot!").also { stage++ }
