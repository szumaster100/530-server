...
class UrtagDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            8 -> npcl(FaceAnim.OLD_NORMAL, "I hear there is quite a craze for surface foods at the moment, so you might be able to sell something there.").also { stage++ }
            9 -> npcl(FaceAnim.OLD_NORMAL, "You might also want to visit the caves to the south of the city.").also { stage++ }
            10 -> npcl(FaceAnim.OLD_NORMAL, "There are many monsters to fight there, and Zanik tells me she has also found ways to cross over their heads!").also { stage = END_DIALOGUE }
        }
        return true
    }
