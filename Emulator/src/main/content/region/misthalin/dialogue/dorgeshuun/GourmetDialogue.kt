...
class GourmetDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            START_DIALOGUE -> npcl(FaceAnim.OLD_NORMAL, "Ooh, a surface-dweller! Have you got any exotic surface foods to sell?").also { stage++ }
            ...
            3 -> npcl(FaceAnim.OLD_NORMAL, "A pity. Dorgeshuun food seems so dull now I've tasted surface food.").also { stage = END_DIALOGUE }
            4 -> npcl(FaceAnim.OLD_NORMAL, "Splendid! Show me what you've got!").also { stage = END_DIALOGUE }
            5 -> npcl(FaceAnim.OLD_NORMAL, "All kinds! I don't want to keep eating the same food, even if I like it. I want to try new tastes!").also { stage++ }
            6 -> npcl(FaceAnim.OLD_NORMAL, "Why not try something on me and see if I like it?").also { stage = END_DIALOGUE }
        }
        return true
    }
