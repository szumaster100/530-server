...
class BarlakDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (playerMeetsBoneReqs && inInventory(player, Items.LONG_BONE_10976, 1)) {
            curItem = 0
            npcl(FaceAnim.OLD_HAPPY, "Those bones! Those are exactly the sort of thing I need! Will you sell them?").also { stage = 0 }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            152 -> options(
                "Then how do you stand up?",
                "What do you need bones for?",
                "Will you buy anything besides bones?",
                "What kind of bones do you need?",
                "Goodbye"
            ).also { stage++ }
            153 -> when (buttonId) {
