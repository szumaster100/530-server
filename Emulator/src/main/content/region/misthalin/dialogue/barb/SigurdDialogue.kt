    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            15 -> when (buttonId) {
                ...
                2 -> npc(FaceAnim.HALF_GUILTY, "Okay, if you change your mind you know where", "to find me.").also { stage = END_DIALOGUE }
            }
            16 -> {
                val level = getStatLevel(player, Skills.WOODCUTTING)
                when {
                    level > 57 -> npcl(FaceAnim.HALF_GUILTY, "You look like you know your way around a tree, you can make a Waka canoe.").also { stage++ }
                    level in 42..57 -> npcl(FaceAnim.HALF_GUILTY, "Well, you're pretty handy with an axe!").also { stage = 21 }
                    level in 27..41 -> npcl(FaceAnim.HALF_GUILTY, "You could make a Dugout canoe with your woodcutting skill, but I don't see why you would want to.").also { stage = END_DIALOGUE }
                    level in 12..26 -> npcl(FaceAnim.HALF_GUILTY, "You can make a log canoe like mine! It'll get you 1 stop down the river.").also { stage = 23 }
                    else -> npcl(FaceAnim.HALF_GUILTY, "Well, you don't look like you have the skill to make a canoe.").also { stage = 25 }
                }
            ...
            21 -> npcl(FaceAnim.HALF_GUILTY, "You could make Stable Dugout canoes, like that snooty fella Tarquin.").also { stage++ }
            22 -> npcl(FaceAnim.HALF_GUILTY, "He reckons his canoes are better than mine. He's never said it to my face though.").also { stage = END_DIALOGUE }
            23 -> npcl(FaceAnim.HALF_GUILTY, "There's some snooty fella down near the Champions Guild who reckons his canoes are better than mine.").also { stage++ }
            ...
            26 -> npcl(FaceAnim.HALF_GUILTY, "Once you are able to make a canoe it makes travel along the river much quicker!").also { stage = END_DIALOGUE }
        }
        return true
    }
