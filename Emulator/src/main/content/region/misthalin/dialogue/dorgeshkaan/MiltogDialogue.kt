...
class MiltogDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            5 -> npcl(FaceAnim.OLD_NORMAL, "You can do that yourself! Just put some swamp tar in the lamp oil still here and then use the lamp on the still.").also { stage = END_DIALOGUE }
            6 -> npcl(FaceAnim.OLD_NORMAL, "They work by our very own Dorgeshuun magic!").also { stage++ }
            7 -> npcl(FaceAnim.OLD_NORMAL, "Sometimes they don't work very well, though. The light orbs blow and they have to be replaced.").also { stage++ }
            8 -> npcl(FaceAnim.OLD_NORMAL, "If you ever see a lamp that's gone out, you can fix it by replacing the light orb. You can make new light orbs out of glass, and use a piece of wire on them to add the filament.").also { stage++ }
