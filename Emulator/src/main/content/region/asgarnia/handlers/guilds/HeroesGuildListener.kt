    ...
    private fun rechargeJewellery(player: Player, n: Node, node: Node) {
        ...
        if (usedWith is Scenery) {
            if (usedItem.name.contains("glory", true)) {
                player.dialogueInterpreter.sendItemMessage(Items.AMULET_OF_GLORY_1704, "You feel a power emanating from the fountain as it", "recharges your amulet. You can now rub the amulet to", "teleport and wear it to get more gems whilst mining.")
            } else {
