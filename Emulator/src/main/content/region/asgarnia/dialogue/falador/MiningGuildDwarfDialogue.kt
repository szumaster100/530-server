        ...
        when (stage) {
            ...
            4 -> {
                if (getStatLevel(player, Skills.MINING) >= 60) {
                    ...
                    npc(FaceAnim.OLD_NORMAL, "Sorry, but the rules are rules. Do some more training", "first.").also { stage = END_DIALOGUE }
                }
            }
            5 -> npc(FaceAnim.OLD_NORMAL, "What do you think? We smelt it into bars, smith the", "metal to make armour and weapons, then we exchange", "them for goods and services.").also { stage++ }
            6 -> playerl(FaceAnim.NEUTRAL, "I don't see many dwarves here.").also { stage++ }
            7 -> npc(FaceAnim.OLD_NORMAL, "No, this is only a mining outpost. We dwarves don't", "much like to settle in human cities. Most of the ore is", "carted off to Keldagrim, the great dwarven city - They've", "got a special blast furnace up there - it makes").also { stage++ }
            8 -> npc(FaceAnim.OLD_NORMAL, "smelting the ore so much easier. There are plenty of", "dwarven traders working in Keldagrim.").also { stage = END_DIALOGUE }
        }
        return true
    }
