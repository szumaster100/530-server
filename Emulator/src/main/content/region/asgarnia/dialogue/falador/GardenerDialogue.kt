        ...
        when (stage) {
            ...
            20 -> when (npc.id) {
                Farmers.ELSTAN.id, Farmers.LYRA.id -> showTopics(
                    Topic(FaceAnim.NEUTRAL, "The north-western allotment.", 21),
                    Topic(FaceAnim.NEUTRAL, "The south-eastern allotment.", 22)
                )

                Farmers.DANTAERA.id, Farmers.KRAGEN.id -> showTopics(
                    Topic(FaceAnim.NEUTRAL, "The northern allotment.", 21),
                    Topic(FaceAnim.NEUTRAL, "The southern allotment.", 22)
                )
            }
            ...
            32 -> showTopics(
                ...
                Topic(FaceAnim.NEUTRAL, "A seed dibber.", 600),
            ...
            1021 -> {
                if (inInventory(player, Items.COINS_995, 200)) {
                    showTopics(
                        Topic(FaceAnim.NEUTRAL, "Here's 200GP - chop my tree down please.", 1022),
                        Topic(FaceAnim.NEUTRAL, "I don't want to pay that much, sorry.", END_DIALOGUE)
                    )
                } else {
                    player("I don't have that much money on me.").also { stage = END_DIALOGUE }
                }
            }
            ...
            2000 -> {
                ...
                npcl(FaceAnim.NEUTRAL, advice.random()).also { stage = START_DIALOGUE }
            }
        }
        return true
    }
