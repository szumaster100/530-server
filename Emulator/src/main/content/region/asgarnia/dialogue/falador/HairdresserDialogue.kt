...
class HairdresserDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            6 -> npc(FaceAnim.HAPPY, "Certainly ${if (player.isMale) "sir" else "madam"}. I cut ${if (player.isMale) "men's" else "women's"} hair at the bargain rate of", "only 2000 gold coins. I'll even throw in a free recolour!").also { stage++ }
            7 ->
                if (player.equipment[EquipmentContainer.SLOT_HAT] == null && player.equipment[EquipmentContainer.SLOT_WEAPON] == null && player.equipment[EquipmentContainer.SLOT_SHIELD] == null) {
                    npc(FaceAnim.NEUTRAL, "Please select the hairstyle and colour you would like", "from this brochure.").also { stage = 9 }
                } else if (player.equipment[EquipmentContainer.SLOT_HAT] != null || player.equipment[EquipmentContainer.SLOT_WEAPON] != null || player.equipment[EquipmentContainer.SLOT_SHIELD] != null) {
                    npc(FaceAnim.SCARED, (if (player.isMale) "Sir, " else "Madam, ") + "I can't cut your hair with those things pointing", "at me. Please take them off and speak to me again.").also { stage = END_DIALOGUE }
                } else {
                    player(FaceAnim.SAD, "I don't have 2000 gold coins on me...").also { stage++ }
                }
            8 -> npc(FaceAnim.ANNOYED, "Well, come back when you do. I'm not running a", "charity here!").also { stage = END_DIALOGUE }
            9 -> {
                end()
                openInterface(player, if (player.isMale) 596 else 592)
            }
