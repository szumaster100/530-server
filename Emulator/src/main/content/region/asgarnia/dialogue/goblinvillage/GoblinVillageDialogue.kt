...
class GoblinVillageDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npc(FaceAnim.OLD_DEFAULT, "I kill you!").also { end(); npc.attack(player) }
            5 -> npc(FaceAnim.OLD_DEFAULT, "Me not sure that allowed. You have to check with", "generals.").also { stage = END_DIALOGUE }
            ...
            9 -> npc(FaceAnim.OLD_DEFAULT, "General Wartface and General Bentnoze both say it is.", "And normally they never agree!").also { stage = 21 }
            10 -> npcl(FaceAnim.OLD_DEFAULT, "Goblin century mark year of big battle on Plain of Mud. That when Big High War God give us commandments.").also { stage++ }
            ...
            13 -> npcl(FaceAnim.OLD_DEFAULT, "Big High War God take goblins and make them strong! Without him, we small and weak and stupid.").also { stage++ }
            14 -> npcl(FaceAnim.OLD_DEFAULT, "But thanks to Big High War God, we most powerful race in Gielinor!").also { stage++ }
            15 -> playerl(FaceAnim.THINKING, "Umm... you're clearly not the most powerful race in Gielinor.").also { stage++ }
            16 -> npcl(FaceAnim.OLD_DEFAULT, "Not to doubt word of Big High War God! I kill you!").also { stage++ }
            17 -> {
                end(); npc.attack(player)
            }
            18 -> npcl(FaceAnim.OLD_DEFAULT, "Slay enemies of Big High War God! Not show mercy! Not run from battle! Not doubt word of Big High War God!").also { stage++ }
            19 -> npc(FaceAnim.OLD_DEFAULT, "Without commandments we not know right from wrong. We be like other races who not know war is good!").also { stage = 17 }
            20 -> npcl(FaceAnim.OLD_DEFAULT, "That goblin secret! No human ever find Plain of Mud!").also { stage = 17 }
            21 ->
                if (npc.id == NPCs.GOBLIN_4488 || npc.id == NPCs.GOBLIN_4489 || npc.id == NPCs.GOBLIN_4491 || npc.id == NPCs.GOBLIN_4492) {
