    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> showTopics(
                ...
                IfTopic("I've lost my oily fishing rod.", 7, isQuestComplete(player, Quests.HEROES_QUEST) && !inInventory(player, Items.OILY_FISHING_ROD_1585) && !inBank(player, Items.OILY_FISHING_ROD_1585)),
            ...
            3 -> npc("Lava eels eh? That's a tricky one that is, you'll need a", "lava-proof fishing line. The method for making this would", "be to take an ordinary fishing rod, and then cover it", "with the fire-proof Blamish Oil.").also { stage++ }

            4 ->
                if (isQuestComplete(player, Quests.HEROES_QUEST)) {
                    npc("Of course, you knew that already.").also { stage = 7 }
                } else {
                    npc("You know... thinking about it... I may have a jar of", "Blamish Slime around here somewhere... Now where did", "I put it?").also { stage++ }
                }

            5 -> sendDialogue("Gerrant search around a bit.").also { stage++ }
            6 -> {
                end()
                npc("Aha! Here it is! Take this slime, mix it with some", "Harralander and water and you'll have the Blamish Oil", "you need for treating your fishing rod.")
