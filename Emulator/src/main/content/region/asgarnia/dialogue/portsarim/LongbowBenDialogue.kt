...
class LongbowBenDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npcl(FaceAnim.THINKING, "Redbeard Frank ye say? He be outside. Says he likes the feel of the wind on his cheeks.").also { stage++ }
            3 -> player("Thanks.").also { stage = END_DIALOGUE }
            4 -> npc("Arrr, that's a strange yarn.").also { stage++ }
            5 -> npc(FaceAnim.HALF_GUILTY, "I was to be marooned, ye see. A scurvy troublemaker had", "taken my ship, and he put me ashore on a little island.").also { stage++ }
            6 -> player(FaceAnim.HALF_GUILTY, "Gosh, how did you escape?").also { stage++ }
            7 -> npc(FaceAnim.HALF_GUILTY, "Arrr, ye see, he made one mistake! Before he sailed", "away, he gave me a bow and one arrow so that I wouldn't have", "to die slowly.").also { stage++ }
            ...
            10 ->
                if (isQuestComplete(player, Quests.PIRATES_TREASURE) && !isQuestComplete(player, Quests.GOBLIN_DIPLOMACY)) {
                    ...
                    npcl(FaceAnim.HALF_GUILTY, "Nay, but the barkeep hears most of the news around here.").also { stage++ }
                }
            11 -> when {
                isQuestComplete(player, Quests.PIRATES_TREASURE) && !isQuestComplete(player, Quests.GOBLIN_DIPLOMACY) -> npcl(FaceAnim.NEUTRAL, "Perhaps ye should be asking him for a quest.").also { stage = 13 }
                else -> npcl(FaceAnim.HALF_GUILTY, "Or Redbeard Frank, he's often spoken of buried treasure. Perhaps ye should be asking them for quests.").also { stage = 13 }
            }
            12 -> npcl(FaceAnim.THINKING, "But I hear there's an old landlubber in Draynor Village who's always a-looking for a lively ${if (player.isMale) "lad" else "lass"} to do him a favour.").also { stage++ }
