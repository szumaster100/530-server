...
class MartinThwaitDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        npc = args[0] as NPC
        npc(FaceAnim.HALF_GUILTY, "You know it's sometimes funny how things work out, I", "lose some gold but find an item, or I lose an item and", "find some gold... no-one ever knows what's gone where", "ya know.")
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> when (buttonId) {
                1 -> player(FaceAnim.HALF_GUILTY, "Yeah I know what you mean, found anything recently?").also { stage = 10 }
            ...
            10 -> {
                ...
                if (getStatLevel(player, Skills.AGILITY) >= 50 && getStatLevel(player, Skills.THIEVING) >= 50 || getStatLevel(player, Skills.THIEVING) == 99) {
                    ...
                    if (getStatLevel(player, Skills.THIEVING) < 50) {
                        npc(FaceAnim.HALF_GUILTY, "Sorry, mate. Train up your Thieving skill to at least", "50 and I might be able to help you out.")
                    }
                    if (getStatLevel(player, Skills.AGILITY) < 50) {
                        npc(FaceAnim.HALF_GUILTY, "Sorry, mate. Train up your Agility skill to at least", "50 and I might be able to help you out.")
                    }
                }
            }
            30 -> npc(FaceAnim.HALF_GUILTY, "Sure, this is a Skillcape of Thieving. It shows my stature as", "a master thief! It has all sorts of uses , if you", "have a level of 99 Thieving I'll sell you one.").also { stage++ }
            31 -> {
                if (getStatLevel(player, Skills.THIEVING) == 99) {
