...
class GamfredDialoue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npc(FaceAnim.CHILD_SAD, "WHAT!? I'll have you know that is the finest piece of", "dwarven engineering for miles around! How DARE you", "insult my work!").also { stage = END_DIALOGUE }
            3 -> end()
            4 -> npc(FaceAnim.CHILD_NORMAL, "Nice to meet someone who appreciates fine work, have", "you tried it out yet?").also { stage++ }
            ...
            11 -> npc(FaceAnim.CHILD_NORMAL, "Glad to hear it. Try it again sometime. We have more", "tests to run.").also { stage = END_DIALOGUE }
            12 -> npc(FaceAnim.CHILD_NORMAL, "Well I guess not everyone will like it. But give it another", "chance before you go.").also { stage = END_DIALOGUE }
            13 -> {
            ...
            14 -> {
                if (hasSpaceFor(player, Item(Items.DEFENSIVE_SHIELD_8856, 1))) {
                    ...
                    npc(FaceAnim.CHILD_NORMAL, "Muffin make some room in your inventory first!").also { stage = END_DIALOGUE }
                }
            }
            15 -> npc(FaceAnim.CHILD_NORMAL, "Are ye blind lad? Tis over there in the next room with me", "assistant working it!").also { stage = END_DIALOGUE }
            16 -> npc(FaceAnim.CHILD_NORMAL, "Come back soon! My catapult needs more test subjects.").also { stage = END_DIALOGUE }
            17 -> npc(FaceAnim.CHILD_NORMAL, "Well ye take the big defence shield in both hands and", "watch the catapult. My assistant will fire different things", "at you and you need to defend against them. To see", "what might be coming your way and wich defensive").also { stage++ }
