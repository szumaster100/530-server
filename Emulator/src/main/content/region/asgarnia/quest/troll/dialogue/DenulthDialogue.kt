...
class DenulthDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (getQuestStage(player!!, Quests.TROLL_STRONGHOLD)) {
            in 1..7 -> {
                when (stage) {
                    START_DIALOGUE -> npcl(FaceAnim.FRIENDLY, "How are you getting on with rescuing Godric?").also { stage++ }
                    1 -> {
                    ...
                    2 -> npcl(FaceAnim.FRIENDLY, "Then hurry, friend! What are you still doing here?").also {
                        stage = END_DIALOGUE
                    }

                    3 -> npcl(FaceAnim.FRIENDLY, "Hurry, friend! Who knows what they'll do with Godric?").also {
                        stage = END_DIALOGUE
                    }
                }
            }
            ...
            in 8..10 -> {
                when (stage) {
                    ...
                    5 -> npcl(FaceAnim.FRIENDLY, "Hurry, friend. Find a way to free Godric!").also {
                        stage = END_DIALOGUE
                    }
                }
            }
            ...
            11 -> {
                when (stage) {
                    ...
                    3 -> npcl(FaceAnim.FRIENDLY, "Oh, what great news! You should hurry to tell Dunstan, he will be overjoyed!").also { stage = END_DIALOGUE }
                }
            }
        }
    }
