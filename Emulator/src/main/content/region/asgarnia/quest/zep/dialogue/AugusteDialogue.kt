    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        ...
        when (getQuestStage(player, Quests.ENLIGHTENED_JOURNEY)) {
            0 -> when (stage) {
                ...
                13 -> when (buttonID) {
                    1 ->
                        if (hasLevelStat(player, Skills.CRAFTING, 36) && hasLevelStat(player, Skills.FARMING, 30) && hasLevelStat(player, Skills.FIREMAKING, 20) && getQuestPoints(player) >= 21) {
            ...
            1 -> when (stage) {
                ...
                3 -> when (buttonID) {
                    ...
                    2 -> playerl(FaceAnim.FRIENDLY, "Err, no. I can't see that kind of detail.").also {
                        stage = 4
                    }
                }
                ...
                5 -> npcl(FaceAnim.FRIENDLY, "Don't you see? It's the hot air! It rises, taking the ashes with it.").also { stage++ }
                6 -> npcl(FaceAnim.FRIENDLY, "I had this epiphany while I was at the glass blower's house. If we pump hot air into an envelope it will rise because it is lighter than the cold air around it.").also { stage++ }
                7 -> playerl(FaceAnim.FRIENDLY, "Come again?").also { stage++ }
                8 -> npcl(FaceAnim.FRIENDLY, "We are going to sew a big sack and light a fire under it. Once the sack fills with hot air it will begin to rise, taking us along with it in the basket.").also { stage++ }
                9 -> playerl(FaceAnim.FRIENDLY, "You seem pretty confident about this. Have you tested it?").also { stage++ }
                10 -> npcl(FaceAnim.FRIENDLY, "Well, no. You see, I don't have the materials here to make any test balloons.").also { stage++ }
                11 -> npcl(FaceAnim.FRIENDLY, "That's where you come in!").also { stage++ }
                12 -> npcl(FaceAnim.FRIENDLY, "You are going to collect materials for two test runs of the balloon.").also { stage++ }
                13 -> npcl(FaceAnim.FRIENDLY, "You will need to get three sheets of papyrus, one ball of wool, one full sack of potatoes, and one unlit candle.").also { stage++ }
                14 -> playerl(FaceAnim.FRIENDLY, "What am I supposed to do with all that junk?").also { stage++ }
                15 -> npcl(FaceAnim.FRIENDLY, "When you have all of it, bring it to me and I'll explain what to do next. Any questions?").also { stage++ }
                16 -> playerl(FaceAnim.FRIENDLY, "Where do I get all this stuff?").also { stage++ }
                17 -> npcl(FaceAnim.FRIENDLY, "You're the adventurer, you should know! Think of logical places, like, churches; they have tons of candles.").also { stage++ }
                18 -> {
                    end()
            ...
            2 -> when (stage) {
                ...
                4 -> npcl(FaceAnim.FRIENDLY, "Good, you have everything! Now, I need you to create an origami balloon.").also { stage = 5 }
                5 -> playerl(FaceAnim.FRIENDLY, "How do you make the origami balloon?").also { stage++ }
                6 -> npcl(FaceAnim.FRIENDLY, "First, use the papyrus on the ball of wool. The papyrus is folded into an origami box and the yarn will support the heat source.").also { stage++ }
                7 -> npcl(FaceAnim.FRIENDLY, "Next, add the unlit candle to the balloon structure. It will act as the heat source.").also { stage++ }
                8 -> npcl(FaceAnim.FRIENDLY, "Once you have done that let me know and we will begin our experiment.").also { stage++ }
                9 -> {
                    end()
                ...
                10 -> {
                    if (!hasCandle && !hasPapyrus && !hasWool && !hasPotatoes) {
                        npcl(FaceAnim.FRIENDLY, "You don't seem to have any of the materials. You need a ball of wool, three sheets of papyrus, an unlit candle, and a full sack of potatoes.").also { stage = END_DIALOGUE }
                    } else if (hasCandle && !hasWool && !hasPapyrus && !hasPotatoes) {
                        npcl(FaceAnim.FRIENDLY, "You need a ball of wool, papyrus, and a full sack of potatoes.").also { stage = END_DIALOGUE }
                    } else if (hasWool && !hasPapyrus && !hasCandle && !hasPotatoes) {
                        npcl(FaceAnim.FRIENDLY, "You need more papyrus, a full sack of potatoes, and an unlit candle.").also { stage = END_DIALOGUE }
                    } else if (hasPapyrus && !hasCandle && !hasWool && !hasPotatoes) {
                        npcl(FaceAnim.FRIENDLY, "You need a ball of wool, an unlit candle, and a full sack of potatoes.").also { stage = END_DIALOGUE }
                    } else if (hasPotatoes && !hasCandle && !hasWool && !hasPapyrus) {
                        npcl(FaceAnim.FRIENDLY, "You need a ball of wool, an unlit candle, and papyrus.").also { stage = END_DIALOGUE }
                    } else if (hasCandle && hasWool && !hasPapyrus && !hasPotatoes) {
                        npcl(FaceAnim.FRIENDLY, "You need more papyrus and a full sack of potatoes.").also { stage = END_DIALOGUE }
                    } else if (hasCandle && hasPapyrus && !hasPotatoes && !hasWool) {
                        npcl(FaceAnim.FRIENDLY, "You need more papyrus, and a ball of wool.").also { stage = END_DIALOGUE }
                    } else if (hasWool && hasPotatoes && !hasCandle && !hasPapyrus) {
                        npcl(FaceAnim.FRIENDLY, "You need papyrus and an unlit candle.").also { stage = END_DIALOGUE }
                    } else if (hasWool && hasPapyrus && !hasPotatoes && !hasCandle) {
                        npcl(FaceAnim.FRIENDLY, "You need an unlit candle and a full sack of potatoes.").also { stage = END_DIALOGUE }
                    } else if (hasPotatoes && hasPapyrus && !hasWool && !hasCandle) {
                        npcl(FaceAnim.FRIENDLY, "You need a ball of wool and an unlit candle.").also { stage = END_DIALOGUE }
                    } else if (!hasCandle) {
            ...
            4 -> when (stage) {
                ...
                3 ->
                    if (!hasPapyrus && !hasPotatoes) {
                        ...
                        npcl(FaceAnim.FRIENDLY, "Commendable. If I may have those, I will construct this experiment.").also { stage = 5 }
                    }
            ...
            5 -> when (stage) {
                ...
                1 -> npcl(FaceAnim.FRIENDLY, "Ahh, the flash mob phenomenon. Many have hypothesized that they are beings of great power sent to smite those who question the gods.").also { stage++ }
                ...
                5 -> playerl(FaceAnim.FRIENDLY, "Was I the ONLY one who saw them burning? BURNING???").also { stage++ }
                6 -> npcl(FaceAnim.FRIENDLY, "Yes, very well indeed. Now we will start building the balloon that will carry us off the island.").also { stage++ }
                7 -> npcl(FaceAnim.FRIENDLY, "This task will be much greater than the last two. I hope you are prepared for it.").also { stage++ }
                ...
                12 -> npcl(FaceAnim.FRIENDLY, "Sandbags can be made by getting empty sacks and filling them at the sand pit here on Entrana.").also { stage++ }
                13 -> npcl(FaceAnim.FRIENDLY, "However, there are other sand pits around the world that will work as well; there is one in Yanille, Rellekka, and Zanaris.").also { stage++ }
                14 -> npcl(FaceAnim.FRIENDLY, "You can bring items back to me as you get them, while you are waiting for the tree to grow.").also { stage++ }
                15 -> playerl(FaceAnim.FRIENDLY, "What tree?").also { stage++ }
                16 -> npcl(FaceAnim.FRIENDLY, "I am going to give you a willow sapling and a basket of apples. You must plant the willow sapling at a tree patch.").also { stage++ }
                17 -> npcl(FaceAnim.FRIENDLY, "If you give the basket of apples to the gardener near the patch, he will look after the tree for you while it grows.").also { stage++ }
                18 -> npcl(FaceAnim.FRIENDLY, "Don't lose the sapling! It took me a long time to save up enough for one. If you do, you'll have to pay me 30,000 coins for a new one.").also { stage++ }
                19 -> npcl(FaceAnim.FRIENDLY, "Once the tree is fully grown, cut twelve branches from it using secateurs. Bring the branches back here and use them on the metal frame on the platform to create the basket.").also { stage++ }
            ...
            6 -> when (stage) {
                ...
                2 -> when (buttonID) {
                    ...
                    2 -> playerl(FaceAnim.FRIENDLY, "I'm having trouble finding some items.").also { stage = 12 }
                ...
                8 -> npcl(FaceAnim.FRIENDLY, "Sandbags can be made by getting empty sacks and filling them at the sandpit here on Entrana.").also { stage++ }
                ...
                11 ->
                    if (hasBowl && getAttribute(player, "enlighted-journey-items", 0) == 3) {
                        ...
                        npcl(FaceAnim.FRIENDLY, "I need a plain clay-fired bowl; they're quite ease to come by.").also { stage = 3 }
                    }
                ...
                15 -> npcl(FaceAnim.FRIENDLY, "I was told a while ago that there was a witch who made dye in Draynor Village. Maybe you should start by looking there.").also { stage = END_DIALOGUE }
                16 -> npcl(FaceAnim.FRIENDLY, "Sandbags can be made by getting empty sacks and filling them at the sandpit here on Entrana.").also { stage++ }
                17 -> npcl(FaceAnim.FRIENDLY, "However, there are other sand pits around the world that will work as well; there is one in Yanille, Rellekka, and Zanaris.").also { stage = END_DIALOGUE }
                18 -> npcl(FaceAnim.FRIENDLY, "Hmm, I believe silk is imported from the desert. Perhaps someone there can tell you where to find it.").also { stage = END_DIALOGUE }
                19 -> npcl(FaceAnim.FRIENDLY, "I think there is a spare one in the glass blower's house. I rent a room from him there, so I don't think he'll mind you taking it.").also { stage = END_DIALOGUE }
                20 -> npcl(FaceAnim.FRIENDLY, "It will cost you 30,000 gold coins to replace it, do you want to pay that?").also { stage++ }
            ...
            7 -> when (stage) {
                ...
                7 -> {
                    ...
                    addItemOrDrop(player, Items.AUGUSTES_SAPLING_9932)
                    npcl(FaceAnim.FRIENDLY, "Use the willow sapling I gave you to grow a willow tree. Cut twelve branches from it using secateurs. Use the branches on the platform here.")
                    stage = END_DIALOGUE
                }
            }
            ...
            100 -> when (stage) {
                ...
                4 -> {
                    if (inBank(player, Items.BOMBER_CAP_9945) || inInventory(player, Items.BOMBER_CAP_9945) || inEquipment(player, Items.BOMBER_CAP_9945)) {
                ...
                5 -> {
                    if (inBank(player, Items.BOMBER_JACKET_9944) || inInventory(player, Items.BOMBER_JACKET_9944) || inEquipment(player, Items.BOMBER_JACKET_9944)) {
...
class AugusteDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            1 -> npcl(FaceAnim.FRIENDLY, "I am considering starting a balloon enterprise. People all over Runescape will be able to travel in a new, exciting way.").also { stage++ }
