...
class WitsTest(private val dialogueNum: Int = 0) : DialogueBuilderFile() {
            ...
            branch.onValue(0).betweenStage { _, player, _, _ ->
                ...
                .branch { player -> getAttribute(player, ATTRIBUTE_RANDOM_RIDDLE, 0) }.let { branch ->
                    ...
                    branch.onValue(2)
                        ...
                        .npc(FaceAnim.THINKING, "How old is my daughter now?").manualStage { _, player, _, _ ->
                            sendInputDialogue(player, true, "Enter the amount:") { value: Any ->
                                if (value == 10) {
                                    setAttribute(player, ATTRIBUTE_CORRECT_ANSWER, true)
                                } else {
                    ...
                    branch.onValue(4)
                        ...
                        .npc(FaceAnim.THINKING, "Which bucket's stone dropped to the bottom of the bucket", "last?")
                        .options().let { optionBuilder ->
