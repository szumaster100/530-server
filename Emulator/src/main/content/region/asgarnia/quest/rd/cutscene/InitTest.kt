...
import content.region.asgarnia.quest.rd.RecruitmentDriveListener
import content.region.asgarnia.quest.rd.RecruitmentDriveListener.Companion.initRoomStage
import core.api.*
...
class SirTiffyCashienDialogueFile : DialogueBuilderFile() {
    ...
    override fun create(b: DialogueBuilder) {
        b.onQuestStages(Quests.RECRUITMENT_DRIVE, 1)
            ...
            .npcl(FaceAnim.FRIENDLY, "Ah, @name! Amik told me all about you, dontchaknow! Spliffing job you you did with the old Black Knights there, absolutely first class.")
            .playerl(FaceAnim.GUILTY, "...Thanks I think.")
            .npcl(FaceAnim.FRIENDLY, "Well, not in those exact words, but you get my point, what?")
            .npcl(FaceAnim.FRIENDLY, "A top-notch filly like yourself is just the right sort we've been looking for for our organisation.")
                ...
                return@let originalPath.builder()
                    ...
                    .let { optionBuilder ->
                        ...
                        optionBuilder.option("Yes, let's go!")
                            ...
                            .branch { player ->
                                if (player.inventory.isEmpty && player.equipment.isEmpty && !(player.familiarManager.hasFamiliar() || player.familiarManager.hasPet())) {
                                    1
                                } else {
                                    0
                                }
...
class InitTest(player: Player) : Cutscene(player) {

    override fun setup() {
        val currentStage = getAttribute(player, RecruitmentDrive.stageArray[0], 0)
        setExit(RecruitmentDriveListener.Companion.Rooms.index[currentStage]!!.location)
    }
