...
class TenzingDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (getQuestStage(player!!, Quests.DEATH_PLATEAU)) {
            ...
            in 21..23 -> {
                when (stage) {
                    START_DIALOGUE -> playerl(FaceAnim.FRIENDLY, "Hello!").also {
            ...
            24 -> {
                when (stage) {
                    START_DIALOGUE -> playerl(FaceAnim.FRIENDLY, "Hello!").also { stage++ }
                    1 -> npcl(FaceAnim.FRIENDLY, "Have you brought me the items I asked for?").also {
                        if (inInventory(player!!, Items.SPIKED_BOOTS_3107) && inInventory(
                                player!!,
                                Items.BREAD_2309,
                                10
                    ...
                    4 -> {
                        if (!inInventory(player!!, Items.BREAD_2309, 10) && !inInventory(
                                player!!,
                                Items.TROUT_333,
                                10
                            )
                    ...
                    11 -> npcl(FaceAnim.FRIENDLY, "You are wise for one so young.").also {
            ...
            in 25..29 -> {
                when (stage) {
                    ...
                    1 -> npcl(FaceAnim.FRIENDLY, "Hello again traveller. What can I do for you?").also {
