...
class AliTheHagDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (isQuestComplete(player, Quests.SWEPT_AWAY) && getDynLevel(player, Skills.MAGIC) > 33 && inInventory(player, Items.BROOMSTICK_14057) && getAttribute(player, GameAttributes.QUEST_SWEPT_AWAY_HETTY_ENCH, false)) {
            ...
            stage = 10
        } else if(isQuestComplete(player, Quests.THE_FEUD)) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            10 -> when(buttonId) {
                1 -> if(isQuestComplete(player, Quests.THE_FEUD)) {
                    player("Good day, Hag.").also { stage = 20 }
                } else {
            ...
            21 -> npcl(FaceAnim.HALF_GUILTY,"Now what do you want from the old hag of Pollnivneach? Hexes? Curses? Eternal youth? Poisons? You name it, I can brew it.").also { stage++ }
            22 -> playerl(FaceAnim.HALF_GUILTY,"Actually I don't want any of that stuff, I'm new in town and I wanted to get to know the locals.").also { stage++ }
            23 -> npcl(FaceAnim.HALF_GUILTY,"I'm busy brewing potions, so if you disturb me again without reason, I will turn you into a frog!").also { stage++ }
            24 -> playerl(FaceAnim.HALF_GUILTY, "Sorry! I won't let it happen again.").also { stage = END_DIALOGUE }
            25 -> npc("Ah, this is Maggie's broom, is it? Yes, I can enchant it", "for you. Just one moment").also { stage++ }
            26 -> {
                end()
                lock(player, 1)
