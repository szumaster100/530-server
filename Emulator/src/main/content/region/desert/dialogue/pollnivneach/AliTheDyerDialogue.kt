...
class AliTheDyerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            20 -> npcl(FaceAnim.HALF_GUILTY, "No silly, it's someone who deals with preservatives and dyes. I didn't really enjoy the preservatives side of the things so I've chosen to deal solely in dyeing.").also { stage++ }
            21 -> playerl(FaceAnim.HALF_GUILTY, "So how's business then?").also { stage++ }
            22 -> npcl(FaceAnim.HALF_GUILTY, "It's a dyeing trade! Oh gosh I'm sorry that was a truly dreadful joke.").also { stage++ }
            23 -> playerl(FaceAnim.HALF_GUILTY, "Hmm yes, about the worst I've heard today at least.").also { stage++ }
            24 -> npcl(FaceAnim.HALF_GUILTY, "Thanks. But really, it's not the best around here. nearly everyone in this town wears only white, which means I don't exactly do a roaring trade. Now is there anything I can get you?").also { stage++ }
            25 -> end()
        }
        return true
    }
