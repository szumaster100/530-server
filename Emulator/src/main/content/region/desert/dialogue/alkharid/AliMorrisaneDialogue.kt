...
class AliMorrisaneDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npcl(FaceAnim.ASKING, "My name is Ali Morrisane - the greatest salesman in the world.").also { stage++ }
            2 -> options(
                "If you are, then why are you still selling goods from a stall?",
                "So what are you selling then?"
            ).also { stage++ }
            3 -> when (buttonId) {
                1 -> playerl(FaceAnim.ASKING, "If you are, then why are you still selling goods from a stall?").also { stage = 10 }
                2 -> {
                    end()
            ...
            10 -> npcl(FaceAnim.FRIENDLY, "Well one can only do and sell so much. If I had more staff I'd be able to sell more").also { stage++ }
            11 -> npcl(FaceAnim.FRIENDLY, "rather than wasting my time on menial things I could get on with selling sand to the Bedabin and useless tourist trinkets to everyone.").also { stage++ }
            12 -> options(
            ...
            13 -> when (buttonId) {
                1 -> playerl(FaceAnim.FRIENDLY, "I'm far too busy - adventuring is a full time job you know.").also { stage = 16 }
                2 -> playerl(FaceAnim.FRIENDLY, "I'd like to help you but.....").also { stage++ }
            }

            14 -> npcl(FaceAnim.FRIENDLY, "Yes I know, I know - the life of a shop keeper isn't slaying dragon and wooing damsels but it has its charms").also { stage = 15 }
            15 -> end()
