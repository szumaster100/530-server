...
class ClayGolemDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg objects: Any?): Boolean {
        ...
        if (isQuestComplete(player, Quests.THE_GOLEM)) {
            npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "Thank you for helping me. A golem can have no greater satisfaction than knowing that its task is complete.")
            return true
        }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> playerl(FaceAnim.HALF_ASKING, "But the whole city is destroyed! Doesn't that bother you?").also { stage++ }
            1 -> npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "I was never programmed to appreciate the city.").also { stage++ }
            2 -> npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "My only purpose was the destruction of the demon, and that is achieved!").also { stage = END_DIALOGUE }
        }
        return false
    }
