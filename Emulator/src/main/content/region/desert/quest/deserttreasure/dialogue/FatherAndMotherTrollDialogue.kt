...
class FatherTrollDialogue(player: Player? = null) : Dialogue(player) {
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        } else if ((getQuestStage(player, DesertTreasure.questName) == 9 && DesertTreasure.getSubStage(player, DesertTreasure.attributeIceStage) >= 5) || getQuestStage(player, DesertTreasure.questName) >= 10) {
            openDialogue(
                player,
                object : DialogueFile() {
                    override fun handle(componentID: Int, buttonID: Int) {
