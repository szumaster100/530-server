        ...
        on(intArrayOf(Scenery.GATE_6451, Scenery.GATE_6452), SCENERY, "open") { player, node ->
            if (getQuestStage(player, Quests.DESERT_TREASURE) == 9) {
                ...
                if (getAttribute(player, DesertTreasure.attributeUnlockedGate, false)) {
                    if (DTUtils.getSubStage(player, DesertTreasure.smokeStage) == 1 && getAttribute<NPC?>(
                            player,
    ...
    override fun run(entity: Entity): Boolean {
        if (entity is Player) {
            when (timerIdentifier) {
                SmokeDiamond.timerIdentifierTorchNE -> sendMessage(
                    entity,
                    "The North-east torch burns out...",
                )

                SmokeDiamond.timerIdentifierTorchSE -> sendMessage(
                    entity,
                    "The South-east torch burns out...",
                )

                SmokeDiamond.timerIdentifierTorchSW -> sendMessage(
                    entity,
                    "The South-west torch burns out...",
                )

                SmokeDiamond.timerIdentifierTorchNW -> sendMessage(
                    entity,
