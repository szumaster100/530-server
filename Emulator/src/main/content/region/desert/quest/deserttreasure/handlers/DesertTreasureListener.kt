...
class DesertTreasureListener : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(Scenery.MYSTICAL_MIRROR_6425, SCENERY, "look-into") { player, _ ->
            SouthWestMirrorLookCutscene(player).start()
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6427, SCENERY, "look-into") { player, _ ->
            NorthWestMirrorLookCutscene(player).start()
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6429, SCENERY, "look-into") { player, _ ->
            NorthMirrorLookCutscene(player).start()
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6431, SCENERY, "look-into") { player, _ ->
            NorthEastMirrorLookCutscene(player).start()
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6433, SCENERY, "look-into") { player, _ ->
        ...
        onUseWith(IntType.SCENERY, Items.BLOOD_DIAMOND_4670, Scenery.OBELISK_6483) { player, used, _ ->
        ...
        onUseWith(IntType.SCENERY, Items.SMOKE_DIAMOND_4672, Scenery.OBELISK_6486) { player, used, _ ->
            if (getDynLevel(player, Skills.MAGIC) > 50) {
                if (removeItem(player, used)) {
                    sendMessage(player, "The diamond is absorbed into the pillar.")
        ...
        onUseWith(IntType.SCENERY, Items.ICE_DIAMOND_4671, Scenery.OBELISK_6489) { player, used, _ ->
            if (getDynLevel(player, Skills.MAGIC) > 50) {
                if (removeItem(player, used)) {
                    sendMessage(player, "The diamond is absorbed into the pillar.")
        ...
        onUseWith(IntType.SCENERY, Items.SHADOW_DIAMOND_4673, Scenery.OBELISK_6492) { player, used, _ ->
            if (getDynLevel(player, Skills.MAGIC) > 50) {
                if (removeItem(player, used)) {
                    sendMessage(player, "The diamond is absorbed into the pillar.")
