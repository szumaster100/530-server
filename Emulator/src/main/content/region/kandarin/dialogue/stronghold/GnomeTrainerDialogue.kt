...
class GnomeTrainerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FaceAnim.OLD_NORMAL, "This isn't a grannies' tea party, let's see some sweat", "human. Go! Go! Go!").also { stage = END_DIALOGUE }
            ...
            5 -> npc(FaceAnim.OLD_NORMAL, "If you complete the course in order from the slippery", "log to the end, your agility will increase much faster", "than by repeating just one obstacle.").also { stage = END_DIALOGUE }

            7 -> npc(FaceAnim.OLD_NORMAL, "I'm amazed by how much humans chat. The sign over", "there says training area, not pointless conversation area.").also { stage = END_DIALOGUE }

            10 -> npc(FaceAnim.OLD_NORMAL, "This is training soldier. If you want fun go make some", "cocktails.").also { stage = END_DIALOGUE }
        }
        return true
    }
