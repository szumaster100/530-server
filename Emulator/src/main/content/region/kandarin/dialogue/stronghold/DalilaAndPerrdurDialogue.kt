...
class DalilaAndPerrdurDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> sendNPCDialogue(player, NPCs.PERRDUR_4587, "I'm sure it will be, dear.", FaceAnim.OLD_NORMAL).also { stage++ }
            3 -> playerl(FaceAnim.HALF_ASKING, "Sorry to disturb you, but what are you two doing here?").also { stage++ }
            4 -> npcl(FaceAnim.OLD_NORMAL, "Perrdur and I are having a nice romantic dinner, aren't we dear?").also { stage++ }
            5 -> sendNPCDialogue(player, NPCs.PERRDUR_4587, "Yes dear.", FaceAnim.OLD_NORMAL).also { stage++ }
            6 -> npcl(FaceAnim.OLD_NORMAL, "We are celebrating the anniversary of our marriage, aren't we dear?").also { stage++ }
            ...
            10 -> playerl(FaceAnim.FRIENDLY, "Clearly. I'll let you two lovebirds carry on, then.").also { stage = END_DIALOGUE }
        }
        return true
    }
