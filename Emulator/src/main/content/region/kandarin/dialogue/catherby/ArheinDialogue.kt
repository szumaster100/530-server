...
class ArheinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (args.size > 1) {
            ...
            npc(FaceAnim.HAPPY, "Hello! Would you like to trade? I've a variety of wares!", "for sale, as well as a special supply of pineapples and", "seaweed!")
            stage = 1
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            100 -> npcl(FaceAnim.NEUTRAL, "Yes, I use it to make deliveries to my customers up and down the coast. These crates here are all ready for my next trip.").also { stage++ }
            101 -> showTopics(
                Topic("Where do you deliver to?", 120),
                Topic("Are you rich then?", 110),
                IfTopic("Do you deliver to the fort just down the coast?", 500, getQuestStage(player!!, Quests.MERLINS_CRYSTAL) == 30)
            )
            110 -> npcl(FaceAnim.NEUTRAL, "Business is going reasonably well... I wouldn't say I was the richest of merchants ever, but I'm doing fairly well all things considered.").also { stage = END_DIALOGUE }
            120 -> npcl(FaceAnim.NEUTRAL, "Various places up and down the coast. Mostly Karamja and Port Sarim.").also { stage++ }
            121 -> showTopics(
            ...
            141 -> npcl(FaceAnim.NEUTRAL, "I'm waiting on a big delivery of candles which I need to deliver further along the coast.").also { stage = END_DIALOGUE }
            500 -> npcl(FaceAnim.HAPPY, "Yes, I do have orders to deliver there from time to time. I think I may have some bits and pieces for them when I leave here next actually.").also { stage++ }
            501 -> {
                showTopics(
            ...
            502 -> npcl(FaceAnim.FRIENDLY, "I don't think Sir Mordred would like that. He wants as few outsiders visiting as possible. I wouldn't want to lose his business.").also { stage = END_DIALOGUE }
            505 -> npcl(FaceAnim.FRIENDLY, "Hey, you gotta take business where you can find it these days! Besides, if I didn't supply them, someone else would.").also { stage = END_DIALOGUE }
            701 -> playerl(FaceAnim.GUILTY, "Yeah... uh... sorry...").also { stage = END_DIALOGUE }
            800 -> {
                selectGoods(Items.PINEAPPLE_2114)
                if (stock == 0) {
                    npcl(FaceAnim.SAD, "Actually, I've run out. Come back tomorrow and I should have some more.").also { stage = END_DIALOGUE }
                } else {
                    this.goodsMessage =
            ...
            1200 -> {
                ...
                sendInputDialogue(player, InputType.AMOUNT, goodsPrompt) { value ->
                    ...
                    var exitMsg = ""
                    exitMsg = if (amountReceived == this.stock) {
