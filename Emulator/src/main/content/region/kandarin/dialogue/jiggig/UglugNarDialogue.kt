...
class UglugNarDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npc(FaceAnim.OLD_DEFAULT, "If yous finds somefin for da sickies, yous brings to", "me...and I's gives you bright pretties, den me", "make more for alls pepels.").also { stage++ }
            4 -> player("Hmm, ok, I'll try to bear that in mind.").also { stage = END_DIALOGUE }
            5 -> npc(FaceAnim.OLD_DEFAULT, "Me's not got no glug-glugs to sell, yous bring me da sickies", "glug-glug den me's open da stufsies for ya.").also { stage = END_DIALOGUE }
        }
        return true
    }
