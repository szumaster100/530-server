...
class MatthiasFalconryDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when {
            ...
            quickOption -> {
                npcl(FaceAnim.FRIENDLY, "If you wish to try falconry, I'd have to request a small fee, mind you; how does 500 gold pieces sound?")
                stage = 5
            }
            else -> {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npc(FaceAnim.HAPPY, "I have some tamer birds that I occasionally lend to rich", "noblemen who consider it a sufficiently refined sport for", "their tastes, and you look like the kind who might", "appreciate a good hunt.").also { stage++ }

            4 -> npc(FaceAnim.HAPPY, "I'd have to request a small fee, mind you; how does", "500 gold pieces sound?").also { stage++ }
            ...
            7 -> npcl(FaceAnim.FRIENDLY, "Well, you're welcome to come back if you change your mind.").also {
                stage = END_DIALOGUE
            }

            8 -> {
                if (getStatLevel(player, Skills.HUNTER) < 43) {
                    npc("Try coming back when you're more experienced", "I wouldn't want my birds being injured.").also { stage = END_DIALOGUE }
                    return true
                }
                ...
                if (!hasHandsFree(player)) {
                    npcl(FaceAnim.HALF_GUILTY, "Sorry, you really need both hands free for falconry. I'd suggest that you put away your weapons and gloves before we start.").also { stage = END_DIALOGUE }
                    return true
                }
            ...
            10 -> npcl(FaceAnim.FRIENDLY, "A good question; straight and to the point. My name is, Matthias, I am a falconer, and this is where I train my, birds.").also { stage++ }

            11 -> options("Do you have any quests I could do?", "That sounds like fun; could I have a go?", "That doesn't sound like my sort of thing.", "What's this falconry thing all about then?").also { stage++ }
            ...
            13 -> npc(FaceAnim.THINKING, "A quest? What a strange notion.", "Do you normally go around asking", "complete strangers for quests?").also { stage++ }
            ...
            16 -> npc(FaceAnim.FRIENDLY, "Fair enough; it does require a great deal of patience", "and skill, so I can understand if you might feel", "intimidated.").also { stage = END_DIALOGUE }

            17 -> npcl(FaceAnim.FRIENDLY, "Well, some people see it as a sport, although such a term does not really convey the amount of patience and dedication required to be proficient at the task.").also { stage++ }

            18 -> npcl(FaceAnim.FRIENDLY, "Putting it simply, it is the training and use of birds of prey in hunting quarry.").also { stage++ }

            19 -> player(FaceAnim.THINKING, "So it's like keeping a pet then?").also { stage++ }
            20 -> npcl(FaceAnim.FRIENDLY, "Not exactly, no. Such a bird can never really be considered tame in the same way that a dog can. They can be trained to associate people or places with food though, and, as such, a good falconer can get a trained bird to do as he wishes.").also { stage = 1 }

            21 -> npcl(FaceAnim.NEUTRAL, "Don't worry; I'll keep an eye on you to make sure you don't upset it too much.").also { stage = END_DIALOGUE }
            ...
            24 -> {
                ...
                player.equipment.add(Item(Items.FALCONERS_GLOVE_10024), true, false)
                sendDialogue("The falconer gives you a large leather glove and brings one of the", "smaller birds over to land on it.")
                stage = 21
            }
            ...
            27 -> npcl(FaceAnim.STRUGGLE, "Sorry, but I was talking to the falcon, not you. But yes it is. Have you had enough yet?").also { stage++ }

            28 -> options("Actually, I'd like to keep trying a little longer.", "I think I'll leave it for now.").also { stage++ }
            ...
            30 -> npcl(FaceAnim.NOD_YES, "Ok then, just come talk to me when you're done.").also {
                stage = END_DIALOGUE
            }
    ...
    fun hasFalcon(player: Player): Boolean {
        ...
        return player.bank.containsItem(falconItem) || player.equipment.containsItem(falconItem) || player.inventory.containsItem(
            falconItem
        )
    }
