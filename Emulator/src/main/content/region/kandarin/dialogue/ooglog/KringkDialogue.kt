...
class KringkDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            1 -> npcl(FaceAnim.CHILD_NORMAL, "Me very busy. No have time to talk to puny creature like you.").also { stage++ }
            ...
            7 -> {
                if (freeSlots(player) == 0) {
                    npcl(FaceAnim.CHILD_NORMAL, "You no have enough room to hold it, human. Come back when you have space.").also { stage = 4 }
                    return true
                }
                if (!removeItem(player, Item(Items.COINS_995, 50))) {
                    ...
                    npcl(FaceAnim.CHILD_NORMAL, "There you go. Nice wig for you, made from de freshest wolfsie bones.").also { stage = 8 }
                }
            }
            8 -> {
                end()
