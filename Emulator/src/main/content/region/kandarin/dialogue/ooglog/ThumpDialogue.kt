    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            START_DIALOGUE -> if (!hasRequirement(player, Quests.AS_A_FIRST_RESORT)) {
                ...
                npcl(FaceAnim.CHILD_NORMAL, "C'mere, human. Me need practice at dis massage thing.").also { stage++ }
            }
            1 -> npcl(FaceAnim.CHILD_NORMAL, "Me not sure how to do it without breaking spine of small, puny creatures.").also { stage++ }
            2 -> playerl(FaceAnim.FRIENDLY, "I...think I'll take a rain check on that.").also { stage = END_DIALOGUE }
            3 -> sendNPCDialogue(player, NPCs.GNOME_66, "Send...help...!", FaceAnim.OLD_ALMOST_CRYING).also { stage = END_DIALOGUE }
        }
        return true
    }
