...
class DawgDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> sendNPCDialogue(player, NPCs.CHIEF_TESS_7051, "Watch out, human; Dawg like human for breakfast.", FaceAnim.CHILD_NORMAL).also { stage++ }
            2 -> end()
            3 -> npc(FaceAnim.CHILD_NEUTRAL, "Grrrr!").also { stage++ }
            4 -> sendNPCDialogue(player, NPCs.CHIEF_TESS_7051, "Huh, huh! Me think Dawg like you now.", FaceAnim.CHILD_NORMAL).also { stage++ }
