...
class CivilianDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npcl(FaceAnim.FRIENDLY, "Yes, but what can you do? It's not like there's many cats around here!").also { stage++ }
            5 -> {
                when {
            ...
            8 -> npcl(FaceAnim.LAUGH, "Hah! that little thing won't catch any mice. I need a fully grown cat.").also { stage = END_DIALOGUE }
            9 -> npcl(FaceAnim.FRIENDLY, "You don't say. Is that it?").also { stage++ }
            10 -> player("Say hello to a real mouse-killer!").also { stage++ }
            11 -> npcl(FaceAnim.FRIENDLY, "Hmmm, not bad, not bad at all. Looks like it's a lively one.").also { stage++ }
            12 -> playerl(FaceAnim.THINKING, "Erm...kind of...").also { stage++ }
            13 -> npcl(FaceAnim.FRIENDLY, "I don't have much in the way of money, but I do have these!").also { stage++ }
            14 -> {
                end()
            ...
            18 -> playerl(FaceAnim.FRIENDLY, "No, you're right, you don't see many around.").also { stage = END_DIALOGUE }
            19 -> npcl(FaceAnim.FRIENDLY, "If you Mourners really wanna help, why don't you do something about these mice?!").also { stage = END_DIALOGUE }
            20 -> npcl(FaceAnim.HALF_ASKING, "If you Mourners really wanna help, why don't you do something about these mice?!").also { stage = END_DIALOGUE }
            21 -> npcl(FaceAnim.NEUTRAL, "Hmmm...doesn't look like it's seen daylight in years. That's not going to catch any mice!").also { stage = END_DIALOGUE }
        }
        return true
    }
