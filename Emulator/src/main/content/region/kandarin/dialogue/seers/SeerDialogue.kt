...
class SeerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> if (player.getQuestRepository().hasStarted(Quests.SCORPION_CATCHER) && player.getQuestRepository().getQuest(Quests.SCORPION_CATCHER).getStage(player) < 36) {
                end()
            ...
            201 -> if (!Diary.flagRewarded(player, DiaryType.SEERS_VILLAGE, 0)) {
            ...
            207 -> npc(FaceAnim.NOD_NO, "I know, I know. The poor boy is a simple soul - he just", "really loves picking flax. A little too much, I fear.").also { stage = END_DIALOGUE }
        }
        return true
    }
