...
class MonkeyMinderDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npcl(FaceAnim.FRIENDLY, "It's entirely necessary: just take a look at the monkeys. They've been multiplying out of control!").also { stage++ }
            5 -> npcl(FaceAnim.FRIENDLY, "Every time we turn our backs another one of them tries to break free. It's almost as if they're plotting to escape!").also {
            ...
            7 -> npcl(FaceAnim.FRIENDLY, "We just don't know. Perhaps it's the weather. Perhaps it's something in the food. Perhaps it's just that time of year.").also { stage++ }
            8 -> npcl(FaceAnim.FRIENDLY, "Your guess is as good as ours.").also { stage = END_DIALOGUE }
            9 -> npcl(FaceAnim.FRIENDLY, "We're going to post enough men here to be able to contain the monkeys in case they manage to escape.").also { stage++ }
