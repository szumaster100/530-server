...
class PenguinKeeperDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            1 -> npcl(FaceAnim.FRIENDLY, "Actually, you might be able to help me with something - if you are interested.").also { stage++ }
            2 -> playerl(FaceAnim.FRIENDLY, "What do you mean?").also { stage++ }
            3 -> npcl(FaceAnim.NEUTRAL, "Well, you see, the penguins have been laying so many eggs recently that we can't afford to raise them all ourselves.").also { stage++ }
            4 -> npcl(FaceAnim.HALF_ASKING, "You seem to know a bit about raising animals - would you like to raise a penguin for us?").also { stage++ }
            ...
            7 -> npcl(FaceAnim.FRIENDLY, "Fair enough. The offer still stands if you change your mind.").also { stage = END_DIALOGUE }
            ...
            10 -> npcl(FaceAnim.FRIENDLY, "They eat raw fish and aren't particularly fussy about anything, so it won't be any trouble to raise.").also { stage++ }
