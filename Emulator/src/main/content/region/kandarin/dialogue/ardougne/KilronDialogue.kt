...
class KilronDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        when {
            isQuestComplete(player, Quests.PLAGUE_CITY) && isQuestComplete(player, Quests.BIOHAZARD) -> npcl(FaceAnim.FRIENDLY, "Looks like you won't be needing the rope ladder any more, adventurer.").also { stage++ }
