...
class ColONiallDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            10 -> npcl(FaceAnim.FRIENDLY, "You look like a capable lad. Tell you what, when you've got a bit more experience under your belt, get yourself over to Falador.").also { stage++ }
            11 -> player(FaceAnim.FRIENDLY, "What's in Falador?").also { stage++ }
            12 -> npcl(FaceAnim.FRIENDLY, "An old friend of mine. You'll find him sitting on a bench in Falador Park. See what he can do for you.").also { stage++ }
            13 -> player(FaceAnim.FRIENDLY, "What's his name? Who should I say sent me?").also { stage++ }
            14 -> npcl(FaceAnim.FRIENDLY, "None of that matters if you can find him and if you're ready my name isn't necessary.").also { stage++ }
