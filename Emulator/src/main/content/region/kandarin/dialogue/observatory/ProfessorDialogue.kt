...
class ProfessorDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> when (buttonId) {
                1 -> if (getQuestStage(player, Quests.OBSERVATORY_QUEST) < 100) openDialogue(player, ProfessorDialogueFile()) else npcl(FaceAnim.ASKING, "Hello, friend. Welcome back. Thanks for all your help with the telescope. What can I do for you?").also { stage++ }
            ...
            8 -> npcl(FaceAnim.NEUTRAL, "Not right now, but the stars may hold a secret for you.").also { stage = END_DIALOGUE }
            9 -> npc(FaceAnim.FRIENDLY, "Ah, I get asked about Treasure Trails all the time!", "Listen carefully and I shall tell you what I know.", "Lots of clues have " + RED + "degrees</col> and " + RED + "minutes</col> written", "on them.").also { stage++ }
            10 -> npcl(FaceAnim.FRIENDLY, "These are coordinates of the place where the treasure is buried. You will have to walk to the correct spot, so that your coordinates are exactly the same as the values written on the clue scroll.").also { stage++ }
            11 -> npc(FaceAnim.FRIENDLY, "To do this, you must use a " + RED + "sextant</col>, a " + RED + "watch</col> and ", "a$RED chart</col> to find the coordinates of where you are.").also { stage++ }
            12 -> npcl(FaceAnim.FRIENDLY, "Once you know the coordinates of your position, you know which way you have to walk to get to the treasure's coordinates!").also { stage++ }
            13 -> playerl(FaceAnim.THINKING, "Riiight. So, where do I get these items from?").also { stage++ }
            14 -> npcl(FaceAnim.FRIENDLY, "I think Murphy, the owner of the Fishing Trawler moored at Port Khazard, might be able to spare you a sextant. After that, the nearest clock tower is south of Ardougne - you could probably get a watch there.").also { stage++ }
            15 -> npcl(FaceAnim.FRIENDLY, "I've got plenty of charts myself; just come back here when you've got the sextant and watch, and I'll give you one and teach you how to use them.").also { stage++ }
