...
class CaptainLawgofDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            0 -> when (stage) {
                0 -> {
                    npc(FaceAnim.OLD_NORMAL, "Guthix be praised, the cavalry has arrived! Hero, how", "would you like to be made an honorary member of the", "Black Guard?")
                    stage++
                }
                ...
                2 -> {
                    npc(FaceAnim.OLD_NORMAL, "Hawhaw! 'What's that' " + (if (player.appearance.isMale) "he" else "she") + " asks, what a sense of", "humour! The Black Guard is the finest regiment in the", "dwarven army. Only the best of the best are allowed to", "join it and then they receive months of rigorous")
                    stage++
                }

                3 -> {
                    npc(FaceAnim.OLD_NORMAL, "training. However, we are currently in need of a hero,", "so for a limited time only I'm offering you, a human, a", "chance to join this prestigious regiment. What do you", "say?")
                    stage++
                }
                ...
                5 -> {
                    quest!!.start(player)
                    npc(FaceAnim.OLD_NORMAL, "That's the spirit! Now trooper, we have no time to waste", "- the goblins are attacking from the forests to the", "South. There are so many of them, they are", "overwhelming my men and breaking through our")
                    stage = 100
                }
            }
            ...
            10 -> when (stage) {
                0 -> {
                    if (DwarfCannon.allRailsFixed(player)) {
                        npc(FaceAnim.OLD_NORMAL, "Well done, trooper! The goblins seems to have stopped", "getting in. I think you've done the job!")
                        stage = 105
                    }
                ...
                2 -> {
                    npc(FaceAnim.OLD_NORMAL, "The goblins are still getting in, so there must still be", "some broken railings.")
                    stage++
                }
                ...
                100 -> {
                    npc(FaceAnim.OLD_NORMAL, "perimeter defences; could you please try to fix the", "stockade by replacing the broken rails with these new", "ones?")
                    stage++
                }
            ...
            80 -> when (stage) {
                ...
                2 ->
                    if (!player.inventory.containsItem(DwarfCannon.MOULD) || !player.inventory.containsItem(DwarfCannon.NULODION_NOTES)) {
