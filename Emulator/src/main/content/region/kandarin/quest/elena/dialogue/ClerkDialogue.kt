    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, Quests.PLAGUE_CITY)) {
            ...
            12 -> when (stage) {
                ...
                1 -> when (buttonID) {
                    1 -> playerl(FaceAnim.FRIENDLY, "I need permission to enter a plague house.").also { stage = 2 }
                ...
                4 -> when (buttonID) {
                    1 -> playerl(FaceAnim.HALF_GUILTY, "I'll try asking them then.").also { stage = END_DIALOGUE }
                    2 -> playerl(
                ...
                10 -> when (buttonID) {
                    1 -> playerl(FaceAnim.ANNOYED, "This is urgent though! Someone's been kidnapped!").also {
                        stage = 11
                    }

                    2 -> playerl(FaceAnim.HALF_GUILTY, "Okay, I'll leave him alone.").also { stage = END_DIALOGUE }
                    3 -> playerl(FaceAnim.HALF_GUILTY, "Do you know when he will be available?").also { stage = 14 }
                }
                ...
                14 -> npcl(FaceAnim.HALF_GUILTY, "Oh I don't know, an hour or so maybe.").also { stage = END_DIALOGUE }
            }

            in 13..15 -> when (stage) {
                0 -> npcl(FaceAnim.FRIENDLY, "Bravek will see you now but keep it short!").also { stage++ }
                1 -> playerl(FaceAnim.FRIENDLY, "Thanks, I won't take much of his time.").also { stage = END_DIALOGUE }
            }
