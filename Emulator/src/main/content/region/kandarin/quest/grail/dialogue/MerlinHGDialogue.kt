        ...
        when (stage) {
            0 -> {
                if (isQuestComplete(player!!, Quests.HOLY_GRAIL)) {
                    ...
                    stage = 20
                } else if (getQuestStage(player!!, Quests.HOLY_GRAIL) >= 50 && player!!.hasItem(Item(Items.HOLY_GRAIL_19, 1))) {
                    npcl(FaceAnim.NEUTRAL, "My magic powers tell me that you have discovered the Grail! Take it to Arthur immediately!")
                    stage = END_DIALOGUE
                } else {
                    playerl(FaceAnim.NEUTRAL, "Hello. King Arthur has sent me on a quest for the Holy Grail. He thought you could offer some assistance.").also { stage++ }
                }
            }
