...
class GolrieDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FaceAnim.OLD_NORMAL, "That's me. I've been stuck in here for weeks, those", "goblins are trying to steal my family's heirlooms. My", "grandad gave me all sorts of old junk.").also { stage++ }
            ...
            6 -> npc(FaceAnim.OLD_NORMAL, "Oh that, yes have it, it's just some old elven junk I", "believe.").also { addItem(player, Items.GLARIALS_PEBBLE_294); stage++ }
            7 -> sendDialogue("You give the key to Golrie.").also {
                removeItem(
                    player,
                    Item(Items.A_KEY_293, 1)
                ); stage++
            }
            8 -> npc(FaceAnim.OLD_HAPPY, "Thanks a lot for the key traveller. I think I'll wait in", "here until those goblins get bored and leave.").also { stage++ }
