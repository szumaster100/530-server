        ...
        when {
            ...
            questStage >= 40 -> {
                when (stage) {
                    ...
                    1 -> npcl(FaceAnim.OLD_NORMAL, "When will this battle end? I feel like I've been fighting forever.").also { stage = END_DIALOGUE }
                }
            }
            ...
            questStage > 30 -> {
                if (inInventory(player!!, Items.ORB_OF_PROTECTION_587)) {
                    when (stage) {
                        ...
                        1 -> npcl(FaceAnim.OLD_NORMAL, "Now we have the orb I'm much better. They won't stand a chance without it.").also { stage = END_DIALOGUE }
                    }
                } else {
                    when (stage) {
                        0 -> playerl(FaceAnim.FRIENDLY, "Hello again.").also { stage++ }
                        1 -> npcl(FaceAnim.OLD_NORMAL, "Well done, you've broken down their defences. This battle must be ours.").also { stage = END_DIALOGUE }
                    }
                }
            }
            ...
            questStage == 30 -> {
                when (stage) {
                    0 -> playerl(FaceAnim.HALF_ASKING, "Do you know the coordinates of the Khazard stronghold?").also { stage++ }
                    ...
                    4 -> npcl(FaceAnim.OLD_NORMAL, "The other two tracker gnomes should have the other coordinates if they're still alive.").also { stage++ }
