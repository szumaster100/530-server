    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        ...
        when {
            inInventory(player!!, Items.ORBS_OF_PROTECTION_588) -> {
                when (stage) {
                    1 -> npcl(FaceAnim.OLD_DEFAULT, "You're back, well done brave adventurer. Now the orbs are safe we can perform the ritual for the spirit tree. We can live in peace once again.").also { stage = END_DIALOGUE }
                }
            }
            ...
            questStage < 40 -> {
                when (stage) {
                    ...
                    3 -> npcl(FaceAnim.OLD_DEFAULT, "Please, we must have the orb if we are to survive.").also {
                        stage = END_DIALOGUE
                    }
                }
            }
            ...
            questStage == 40 -> {
                when (stage) {
                    1 -> npcl(FaceAnim.OLD_DEFAULT, "Khazard's men came. Without the orb we were defenceless. They killed many and then took our last hope, the other orbs. Now surely we're all doomed. Without them the spirit tree is useless.").also { stage = END_DIALOGUE }
                }
            }
