        ...
        when (stage) {
            ...
            4 ->
                if (!getAttribute(player!!, ZUtils.ASK_SITHIK_AGAIN, false)) {
                    when (buttonID) {
                        1 -> player(FaceAnim.HALF_GUILTY, "Do you know anything about the undead ogres at", "Jiggig?").also { stage++ }
                        ...
                        3 ->
                            if (getAttribute(player!!, ZUtils.ASK_SITHIK_ABOUT_OGRES, false)) {
                                ...
                                playerl(FaceAnim.HALF_GUILTY, "Why do you spend most of your time in bed?").also { stage = 23 }
                            }
...
class SithikIntsHamBookDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            5 -> npcl("Hmm, that's an interesting theory, care to back it up with any facts?").also {
                stage = END_DIALOGUE
            }
        }
    }
...
class SithikIntsUsedPortraitDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            2 -> npcl(FaceAnim.THINKING, "Hmmm, well it's not the most flattering of portraits, but I like the 'honesty' of the work...well done.").also { stage = END_DIALOGUE }
...
class SithikIntsSignedPortraitDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            6 -> playerl(FaceAnim.HALF_THINKING, "Ok, let's say two million...two million to keep quiet and give me the picture.").also { stage++ }
            ...
            8 -> playerl(FaceAnim.HALF_WORRIED, "Oh...erm...well, that is a lot of money actually...er....").also { stage++ }
            ...
            11 -> when (buttonID) {
                1 -> player(FaceAnim.GUILTY, "No, I won't take the money, I'm going to bring you to justice!").also { stage++ }
            ...
            13 -> npcl(FaceAnim.LAUGH, "Ha! Ha! You believed me! I'm not going to give you all my money! No one will believe a crazy adventurer and a local Inn keep!").also { stage++ }
...
class SithikIntsAfterTransformDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            6 -> npc(FaceAnim.OLD_DEFAULT, "Unfortunately you can't. The spell is permanent, it will", "last forever, the only option you have is to move the", "ceremonial area.").also { stage++ }
            ...
            8 -> npc(FaceAnim.OLD_DEFAULT, "No...no, let me try to make amends...please I can help", "you. Just don't leave me like this.").also { stage = 4 }
            9 -> npc(FaceAnim.OLD_DEFAULT, "Ok, similar spells have been cast before and the only", "way to deal with the resulting creatures is to cordon off", "the area and not go in there again.").also { stage++ }
            10 -> npc(FaceAnim.OLD_DEFAULT, "The undead creatures usually manifest some sort of", "disease so it's best to attack them from a distance with a", "ranged weapon.").also { stage++ }
            11 -> npc(FaceAnim.OLD_DEFAULT, "Normal missiles like arrows and darts do very little", "damage to them because they're designed to destroy", "internal organs. This is a waste of time with undead", "creatures like undead ogres.").also { stage++ }
            12 -> player("Yeah, clearly so what should we use?").also { stage++ }
            13 -> npc(FaceAnim.OLD_DEFAULT, "From my research it looks like a flat ended arrow was", "designed called a 'Brutal arrow'. This does large", "amounts of crushing damage to the creature. You can", "make them by using larger arrows.").also { stage++ }
            14 -> npc(FaceAnim.OLD_DEFAULT, "I think some Ogre hunters make them. But instead of", "adding an arrowhead, you hammer a large nail into the", "end of the shaft.").also { stage = 4 }
            15 -> npc(FaceAnim.OLD_DEFAULT, "My research shows that two jungle based herbs can be", "used, one is found near river tributaries and looks like", "a vine, the other is found in caves and grows on the", "wall.").also { stage++ }
            16 -> npc(FaceAnim.OLD_DEFAULT, "It's quite well camouflaged so it's unlikely that you'll", "find it.").also { stage++ }
