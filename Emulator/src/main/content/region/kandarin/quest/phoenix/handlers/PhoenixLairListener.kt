    ...
    override fun defineListeners() {
        ...
        on(Scenery.CAVE_ENTRANCE_41900, IntType.SCENERY, "enter") { player, _ ->
            ...
            if (player.familiarManager.hasFamiliar() && familiar != null && (familiar.id == NPCs.PHOENIX_8575 || familiar.id == NPCs.PHOENIX_8576)) {
                sendNPCDialogue(player, NPCs.PHOENIX_8575, "${player.username}, this is my lair. You must dismiss my summoned form if you wish to enter; I will only duel you when I am at full strength.")
                return@on true
            }
            if (getVarbit(player, Vars.VARBIT_QUEST_IN_PYRE_NEED_PROGRESS_5761) < 1 || !isQuestComplete(player, Quests.IN_PYRE_NEED)) {
                sendPlayerDialogue(player, "Hey, wait! I have something to ask you before going down there.", FaceAnim.THINKING)
        ...
        onUseWith(IntType.ITEM, Items.KNIFE_946, *allTwigs) { player, _, with ->
            if (allTwigs.isNotEmpty() && allTwigs.all { hasAnItem(player, it).container == player.inventory }) {
                ...
                queueScript(player, 2, QueueStrength.SOFT) {
                    ...
                    sendItemDialogue(player, with.id, "You fetch the ${getItemName(with.asItem().id).lowercase()} into wooden ribbons.")
                    return@queueScript stopExecuting(player)
                }
            } else {
