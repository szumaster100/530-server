...
class DronDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> when (buttonId) {
                1 -> playerl(FaceAnim.HALF_GUILTY, "Erm, sorry. I don't mean to cause offence...").also { stage = 5 }
            ...
            47 -> when (buttonId) {
                1 -> playerl(FaceAnim.FRIENDLY, "12, but what does that have to do with anything?.").also { stage++ }
