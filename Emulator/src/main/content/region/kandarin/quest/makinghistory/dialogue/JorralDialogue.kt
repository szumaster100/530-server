...
class JorralDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when {
            ...
            inInventory(player, Items.LETTER_6756) && questStage >= 1 -> npcl(
                FaceAnim.HALF_ASKING,
                "Have you taken that letter to King Lathas in Ardougne yet?"
            ).also { stage = 400 }

            inInventory(player, Items.LETTER_6757) -> playerl(
                FaceAnim.FRIENDLY,
                "I've been to see the king and he gave me this letter."
            ).also { stage = 88 }

            isQuestComplete(player, Quests.MAKING_HISTORY) -> options(
                "How's the outpost?",
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (stage) {
            ...
            43 ->
                if (getAttribute(player, MakingHistoryUtils.ATTRIBUTE_ERIN_PROGRESS, false) || getVarbit(
                        player,
                ...
                } else if (getAttribute(player, MakingHistoryUtils.ATTRIBUTE_DROALAK_PROGRESS, false) || getVarbit(
                        player,
                ...
                } else if (getAttribute(player, MakingHistoryUtils.ATTRIBUTE_DRON_PROGRESS, false) || getVarbit(
                        player,
            ...
            65 -> npcl(FaceAnim.FRIENDLY, "Excellent! What did he say?").also {
                openInterface(player!!, Components.FADE_TO_BLACK_115)
                stage++
            }

            66 -> playerl(FaceAnim.FRIENDLY, "Well...").also {
