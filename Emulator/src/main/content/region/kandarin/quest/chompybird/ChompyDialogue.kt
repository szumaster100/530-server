...
class ChompyDialogue(val quest: Quest) : DialogueFile() {
    ...
    private fun handleQuestStartDialogue(player: Player?, buttonId: Int) {
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Hey you creature! Make some stabbers! I wanna hunt da chompy?").also { stage++ }
            ...
            3 -> npcl(FaceAnim.OLD_NORMAL, "For da stabbie chucker, I's wanna hunt da chompy! Creature knows what Rantz wants... ...flyin' to stabbie da chompy!").also { stage++ }
            4 -> npcl(FaceAnim.OLD_NORMAL, "The ogre shows you a huge but crude bow and then starts to nod energetically in an effort to help you understand.").also { stage++ }
            5 -> playerl("I think I understand. You want me to make some arrows for you?").also { stage++ }
            6 -> npcl(FaceAnim.OLD_NORMAL, "Yeah, is what Rantz sayed, make da stabbers for da stabby chucker!").also { stage = 1 }
            7 -> npcl(FaceAnim.OLD_NORMAL, "Da chompy is der bestest yummies for Rantz, Fycie and Bugs! We's looking for da yummies all da time. Da chompy is a big flapper, Rantz want's stabbers to sneaky, sneaky, stick da chompy.").also { stage++ }
            8 -> playerl("Ah, so 'da chompy' is some kind of bird?").also { stage++ }
            9 -> npcl(FaceAnim.OLD_NORMAL, "Yeah, is what Rantz sayed, Da chompy is da big flapper and is bestest yummies. But Rantz needs stabbers to stick da chompy... Will creatures make dem stabbers for us?").also { stage = 1 }
            10 -> npcl(FaceAnim.OLD_NORMAL, "Good you creature, you need sticksies from achey tree and stabbies from dog bones.").also { stage = END_DIALOGUE; quest.start(player) }
        }
    }
    ...
    private fun handleImpatienceDialogue(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Hey you creature... Have you made me da stabbers? I wanna stick da chompy?").also { stage++ }

            1 -> {
                if (!hasArrows) {
                    playerl("Er not exactly?").also { stage++ }
                } else {
                    playerl(FaceAnim.ANNOYED, "Well, yes actually, as you asked so nicely. Here you go! Here's your 'stabbers'.")
                    stage = 100
                }
            }

            2 -> npcl(FaceAnim.OLD_NORMAL, "You do stabbers quick!...quick!.. Or Rantz make stabbers for Rantz and then practice for chompy sticking on creature!").also { stage++ }
            ...
            5 -> npcl(FaceAnim.OLD_NORMAL, "Grrr creature... you's no good stabber maker! You's make da stabbie bit from da dog bones, and get da sticksies from da Achey tree... simple see! Oh and da flufgies from da flappers as well!").also { stage++ }
    ...
    private fun handleToadsiesDialogue(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            ...
            1 -> npcl(FaceAnim.OLD_NORMAL, "Da chompy still not coming! We need da fatsy toady to get da chompy, do you got it? Do you got da fatsy toady? Then we can sneaky, sneaky stick da chompy.").also { stage++ }
            ...
            6 -> npcl(FaceAnim.OLD_NORMAL, "Chompys love da fatsy toadies. Toadies get big on der swamp gas and der chomys are licking der lips for em as me is licking lips for da chompy. Da chompys don't like da smaller toadies from nearby swampy.").also { stage++ }
            7 -> npcl(FaceAnim.OLD_NORMAL, "Dey's fussie eaters like Rantz. Fycie an Bugs play with toadies and blower dey's all times making fatsy toadies.").also { stage = 4 }
            8 -> npcl(FaceAnim.OLD_NORMAL, "Fatsy toadies are da chompy burds bestest yumms. But da toadies here are too small for da chompy. You've godda make da toadies big and round!").also { stage = 4 }
            ...
            11 -> npcl(FaceAnim.OLD_NORMAL, "Ok creature? You got dat? Over here by der no tree's place.").also { stage = 4 }
            12 -> npcl(FaceAnim.OLD_NORMAL, "Duh! You creature is a bit stoopid yes? Us needs to sneaky, sneaky.. and stick da chompy! Den we can eat da chompy!").also { stage = 4 }
            100 -> sendItemDialogue(player, Items.BLOATED_TOAD_2875, "You show the bloated toad to Rantz. He nods with approval.").also { stage++ }
            101 -> npcl(FaceAnim.OLD_NORMAL, "Dat's a good fatsy toady, now we's need to put it for da chompy to come.").also { stage++ }
            102 -> {
