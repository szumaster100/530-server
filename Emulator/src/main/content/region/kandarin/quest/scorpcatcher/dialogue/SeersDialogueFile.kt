...
class SeersDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when {
            (questStage in 1..30) -> {
                when (stage) {
                    ...
                    1 -> when (buttonID) {
                        ...
                        2 -> playerl(FaceAnim.NEUTRAL, "Your friend Thormac sent me to speak to you.").also { stage = 2 }
                    ...
                    5 -> npcl(FaceAnim.NEUTRAL, "Let me look into my looking glass.").also {
                        lock(player!!, 6)
            ...
            (questStage == 10) -> {
                when (stage) {
                    0 -> playerl(FaceAnim.NEUTRAL, "Hi, I have retrieved the scorpion from near the spiders.").also { stage++ }
                    1 -> npc(FaceAnim.NEUTRAL, "Well, I've checked my looking glass. There seems to be", "a kharid scorpion in a little village in the east,", "surrounded by lots of uncivilized-looking warriors. Some", "kind of merchant there seems to have picked it up.").also { stage++ }
                    ...
                    4 -> npcl(FaceAnim.NEUTRAL, "It's good that you should ask. I have information on the last scorpion for you.").also { stage++ }
                    5 -> npcl(FaceAnim.NEUTRAL, "It seems to be in some sort of upstairs room. There seems to be some sort of brown clothing lying on a table.").also { stage++ }
                    6 -> {
                        end()
