...
@Initializable
class WatchtowerWizard(player: Player? = null) : Dialogue(player) {

    private val evidenceItems = intArrayOf(Items.FINGERNAILS_2384, Items.DAMAGED_DAGGER_2387, Items.TATTERED_EYE_PATCH_2388, Items.OLD_ROBE_2385, Items.UNUSUAL_ARMOUR_2386)
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage) {
            ...
            30 -> if(!allInInventory(player, *evidenceItems)) {
                player("Have a look at these...").also { stage++ }
            } else {
                npc("Interesting, very interesting.").also { stage += 2 }
            }

            31 -> npcl(FaceAnim.NEUTRAL, "No, sorry, this is not evidence. You need to keep searching, I'm afraid.").also { stage = END_DIALOGUE }
            ...
            37 -> npc("They normally keep to themselves, though. It's unusual", "for them to venture from their caves.").also { stage++ }
            ...
            42 -> npc("They are deep and complex. The only way you will", "navigate the caves is to have a map or something.").also { stage++ }
            ...
            48 -> npc("You need to be fearless and gain entrance to", "Gu'Tanoth, the city of the ogres, then find out how to", "navigate the caves.").also { stage++ }
            49 -> player("That sounds scary...").also { stage++ }
            50 -> npc("Ogres are nasty creatures, yes. Only a strong warrior,", "and a clever one at that, can get the better of them.").also { stage++ }
