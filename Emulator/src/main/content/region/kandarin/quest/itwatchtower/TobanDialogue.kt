...
class TobanDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (questStage == 5) {
            npc(FaceAnim.OLD_NEUTRAL, "Hahaha! Small t'ing returns. Did you bring the dragon", "bone?").also { stage = 10 }
            return true
        }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            11 -> sendDialogue(player, "Toban is glad, but you can't hold his reward; you'd best put somet'ing down.").also { stage = END_DIALOGUE }
            12 -> npc(FaceAnim.OLD_NEUTRAL, "Then you shall get nothing from me!").also { stage = END_DIALOGUE }
            13 -> npc(FaceAnim.OLD_NEUTRAL, "Hahaha! Small t'ing has done it. Toban is glad - take", "this...").also { stage++ }
            14 -> {
