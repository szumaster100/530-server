    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, Quests.FIGHT_ARENA)) {
            ...
            20 -> when (stage) {
                ...
                1 -> npcl(FaceAnim.CHILD_SAD, "Please " + (if (player!!.isMale) "Sir" else "Madam") + ", don't hurt me.").also { stage++ }
                2 -> playerl(FaceAnim.FRIENDLY, "Sshh. This uniform is a disguise. I'm here to help. Where do they keep the keys?").also { stage++ }
            ...
            in 90..100 -> when (stage) {
                0 -> npcl(FaceAnim.CHILD_NORMAL, "You need to kill the creatures in the arena").also {
                    stage = END_DIALOGUE
                }
            }
        }
