...
class LocalDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npcl(FaceAnim.FRIENDLY, "I heard the Servil family are fighting soon. Should be very entertaining.").also { stage++ }
            ...
            6 -> npcl(FaceAnim.FRIENDLY, "Only Khazard guards are allowed to see prisoners. Unless you know where to get some Khazard armour, you won't be visiting anyone.").also { stage = END_DIALOGUE }
            7 -> npcl(FaceAnim.ASKING, "Hello stranger, are you new to these parts? You look lost.").also { stage++ }
            8 -> npcl(FaceAnim.HALF_THINKING, "I suppose you're here for the fight arena? There are some rich folk fighting tomorrow. Should be entertaining.").also { stage = END_DIALOGUE }
            9 -> npcl(FaceAnim.FRIENDLY, "You're the guy who beat Bouncer! Amazing! That makes you a champion to many, but you'd better get out of here while you still can. General Khazard liked that brute and will be after you.").also { stage++ }
            10 -> playerl(FaceAnim.FRIENDLY, "He tried, but I killed him. You are safe now.").also { stage++ }
            11 -> npcl(FaceAnim.FRIENDLY, "Sadly, you can't kill what is already dead. He's probably still out there, somewhere.").also { stage = END_DIALOGUE }
        }
        return true
    }
