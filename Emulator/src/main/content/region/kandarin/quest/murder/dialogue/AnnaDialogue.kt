    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        ...
        when (questStage) {
            in 1..4 -> when (stage) {
                ...
                2 ->
                    if (getQuestStage(player, Quests.MURDER_MYSTERY) == 3) {
                        when (buttonID) {
                            ...
                            2 -> playerl(FaceAnim.SUSPICIOUS, "Where were you when the murder happened?").also { stage = 6 }
                        ...
                        when (buttonID) {
                            ...
                            2 -> playerl(FaceAnim.SUSPICIOUS, "Where were you when the murder happened?").also { stage = 6 }
                            3 -> playerl(FaceAnim.SUSPICIOUS, "Do you recognise this thread?").also { stage = 7 }
                            4 -> playerl(FaceAnim.SUSPICIOUS, "Why'd you buy poison the other day?").also { stage = 9 }
                        }
                    }
                ...
                6 -> npcl(FaceAnim.ANNOYED, "In the library. No one else was there so you'll just have to take my word for it.").also { stage = END_DIALOGUE }
                ...
                7 -> {
                    if (inInventory(player!!, Items.CRIMINALS_THREAD_1809)) {
                        ...
                        npcl(FaceAnim.ANNOYED, "Not really, no. Thread is fairly common.").also { stage = END_DIALOGUE }
                    }
                }

                8 -> npcl(FaceAnim.ANNOYED, "It's some $threadColor thread. It's not exactly uncommon is it? My trousers are made of the same material.").also { stage = END_DIALOGUE }

                9 -> npcl(FaceAnim.NEUTRAL, "That useless Gardener Stanford has let his compost heap fester. It's an eyesore to the garden! So I bought some poison from a travelling salesman so that I could kill off some of the wildlife living in it.").also { stage++ }
