        ...
        when (getQuestStage(player!!, Quests.TOWER_OF_LIFE)) {
            ...
            3 ->
                if (getAttribute(player, TowerOfLifeUtils.TOL_TOWER_ACCESS, 0) == 1) {
                    ...
                    when (stage) {
                        ...
                        4 -> playerl(FaceAnim.FRIENDLY, "I'm going to stop before this gets violent.").also {
                            stage = END_DIALOGUE
                        }
                    }
                }
            ...
            6 -> if (getAttribute(player, TowerOfLifeUtils.TOL_CUTSCENE, false)) {
                when (stage) {
                    ...
                    1 -> npcl(FaceAnim.FRIENDLY, "It'll be alright: it just needs a pat on the back.").also { stage++ }
                    2 -> playerl(
                        FaceAnim.FRIENDLY,
            ...
            8 -> when (stage) {
                START_DIALOGUE -> playerl(FaceAnim.FRIENDLY, "The Homunculus is a bit better off now.").also { stage++ }
