...
class EffigyDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, "Tower of Life")) {
            0 -> when (stage) {
                START_DIALOGUE -> playerl(FaceAnim.FRIENDLY, "Hi. Interesting building you have here.").also { stage++ }
                ...
                7 -> npcl(FaceAnim.FRIENDLY, "It's going to have a use that will change this world!").also { stage++ }
                8 -> npcl(FaceAnim.FRIENDLY, "Or at least it would if we could ever get it finished.").also { stage++ }
                ...
                20 -> when (buttonID) {
                    ...
                    2 -> playerl(FaceAnim.FRIENDLY, "Sorry, feeling a bit ill today.").also { stage = END_DIALOGUE }
                }
            ...
            3 -> if (getAttribute(player, TowerOfLifeUtils.TOL_TOWER_ACCESS, 0) == 1) when (stage) {
                ...
                5 -> npcl(FaceAnim.FRIENDLY, "Oh, what a wondrous person you are!").also { stage = END_DIALOGUE }
            }
            ...
            8 -> when (stage) {
                ...
                14 -> npcl(FaceAnim.FRIENDLY, "Go look dungeon, ${player.username}. Please meet there.").also {
                    stage = END_DIALOGUE
                }
            }
        }
