...
class TrainingGroundListener : InteractionListener {
    ...
    override fun defineListeners() {
        on(intArrayOf(Scenery.LARGE_DOOR_25594, Scenery.LARGE_DOOR_25595), IntType.SCENERY, "open") { player, node ->
            ...
            if (getAttribute(player, KnightWaveAttributes.KW_COMPLETE, false) || !getAttribute(player, KnightWaveAttributes.KW_BEGIN, false)) {
            ...
            if (player.location.x >= 2752 && getAttribute(player, KnightWaveAttributes.KW_BEGIN, false)) {
                ...
                removeAttributes(player, GameAttributes.PRAYER_LOCK, KnightWaveAttributes.KW_SPAWN, KnightWaveAttributes.KW_TIER, KnightWaveAttributes.KW_BEGIN)
            ...
            GameWorld.Pulser.submit(
                object : Pulse(1) {
                    override fun pulse(): Boolean {
                        TrainingGroundActivity.KnightWavesNPC(player.getAttribute(KnightWaveAttributes.KW_TIER, 6177), Location.create(2758, 3508, 2), player).init()
