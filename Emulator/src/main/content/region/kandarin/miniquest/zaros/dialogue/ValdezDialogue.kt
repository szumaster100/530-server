...
class ValdezDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (CurseOfZaros.hasTag(player, npc.id) || CurseOfZaros.hasComplete(player)) {
            val item = Items.GHOSTLY_ROBE_6107
            if (!CurseOfZaros.hasItems(player, item)) {
                player(FaceAnim.SAD, "I lost that Robe top you gave me...", "Can I have another please?").also {
                    stage = 38
                }
            } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            35 -> npcl(FaceAnim.HALF_GUILTY, CurseOfZaros.getLocationDialogue(npc.id, npc.location)?.joinToString("\n") ?: "").also { stage++ }
