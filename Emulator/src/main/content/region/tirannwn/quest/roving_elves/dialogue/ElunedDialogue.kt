...
class ElunedDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        if (getQuestStage(player, Quests.ROVING_ELVES) == 10) {
            player(FaceAnim.HALF_GUILTY, "Hey there... Islwyn said you may be able to help me.", "He told me you know how to consecrate ground for an", "elven burial. I need to reconsecrate Glarial's resting", "place.")
            stage = 1
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            500 -> end()
            1000 -> {
                if (getQuestStage(player, Quests.ROVING_ELVES) == 15 && !inInventory(player, RovingElves.CONSECRATION_SEED_CHARGED, 1)) {
                    npc(FaceAnim.HALF_GUILTY, "Hello, any luck finding the consecration seed?")
                    stage = 1002
                }
                if (getQuestStage(player, Quests.ROVING_ELVES) == 15 && inInventory(player, RovingElves.CONSECRATION_SEED_CHARGED, 1)) {
                    npc(FaceAnim.HALF_GUILTY, "You still have the charged seed with you, I can feel it.", "Hurry adventurer, go plant the seed and free", "my grandmother's spirit.")
                    stage = 500
                }
                if (getQuestStage(player, Quests.ROVING_ELVES) == 15 && inInventory(player, RovingElves.CONSECRATION_SEED, 1)) {
                    npc(FaceAnim.HALF_GUILTY, "Hello, any luck finding the consecration seed?")
                    stage = 1002
                }
                if (getQuestStage(player, Quests.ROVING_ELVES) == 20) {
                    npc(FaceAnim.HALF_GUILTY, "Hey... how's it going? Have you managed to", "reconsecrate Glarial's resting place?")
                    stage = 12
                } else if (getQuestStage(player, Quests.ROVING_ELVES) != 15) {
                    npc(FaceAnim.HALF_GUILTY, "Hello there, it's a lovely day for a walk in the woods.", "So what can I help you with?")
                    stage = 1001
                }
            }

            1001 ->
                stage = if (getQuestStage(player, Quests.ROVING_ELVES) >= 100 && inInventory(player, Items.TINY_ELF_CRYSTAL_6103, 1)) {
            ...
            1002 ->
                stage = if (inInventory(player, RovingElves.CONSECRATION_SEED, 1)) {
            ...
            1003 -> {
                npc(FaceAnim.HALF_GUILTY, "Re-enter Glarial's tomb and defeat the tomb's guardian.", "Take the consecration seed it is guarding and", "bring it back to me.")
                stage = 500
            }
            ...
            1200 -> {
                ...
                npc(FaceAnim.HALF_GUILTY, "Very well. I'll recharge your teleportation", String.format("crystal for %d gold. What do you say?", price))
                stage = 1202
            }
            ...
            1204 -> end()
            1 -> {
                npc(FaceAnim.HALF_GUILTY, "That I do... It is elvish tradition to plant a specially", "enchanted crystal seed at the graves of our ancestors.", "The seed will create guardians to protect the area.")
                stage = 2
            }

            2 -> {
                npc(FaceAnim.HALF_GUILTY, "Unfortunately the crystal seed must be tuned to the", "person it's protecting... a new seed won't do. But you", "should be able to recover the seed from her old tomb.")
                stage = 3
            }

            3 -> {
                npc(FaceAnim.HALF_GUILTY, "The tomb's guardian will be protecting the seed, you'll", "need to defeat him to get it. Once you have it, return", "here and I will re-enchant it.")
                stage = 4
            }
            ...
            10 -> {
                npc(FaceAnim.HALF_GUILTY, "Plant this seed in Glarial's new tomb, close to her", "remains and she will rest in peace.")
                stage = 11
            }
