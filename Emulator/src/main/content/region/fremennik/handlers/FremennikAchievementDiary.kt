...
class FremennikAchievementDiary : DiaryEventHookBase(DiaryType.FREMENNIK) {
    ...
    companion object {
        ...
        private val FISHING_SPOTS =
            arrayOf(NPCs.FISHING_SPOT_309, NPCs.FISHING_SPOT_334, NPCs.FISHING_SPOT_324, NPCs.FISHING_SPOT_322)
        private val ROCK_CRABS = arrayOf(NPCs.ROCK_CRAB_1265, NPCs.ROCK_CRAB_1267)
        private val CAVE_CRAWLERS =
            arrayOf(NPCs.CAVE_CRAWLER_1600, NPCs.CAVE_CRAWLER_1601, NPCs.CAVE_CRAWLER_1602, NPCs.CAVE_CRAWLER_1603)
        ...
        private val ICE_TROLLS = arrayOf(
            ...
            NPCs.ICE_TROLL_RUNT_5525,
            NPCs.ICE_TROLL_MALE_5526,
            NPCs.ICE_TROLL_FEMALE_5527,
    ...
    override fun onDialogueOptionSelected(player: Player, event: DialogueOptionSelectionEvent) {
        when (event.dialogue) {
            ...
            is content.region.fremennik.quest.viking.dialogue.ChieftanBrundtDialogue -> {
    ...
    override fun onInteracted(player: Player, event: InteractionEvent) {
        when (player.viewport.region.id) {
            ...
            10811 -> if (event.target.id == Scenery.COLLAPSED_TRAP_19233 && inBorders(
                    player,
                    RELLEKKA_HUNTING_AREA
                ) && event.option == "dismantle"
            ) {
                finishTask(
                    player,
                    DiaryLevel.MEDIUM,
    ...
    override fun onButtonClicked(player: Player, event: ButtonClickEvent) {
        when {
            inBorders(player, RELLEKKA_BLACKSMITH_AREA) -> {
                if (event.iface == Components.CRAFTING_GLASS_542 && event.buttonId == 38 && getStatLevel(
                        player,
                        Skills.CRAFTING
                    ) >= 33
                ) {
