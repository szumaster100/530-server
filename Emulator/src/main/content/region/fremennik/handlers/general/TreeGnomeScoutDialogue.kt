...
class TreeGnomeScoutDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npc(FaceAnim.ANNOYED, "I'll be glad when all this is over. The General will be", "strong again and we will push back this tide of filth that", "is humanity.").also { stage++ }
            5 -> player(FaceAnim.ANGRY, "Oi! I am one of the filth, I mean, humans! Besides, we", "were here first.").also { stage++ }
            6 -> npc(FaceAnim.EXTREMELY_SHOCKED, "Haa, haa, haa. Oh, you naive mortals. There were many", "here before you. The last time the General went north, ", "all this land belonged to the elves. Do not think so", "highly of your race.").also { stage++ }
            7 -> player("Discovered anything interesting on your travels?").also { stage++ }
            8 -> if (isQuestComplete(player, Quests.BIOHAZARD) || isQuestComplete(player, Quests.PLAGUE_CITY)) {
            ...
            10 -> if (!hasRequirement(player, Quests.HAZEEL_CULT)) {
            ...
            11 -> if (!isQuestComplete(player, Quests.THE_GRAND_TREE)) {
            ...
            100 -> npcl(FaceAnim.FRIENDLY,"I can't speak to you; I must continue on my mission.").also { stage = END_DIALOGUE }
        }
        return true
    }
