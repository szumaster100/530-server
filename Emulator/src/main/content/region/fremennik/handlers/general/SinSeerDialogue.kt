...
class SinSeerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        npcl(FaceAnim.SCARED, "My inner eye is clouded from the wave of darkness; give me a moment. What do you want? Augh! Your sins! They BLIND me!").also { stage = END_DIALOGUE }
        return true
    }

    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FaceAnim.THINKING, "Are you another one of those 'holier than thou' fools", "wanting to redeem themselves?").also { stage++ }
            1 -> player(FaceAnim.CALM, "Uh, no actually. I'm trying to join a really evil guy's", "team.").also { stage++ }
            ...
            4 -> npcl(FaceAnim.HALF_ASKING, "So, whom are you trying to hook up with? Dark priest? Warlord? Penguin?").also { stage++ }
            5 -> playerl(FaceAnim.FRIENDLY, "Uh, I want to prove to General Khazard I'm trustworthy.").also { stage++ }
            6 -> npcl(FaceAnim.FRIENDLY, "Oh, that old fart? Is he still around? Humph. I dunno if I can be bothered.").also { stage++ }
            ...
            18 -> npcl(FaceAnim.HALF_ASKING, "Ooooh, you're going to learn of his plans and then stab him in the back?").also { stage++ }
