...
class SkulgrimenDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            108 -> when (buttonId) {
                1 -> {
                    if (amountInInventory(player, Items.COINS_995) < 5000 || !inInventory(
                            player,
                            Items.DAGANNOTH_HIDE_6155
            ...
            110 -> when (buttonId) {
                1 -> {
                    if (amountInInventory(player, Items.COINS_995) < 10000 || amountInInventory(
                            player,
                            Items.DAGANNOTH_HIDE_6155
                        ) < 3 || !inInventory(player, Items.ROCK_SHELL_SHARD_6159)
                    ) {
                        npcl(
                            FaceAnim.HALF_GUILTY,
            ...
            112 -> when (buttonId) {
                1 -> {
                    if (amountInInventory(player, Items.COINS_995) < 7500 || amountInInventory(
                            player,
                            Items.DAGANNOTH_HIDE_6155
                        ) < 2 || !inInventory(player, Items.ROCK_SHELL_SPLINTER_6161)
                    ) {
                        npcl(
                            FaceAnim.HALF_GUILTY,
