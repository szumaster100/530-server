...
class LighthouseBridgeListener : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        onUseWith(IntType.SCENERY, Items.PLANK_960, *BROKEN_BRIDGE) { player, _, bridge ->
            if (getQuestStage(player, Quests.HORROR_FROM_THE_DEEP) in if (bridge.id == BROKEN_BRIDGE_1) 5..10 else 10..20) {
            ...
            if (!inInventory(player, Items.HAMMER_2347, 1) && inInventory(player, Items.PLANK_960, 1) && inInventory(
                    player,
                ...
                queueScript(player, 1, QueueStrength.SOFT) { stage: Int ->
                    when (stage) {
                        ...
                        1 -> {
                            ...
                            sendDialogue(
                                player,
                                if (bridge.id == BROKEN_BRIDGE_1) "You create half a makeshift walkway out of the plank." else "You have now made a makeshift walkway over the bridge."
                            )
                            sendMessage(
                                player,
                                if (bridge.id == BROKEN_BRIDGE_1) "You create half a makeshift walkway out of the plank." else "You have now made a makeshift walkway over the bridge."
        ...
        on(BROKEN_BRIDGE_1, IntType.SCENERY, "Cross") { player, _ ->
            ...
            else if (getAttribute(player, HorrorFromTheDeepUtils.UNLOCK_BRIDGE, 0) == 2 || isQuestComplete(
                    player,
                    Quests.HORROR_FROM_THE_DEEP
                )
            ) {
        ...
        on(BROKEN_BRIDGE_2, IntType.SCENERY, "Cross") { player, _ ->
            if (getAttribute(player, HorrorFromTheDeepUtils.UNLOCK_BRIDGE, 0) == 2 || isQuestComplete(
                    player,
                    Quests.HORROR_FROM_THE_DEEP
                )
            ) {
