...
class AdvisorGhrimDiaryDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            0 -> if (player!!.achievementDiaryManager.getDiary(DiaryType.FREMENNIK)!!.isComplete(2, true) && !Diary.hasClaimedLevelRewards(player!!, DiaryType.FREMENNIK, 2)) {
                player(FaceAnim.FRIENDLY, "About the Achievement Diary...").also { stage = 10 }
            } else if (Diary.canReplaceReward(player!!, DiaryType.FREMENNIK, 2)) {
