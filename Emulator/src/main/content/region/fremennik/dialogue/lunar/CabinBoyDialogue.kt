...
class CabinBoyDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (inInventory(player, Items.EMERALD_LENS_9066, 1) || player.bank.contains(Items.EMERALD_LENS_9066, 1)) {
            playerl(FaceAnim.FRIENDLY, "So you've plucked up the courage to come and confront that girl!").also { stage = 0 }
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npcl(FaceAnim.CHILD_FRIENDLY, "She turned out to be really nice! She's joining us to become a pirate!").also { stage++ }
            3 -> playerl(FaceAnim.HALF_WORRIED, "Really? And you're not sceptical about this? You know, after what she did last time?").also { stage++ }
            ...
            7 -> playerl(FaceAnim.HALF_ASKING, "Like someone else was talking for you. Are you sure she hasn't put a spell on you again? Did she wave a watch in front of your face? You know, tell you to look deep into her eyes?").also { stage++ }
            8 -> npc(FaceAnim.CHILD_SUSPICIOUS, "She---has---no---spell---on---me.").also { stage++ }
            9 -> playerl(FaceAnim.HALF_THINKING, "I think you've been hypnotised. I wonder what happens if I click my fingers?").also { sendChat(player, "*click*").also { stage++ } }
            10 -> npc(FaceAnim.CHILD_SAD, "*Cluck* *cluck* *bwaarrk*").also { stage++ }
            11 -> npc(FaceAnim.CHILD_SAD, "").also { stage++ }
            12 -> player(FaceAnim.WORRIED, "Oh dear. Oh well, I'm sure you'll learn one day.").also { stage = END_DIALOGUE }
            ...
            22 -> npcl(FaceAnim.CHILD_FRIENDLY, "Hey, I think I'm learning a thing or two from these Moon Clan ladies.").also { stage++ }
            ...
            25 -> npc(FaceAnim.CHILD_FRIENDLY, "Huh, oh ok, I suppose I owe you one.").also {
                end()
