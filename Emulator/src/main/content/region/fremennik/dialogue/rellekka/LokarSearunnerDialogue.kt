    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (npc.id) {
            NPCs.LOKAR_SEARUNNER_4536 -> when (stage) {
                0 -> {
                    if (!isQuestComplete(player, Quests.THE_FREMENNIK_TRIALS)) {
                        npcl(FaceAnim.ANNOYED, "Don't talk to me outerlander! I don't have time for the likes of you!")
                        stage = 40
                    } else {
                        npcl(FaceAnim.FRIENDLY, "Hi again ${getAttribute(player, "fremennikname", "fremmyname")}! What can I do for you?").also { stage++ }
                        stage = 2
                    }
                }
                2 -> options(
                ...
                3 -> when (buttonId) {
                    ...
                    2 -> playerl(FaceAnim.FRIENDLY, "It seems you've been charming Pauline Polaris.").also { stage = 8 }
                    3 -> playerl(FaceAnim.FRIENDLY, "Nothing thanks! I just saw you here and thought I'd say hello!").also { stage = 43 }
                }
                4 -> npcl(FaceAnim.FRIENDLY, "Hey, if you want to go back to Loserville with the Loser Gang, who am I to stop you?").also { stage++ }
                5 -> {
                    end()
                    travel(player, Location(2621, 3687, 0))
                    return true
                }
                6 -> npcl(FaceAnim.FRIENDLY, "You are possibly the most indecisive person I have ever met...").also { stage++ }
                7 -> playerl(FaceAnim.FRIENDLY, "Well, 'bye then.").also { stage = END_DIALOGUE }
                8 -> npcl(FaceAnim.FRIENDLY, "What did you say?").also { stage = 10 }
                10 -> playerl(FaceAnim.FRIENDLY, "Sorry. I mean, she was, erm, telling me a little about your business deal with her farm.").also { stage++ }
                ...
                16 -> playerl(FaceAnim.FRIENDLY, "She said you'd got the seeds from the Jade Vine and that she was making it into livid.").also { stage++ }
                17 -> npcl(FaceAnim.FRIENDLY, "Yeah, it stands for Lokar's Infernal Vine of Incredible Death!").also { stage++ }
                18 -> npcl(FaceAnim.FRIENDLY, "Muahahaha!").also { stage++ }
                19 -> playerl(FaceAnim.FRIENDLY, "Why am I not surprised.").also { stage++ }
                20 -> npcl(FaceAnim.FRIENDLY, "I stole them from that nuts Papa Mambo. Wasn't sure what to do with the seeds at the time, they just seemed valuable.").also { stage++ }
                21 -> npcl(FaceAnim.FRIENDLY, "Then, when I was at Lunar Isle, I remembered Pauline and wondered if she might be able to turn it into something truly deadly. Not that I said that to her, obviously.").also { stage++ }
                22 -> playerl(FaceAnim.FRIENDLY, "Obviously.").also { stage++ }
                23 -> npcl(FaceAnim.FRIENDLY, "I told her they could be used to help protect the people of Rellekka.").also { stage++ }
                24 -> playerl(FaceAnim.FRIENDLY, "So, you lied?").also { stage++ }
                25 -> npcl(FaceAnim.FRIENDLY, "Well...at first. Afterwards, I was pleased to find that Brundt would actually pay a hefty fee to get hold of them. Gullible idiot.").also { stage++ }
                26 -> playerl(FaceAnim.FRIENDLY, "I think Pauline would be very disappointed to hear that.").also { stage++ }
                27 -> npcl(FaceAnim.FRIENDLY, "Hey, don't you go telling her!").also { stage++ }
                28 -> playerl(FaceAnim.FRIENDLY, "Okay, okay!").also { stage++ }
                29 -> playerl(FaceAnim.FRIENDLY, "It's probably for the best. She might shut the whole enterprise down.").also { stage++ }
                ...
                38 -> playerl(FaceAnim.FRIENDLY, "She's not used to your ways. It will only spell trouble.").also { stage++ }
                39 -> npcl(FaceAnim.FRIENDLY, "Don't worry yourself. Luckily for me, you can't decide what I do.").also { stage = END_DIALOGUE }
                40 -> player("What makes you say that?").also { stage++ }
                41 ->
                    if (!isQuestComplete(player, Quests.THE_FREMENNIK_TRIALS)) {
                        npcl(FaceAnim.FRIENDLY, "You've experience yet to gain. Especially in keeping with the Fremenniks and their trials!").also { stage++ }
                    } else {
                        npcl(FaceAnim.NEUTRAL, "Only people who've travelled on quests far from here can talk to me!").also { stage++ }
                    }
                42 -> player("Oh.").also { stage = END_DIALOGUE }
                43 -> npcl(FaceAnim.FRIENDLY, "Hey, I knew you seemed cool when I met you ${getAttribute(player, "fremennikname", "fremmyname")}!").also { stage++ }
                44 -> end()
            }
            NPCs.LOKAR_SEARUNNER_4537 -> when (stage) {
                ...
                3 -> npcl(FaceAnim.FRIENDLY, "Sheesh! Make your mind up, lady, I'm not a taxi service! Do you want to go or not?").also { stage++ }
