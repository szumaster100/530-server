    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when ((0..3).random()) {
            0 -> when (stage) {
                ...
                3 -> npcl(FaceAnim.HALF_GUILTY, "Yes, you know - peons, plebs, the downtrodden. He treats us like his own personal possessions.").also { stage++ }
                4 -> player("You should leave this place.").also { stage++ }
                5 -> npcl(FaceAnim.HALF_GUILTY, "I keep trying to save up enough to leave, but the King keeps taxing us! We have no money left.").also { stage++ }
                6 -> player("Oh dear.").also { stage = END_DIALOGUE }
            }

            1 -> when (stage) {
                START_DIALOGUE -> player("It's a bit grey round here, isn't it?").also { stage++ }
                1 -> npcl(FaceAnim.HALF_GUILTY, "It gets you down after a while, you know. There are 273 shades of grey, you know, and we have them all.").also { stage++ }
                2 -> player("That's grey-t.").also { stage++ }
                3 -> npcl(FaceAnim.HALF_GUILTY, "That attempt at humour merely made me more depressed. Leave me alone.").also { stage = END_DIALOGUE }
            }
            ...
            3 -> when (stage) {
                ...
                1 -> npcl(FaceAnim.HALF_GUILTY, "I'd prefer that, if it meant I didn't have to talk to people as insanely happy as you.").also { stage++ }
