...
class TowerGuardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npc(FaceAnim.NEUTRAL, "I'M SORRY I'VE BEEN SHOUTING", "INSULTS SO LONG I CAN'T HELP IT!").also { stage++ }
            3 -> playerl(FaceAnim.HALF_THINKING, "Who are you insulting?").also { stage++ }
            4 -> npc(FaceAnim.NEUTRAL, "THE TOWER IN ${if (npc.id == NPCs.GUARD_5489) "NEITIZNOT" else "JATIZSO"}.", "THEY SHOUT INSULTS BACK.").also { stage++ }
            5 -> playerl(FaceAnim.ASKING, "Err, why?").also { stage++ }
            6 -> npc(FaceAnim.NEUTRAL, "THE ${if (npc.id == NPCs.GUARD_5489) "KING" else "BURGHER"} TELLS US TO.").also { stage++ }
            7 -> playerl(FaceAnim.HALF_THINKING, "Your ${if (npc.id == NPCs.GUARD_5489) "King" else "Burgher"} is a strange person.").also { stage++ }
            ...
            10 -> npcl(FaceAnim.NEUTRAL, "IF YOU LIKE!").also {
                stage = END_DIALOGUE
