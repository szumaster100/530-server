...
class ThorodinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npc(FaceAnim.OLD_DEFAULT, "We're extending the cave so more people can live in it.", "These Miscellanians aren't so bad.", "They appreciate the benefits of living underground.").also { stage++ }
            3 -> player(FaceAnim.ASKING, "...such as?").also { stage++ }
            4 -> npc(FaceAnim.OLD_DEFAULT, "Not getting rained on, for example.", "Did you do anything about that monster Donal", "was talking about?").also { stage++ }
            5 -> player(FaceAnim.FRIENDLY, "It's been taken care of.").also { stage++ }
            6 -> npc(FaceAnim.OLD_HAPPY, "Glad to hear it.", "Now we can get on with excavating.").also { stage = END_DIALOGUE }
        }
        return true
    }
