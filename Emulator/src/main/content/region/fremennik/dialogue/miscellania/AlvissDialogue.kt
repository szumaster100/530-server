...
class AlvissDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npc(FaceAnim.OLD_DEFAULT, "I'm waiting for my shift, of course.", "We can't dig all the time, you know.").also { stage++ }
            3 -> npc(FaceAnim.OLD_DEFAULT, "I'm also researching the links between the", "Fremenniks and the Dwarves.").also { stage++ }
            4 -> npc(FaceAnim.OLD_DEFAULT, "I've found that we have some mythology in common.").also { stage = END_DIALOGUE }
        }
        return true
    }
