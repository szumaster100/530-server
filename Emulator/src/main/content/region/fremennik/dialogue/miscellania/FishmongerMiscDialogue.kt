...
class FishmongerMiscDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (!hasRequirement(player, Quests.THRONE_OF_MISCELLANIA)) {
            npcl(FaceAnim.FRIENDLY, "Greetings, Sir. Get your fresh fish here! I've heard that the Etceterian fish is stored in a cow shed.").also { stage = 0 }
        } else {
            npcl(FaceAnim.FRIENDLY, "Greetings, Your Highness. Have some fresh fish! I've heard that the Etceterian fish is stored in a cow shed.").also { stage = 0 }
        }
        return true
    }
