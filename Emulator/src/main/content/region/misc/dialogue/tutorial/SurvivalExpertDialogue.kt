...
class SurvivalExpertDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when(tutStage) {
            4 -> sendTutorialNPCDialogue(player, NPCs.SURVIVAL_EXPERT_943, "Hello there, newcomer. My name is Brynna. My job is", "to teach you a few survival tips and tricks. First off", "we're going to start with the most basic survival skill of", "all: making a fire.")
            11 -> sendTutorialNPCDialogue(player, NPCs.SURVIVAL_EXPERT_943, "Well done! Next we need to get some food in our", "bellies. We'll need something to cook. There are shrimp", "in the pond there, so let's catch and cook some.")
            5, 14, 15 -> {
            ...
            12 -> if(!inInventory(player, Items.SMALL_FISHING_NET_303)) {
                ...
                sendTutorialNPCDialogue(player, NPCs.SURVIVAL_EXPERT_943, "Well done! Next we need to get some food in our", "bellies. We'll need something to cook. There are shrimp", "in the pond there, so let's catch and cook some.").also { stage = END_DIALOGUE }
            }
...
class SurvivalExpert : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            1 -> when (buttonID) {
                1 -> npcl(FaceAnim.HAPPY, "After you leave Tutorial Island, you'll be able to use your <col=08088A>Fletching</col> skill to craft your own bows and arrows from trees.").also { stage = 2 }
                2 -> npcl(FaceAnim.FRIENDLY, "Light the logs in your backpack to make a fire.").also { stage = 0 }
                3 -> npcl(FaceAnim.HAPPY, "Whenever you see bubbles in the water, there's probably some good fishing to be had there!").also { stage = 0 }
                4 -> npcl(FaceAnim.HAPPY, "You'll cook your shrimp on a fire. If you're fire's gone out, chop a tree to get some logs and make a new fire.").also { stage = 3 }
                5 -> npcl(FaceAnim.HAPPY, "Open the gate, follow the path to the next area and talk to the master chef. He'll teach you more about <col=08088A>Cooking</col>.").also { stage = END_DIALOGUE }
            }
