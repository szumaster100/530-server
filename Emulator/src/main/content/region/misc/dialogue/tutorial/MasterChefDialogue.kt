...
class MasterChefDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (getAttribute(player, TutorialStage.TUTORIAL_STAGE, 0)) {
            18 -> when (stage) {
                0 -> sendTutorialNPCDialogue(player, npc.id, "I already know how to cook. Brynna taught me just", "now.").also { stage++ }
                1 -> sendTutorialNPCDialogue(player, npc.id, "Hahahahahaha! You call THAT cooking? Some shrimp", "on an open log fire? Oh, no, no no. I am going to", "teach you the fine art of cooking bread.").also { stage++ }
                2 -> sendTutorialNPCDialogue(player, npc.id, "And no fine meal is complete without good music, so", "we'll cover that while you're here too.").also { stage++ }
                3 -> {
            ...
            19, 20 -> when (stage) {
                0 -> {
                    if (!player.inventory.containsItems(Item(Items.BUCKET_OF_WATER_1929, 1), Item(Items.POT_OF_FLOUR_1933, 1)) || player.inventory.containsItems(Item(Items.BUCKET_1925, 1), Item(Items.EMPTY_POT_1931, 1))) {
                        ...
                        sendDoubleItemDialogue(player, Items.BUCKET_OF_WATER_1929, Items.POT_OF_FLOUR_1933, "The Cooking Guide gives you a <col=08088A>bucket of water<col> and a <col=08088A>pot of flour</col>.")
                    ...
                    if (!player.inventory.containsItems(Item(Items.BUCKET_OF_WATER_1929, 1)) && removeItem(player, Items.BUCKET_1925)) {
            ...
            in 21..26 -> when (buttonId) {
                0 -> player("Making dough.").also { stage = 150 }
                1 -> player("Range cooking.").also { stage = 200 }
                2 -> TutorialStage.rollback(player)
            }
            150 -> sendNPCDialogue(player, npc.id, "This is the base for many of the meals. To make dough we must mix flour and water. First, right click the bucket of water and select use, then left click on the pot of flour.").also { stage = 1000 }
            200 -> sendNPCDialogue(player, npc.id, "To cook the dough, use it with the range shown by the arrow.").also { stage = 1000 }
            in 27..100 -> sendNPCDialogue(player, npc.id, "Follow the path to the home of the quest guide.").also { stage = 1000 }
            1000 -> {
                stage = END_DIALOGUE
