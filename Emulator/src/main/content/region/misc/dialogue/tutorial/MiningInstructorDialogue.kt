        ...
        when (getAttribute(player, TutorialStage.TUTORIAL_STAGE, 0)) {
            ...
            35 -> {
                if (!inInventory(player, Items.BRONZE_PICKAXE_1265)) {
                    ...
                    sendItemDialogue(player, Items.BRONZE_PICKAXE_1265, "Dezzick gives you a$BLUE bronze pickaxe</col>!")
                    stage = 3
                } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (getAttribute(player, TutorialStage.TUTORIAL_STAGE, 0)) {
            30 -> when (stage) {
                ...
                1 -> sendNPCDialogue(player, npc.id, "Ok then, ${player.username}. My name is Dezzick and I'm a miner by trade. Let's prospect some of these rocks.").also { stage++ }
                2 -> {
                    end()
            ...
            34, 35 -> when (stage) {
                0 -> sendNPCDialogue(player, npc.id, "Absolutely right, ${player.username}. These two ore types can be smelted together to make bronze.").also { stage++ }
                1 -> sendNPCDialogue(player, npc.id, "So now you know what ore is in the rocks over there, why don't you have a go at mining some tin and copper? Here, you'll need this to start with.").also { stage++ }
                2 -> {
                    addItem(player, Items.BRONZE_PICKAXE_1265)
                    sendItemDialogue(player, Items.BRONZE_PICKAXE_1265, "Dezzick gives you a$BLUE bronze pickaxe</col>!")
                    stage++
            ...
            40, 41 -> when (stage) {
                0 -> sendNPCDialogue(player, npc.id, "Okay, I'll show you how to make a dagger out of it. You'll be needing this..").also { stage++ }
                1 -> {
                    addItem(player, Items.HAMMER_2347)
            ...
            150 -> {
                sendNPCDialogue(player, npc.id, "To prospect a mineable rock, just right click it and select the 'prospect rock' option. This will tell you the type of ore you can mine from it. Try it now on one of the rocks indicated.")
                TutorialStage.rollback(player)
                end()
            }
            200 -> {
                sendNPCDialogue(player, npc.id, "It's quite simple really. All you need to do is right click on the rock and select 'mine' You can only mine when you have a pickaxe. So give it a try: first mine one tin ore.")
                TutorialStage.rollback(player)
                end()
            }
        }
