...
class BarmanDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (amountInInventory(player, Items.COINS_995) > 2000) {
            npc(FaceAnim.OLD_HAPPY, "And what will it be for you, fine " + (if (player!!.isMale) "Sir" else "Madam") + "?")
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            20 -> npc(FaceAnim.OLD_HAPPY, "Ah, " + (if (player!!.isMale) "Sir" else "Madam") + " has taste! A jug of wine will cost", "you 10 gold coins.").also { stage++ }
