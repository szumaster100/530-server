...
class BentamirDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npcl(FaceAnim.OLD_ANGRY1, "We can't all live in plush houses, you know. But that doesn't mean us mining dwarves don't work hard.").also { stage++ }
            4 -> playerl(FaceAnim.FRIENDLY, "Where do you do your mining?").also { stage++ }
            5 -> npcl(FaceAnim.OLD_NOT_INTERESTED, "Normally the coal mine to the north. We need a lot of coal to keep our steam engines going, you know.").also { stage++ }
            6 -> playerl(FaceAnim.FRIENDLY, "Can I do a bit of mining there as well?").also { stage++ }
            7 -> npcl(FaceAnim.OLD_NOT_INTERESTED, "I'm not sure, but as long as no one notices I don't think anyone is going to care.").also { stage = END_DIALOGUE }
        }
        return true
    }
