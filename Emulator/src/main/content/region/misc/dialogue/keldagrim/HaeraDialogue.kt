...
class HaeraDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> sendNPCDialogue(player, NPCs.HEGIR_2188, "I didn't let him in, he let himself in!", FaceAnim.OLD_NORMAL).also { stage++ }
            ...
            5 -> npc(FaceAnim.OLD_NORMAL, "Well go ask someone else instead of barging", "into people's homes!").also { stage++ }
            6 -> sendNPCDialogue(player, NPCs.HEGIR_2188, "There's no need to be rude to the human, Haera dear.", FaceAnim.OLD_NORMAL).also { stage++ }
