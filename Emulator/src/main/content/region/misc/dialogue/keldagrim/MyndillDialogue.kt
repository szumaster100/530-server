...
class MyndillDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            6 -> npc(FaceAnim.OLD_NORMAL, "No, it's just that no company has ever willingly", "left the Consortium before.").also { stage++ }
            7 -> player(FaceAnim.HALF_ASKING, "Companies have been forced out of the", "Consortium then?").also { stage++ }
            8 -> npc(FaceAnim.OLD_NORMAL, "Oh yes. It hasn't happened for a long while now,", "but sometimes a company goes bankrupt. Or the value", "of the company becomes so low that it's relegated", "to being a minor company.").also { stage++ }
            ...
            11 -> npc(FaceAnim.OLD_NORMAL, "No, I have nothing that I could possible", "want doing. Try the shops or the market instead.").also { stage = 0 }
