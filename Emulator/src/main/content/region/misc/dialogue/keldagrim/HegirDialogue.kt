...
class HegirDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            1 -> sendNPCDialogue(player, NPCs.HAERA_2189, "Oh yes, young fellow? Yes, what do you want?", FaceAnim.OLD_NORMAL).also { stage++ }
            2 -> sendNPCDialogue(player, NPCs.HAERA_2189, "Who's that you're talking to Hegir?", FaceAnim.OLD_NORMAL).also { stage++ }
            ...
            7 -> sendNPCDialogue(player, NPCs.HAERA_2189, "Well ask him what he wants then!", FaceAnim.OLD_NORMAL).also { stage++ }
            ...
            11 -> sendNPCDialogue(player, NPCs.HAERA_2189, "What makes him think he lost his quest in here?", FaceAnim.OLD_NORMAL).also { stage++ }
            ...
            15 -> sendNPCDialogue(player, NPCs.HAERA_2189, "Stop talking to that human and get back in here before I have a task for you!", FaceAnim.OLD_NORMAL).also { stage++ }
            16 -> npc(FaceAnim.OLD_NORMAL, "No need to talk to me like that, Haera.").also { stage++ }
            17 -> sendNPCDialogue(player, NPCs.HAERA_2189, "I'll talk to you in whatever way I like!", FaceAnim.OLD_NORMAL).also { stage++ }
