...
class JoeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            10 -> npcl(FaceAnim.FRIENDLY, "All of a sudden I turn an there, bearin' down on me is this albatross...").also { stage++ }
            11 -> playerl(FaceAnim.NEUTRAL, "Sorry to interrupt, but I would just like to clear something up.").also { stage++ }
            12 -> npcl(FaceAnim.FRIENDLY, "What? I was just gettin' to the good bit.").also { stage++ }
            13 -> playerl(FaceAnim.NEUTRAL, "When I said 'I wanted to know where you got that harpoon hand', I meant where did you get it made, or buy it from?").also { stage++ }
            14 -> npcl(FaceAnim.FRIENDLY, "If ye don't wants to hear the story then I ain't gonna tell ye!").also { stage = END_DIALOGUE }
            16 -> {
                end()
