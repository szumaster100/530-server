...
class FairyQueenDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npcl(FaceAnim.OLD_DEFAULT, "Clearly you come from a plane dependent on sunlight. Down here, the plants grow in the aura of faerie.").also { stage = END_DIALOGUE }
            2 -> npcl(FaceAnim.OLD_DEFAULT, "Zanaris is a meeting point of cultures. Those from many worlds converge here to exchange knowledge and goods.").also { stage = END_DIALOGUE }
            3 -> npcl(FaceAnim.OLD_DEFAULT, "Not yet, " + player.username + ", but it looks like we'll need to ask you for your help again, I'm afraid. I'll be able to tell you more once we have finished making our battle plans.").also { stage = END_DIALOGUE }
        }
        return true
    }
