...
class GateKeeperDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FaceAnim.OLD_CALM_TALK1, "You mean my father? He went into retirement.", "I've taken over the family business.").also { stage++ }
            1 -> player("Your father? But you don't look anything like him!").also { stage++ }
            2 -> npc(FaceAnim.OLD_CALM_TALK1, "No. Fortunately for me, I inherited my good looks from", "my mother.").also { stage = END_DIALOGUE }
        }
        return true
    }
