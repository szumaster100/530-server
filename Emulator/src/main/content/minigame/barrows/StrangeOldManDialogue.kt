        ...
        if (inInventory(player, Items.SPADE_952)) {
            when (conversationNum) {
                ...
                4 -> when (stage) {
                    ...
                    2 -> when (buttonId) {
                        1 -> npcl(FaceAnim.FURIOUS, "AAAAAAAAARRRRRRGGGGGHHHHHHHH!").also { stage = END_DIALOGUE }
                        2 -> playerl(FaceAnim.STRUGGLE, "I'll leave you to it, then...").also { stage = END_DIALOGUE }
                    }
                }
            }
        } else {
            when (stage) {
                ...
                2 -> npcl(FaceAnim.FRIENDLY, "Ohh... Spades have cost me so much hassle - always being pestered for them! I ended up giving in and just putting one at each mound for you forgetful adventurers to use.").also { stage = END_DIALOGUE }
            }
        }
        return true
    }
