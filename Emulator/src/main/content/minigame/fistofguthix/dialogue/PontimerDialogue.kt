...
class PontimerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            17 -> when (buttonId) {
                1 -> npc(
                    "Well, this is Getorix's silly theory. Listen to it, if you must.",
            ...
            19 -> npc(
                "So, maybe he punched the ground in anger, leaving an",
                "imprint of power on the ground that has lasted ever since."
            ).also { stage++ }

            20 -> npc(
                "Because he was angry at the time, the power would have",
                "been tainted with aggression, which is why he left Fiara as",
                "a guardian, to make sure the power did not escape unfettered."
            ).also { stage++ }
            ...
            22 -> npc(
                ...
                "indentation in the centre of the cave doesn't look like a fist",
                "to me."
            ).also { stage++ }
            ...
            34 -> npc(
                "Well, no! I agree with you entirely. This theory has",
                "so many holes that it makes no sense at all!"
            ).also { stage++ }
