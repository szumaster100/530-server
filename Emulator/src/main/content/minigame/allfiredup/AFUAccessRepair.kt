...
class AFURepairClimbHandler : InteractionListener {
    ...
    private fun repair(player: Player, rco: RepairClimbObject) {
        ...
        if (requiresNeedle) {
            if (player.inventory.containsItem(Item(Items.NEEDLE_1733)) && player.inventory.containItems(*requiredItems.map { it.id }
                    .toIntArray())) {
            ...
            if (player.inventory.containsItem(Item(Items.HAMMER_2347)) && player.inventory.containItems(*requiredItems.map { it.id }
                    .toIntArray())) {
                Nails.get(player, 4)?.let { nails ->
    ...
    private enum class RepairClimbObject(
        ...
        fun getAnimation(player: Player): Animation =
            if (getOtherLocation(player) == destinationDown) Animation(Animations.WALK_BACKWARDS_CLIMB_1148) else Animation(
