...
class ProgressHatDialogue(player: Player? = null) : Dialogue(player) {
    ...
    val randomGreetings = arrayOf("Hello... Mr Progress Hat?", "Erm, excuse me Mr Progress Hat?", "Mr Progress Hat... sir?", "Mr Progress Hat? Hello?")
    private val responses = listOf(
        listOf("That's me, why are you bothering me human?", "Sorry, but do you think you could tell me my Pizazz Points?"),
        ...
        when (stage) {
            ...
            5 -> when (buttonId) {
                1 -> {
                    ...
                    if (!hasHat || !anyInEquipment(player, *ProgressHat.hats) || anyInInventory(player, *ProgressHat.hats)) {
                        end()
                        return true
                    }
