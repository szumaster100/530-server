...
class CreatureGraveyard : MTAZone(
    "Creature Graveyard",
    arrayOf(
    ...
    override fun interact(e: Entity, target: Node, option: Option): Boolean {
        if (e is Player) {
            when (target.id) {
                ...
                in org.rs.consts.Scenery.BONES_10725..org.rs.consts.Scenery.BONES_10728 -> BoneType.forObject(target.id)?.grab(e, target.asScenery())
    ...
    private fun deposit(player: Player) {
        ...
        var totalAmount = player.getAttribute("grave-amt", 0) + items.sumOf { item ->
        ...
        if (totalAmount >= 16) {
            ...
            sendDialogueLines(
                player,
                "Congratulations - you've been awarded a${if (StringUtils.isPlusN(runeName)) "n" else ""} $runeName and extra",
                "magic XP."
            )
        }
    ...
    enum class BoneType(val objectsId: Int, val item: Item) {
        ...
        fun grab(player: Player, scenery: Scenery) {
    ...
    companion object {
        ...
        private val GFX_POS = arrayOf(
            Location.create(3370, 9634, 1), Location.create(3359, 9634, 1), Location.create(3360, 9643, 1),
            Location.create(3363, 9643, 1), Location.create(3363, 9633, 1), Location.create(3367, 9637, 1),
            Location.create(3366, 9642, 1), Location.create(3380, 9647, 1), Location.create(3375, 9648, 1),
            Location.create(3370, 9649, 1), Location.create(3372, 9644, 1), Location.create(3379, 9646, 1),
            Location.create(3381, 9651, 1), Location.create(3379, 9656, 1), Location.create(3373, 9658, 1),
            Location.create(3353, 9648, 1), Location.create(3356, 9653, 1), Location.create(3346, 9657, 1),
            Location.create(3347, 9655, 1), Location.create(3354, 9634, 1), Location.create(3352, 9631, 1),
            Location.create(3355, 9627, 1), Location.create(3345, 9631, 1), Location.create(3345, 9627, 1),
            Location.create(3347, 9623, 1), Location.create(3351, 9621, 1), Location.create(3368, 9624, 1),
            Location.create(3368, 9631, 1), Location.create(3375, 9630, 1), Location.create(3375, 9635, 1),
            Location.create(3371, 9621, 1), Location.create(3377, 9621, 1), Location.create(3381, 9624, 1),
            Location.create(3382, 9629, 1), Location.create(3371, 9635, 1), Location.create(3365, 9634, 1),
            Location.create(3361, 9641, 1), Location.create(3363, 9644, 1)
        )
