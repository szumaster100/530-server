...
class SnowmanHolidayRandomDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            0 -> npcl(FaceAnim.CHILD_NEUTRAL, "Greetings, ${player!!.username.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }}. I'm visiting from the Land of Snow, but I need some help.").also { stage++ }
            ...
            10 -> npcl(FaceAnim.CHILD_NEUTRAL, "I visit Gielinor every year during this time when it is the coldest here, but it is still a little warm for us snowmen.").also { stage++ }
            11 -> npcl(FaceAnim.CHILD_NEUTRAL, "I'm really enjoying my visit, but I have started to melt. Would you happen to have a snowball?").also { stage++ }
            ...
            21 -> {
                if (player!!.emoteManager.isUnlocked(Emotes.SNOWMAN_DANCE)) {
                    npcl(FaceAnim.CHILD_NEUTRAL, "Please take this lamp I found on my travels. Happy holidays, ${player!!.username}!").also { stage = 22 }
                } else {
                    npcl(FaceAnim.CHILD_NEUTRAL, "Before I leave I will show you how to dance like us snowmen. Happy holidays, ${player!!.username}!").also { stage = 23 }
                }
            }
            ...
            30 -> npcl(FaceAnim.CHILD_NEUTRAL, "I must be going before I melt. Happy Holidays, ${player!!.username}.").also { stage++ }
