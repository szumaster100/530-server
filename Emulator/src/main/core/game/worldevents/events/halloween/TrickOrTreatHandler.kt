...
class TrickOrTreatHandler : InteractionListener {
    ...
    override fun defineListeners() {
        on(IntType.NPC, "trick-or-treat") { player, node ->
            ...
            if (hasDone5) {
                sendNPCDialogue(player, node.id, "My informants tell me you've already collected candy from 5 people today.", FaceAnim.FRIENDLY)
                return@on true
            }

            if (hasDoneMe) {
                sendNPCDialogue(player, node.id, "You've already asked me today! Don't get greedy, now.", FaceAnim.ANNOYED)
                return@on true
            }
            ...
            player.dialogueInterpreter.open(
                object : DialogueFile() {
                    override fun handle(componentID: Int, buttonID: Int) {
                        when (stage) {
                            0 -> playerl(FaceAnim.FRIENDLY, "Trick or treat!").also { if (RandomFunction.roll(20)) stage = 10 else stage++ }
                            ...
                            2 -> {
                                ...
                                when(rand) {
                                    0 -> {
                                        sendItemDialogue(player, Items.POPCORN_BALL_14082, "They hand you a popcorn ball.")
                                        addItemOrDrop(player, Items.POPCORN_BALL_14082, 1)
                                    }
                                    1 -> {
                                        sendItemDialogue(player, Items.CHOCOLATE_DROP_14083, "They hand you a chocolate drop.")
