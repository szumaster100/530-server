    ...
    fun handleRewardShop(player: Player, buttonId: Int) {
        ...
        if (!hasUnlocked && !hasEmote) {
            when (stage) {
                ...
                105 -> when (buttonId) {
                    1 -> if (canPurchase(player, 40)) buyStaff(player) else npc(FaceAnim.NEUTRAL, "You can't afford that.")
                    2 -> if (canPurchase(player, 10)) buyEmote(player) else npc(FaceAnim.NEUTRAL, "You can't afford that")
                }
            }
            ...
            when (stage) {
                ...
                105 -> when (buttonId) {
                    1 -> if (canPurchase(player, 10)) buyRecolor1(player) else npc(FaceAnim.NEUTRAL, "You can't afford that.")
                    2 -> if (canPurchase(player, 10)) buyRecolor2(player) else npc(FaceAnim.NEUTRAL, "You can't afford that.")
                    3 -> if (canPurchase(player, 10)) buyEmote(player) else npc(FaceAnim.NEUTRAL, "You can't afford that.")
                }
            }
        } else if (hasUnlocked && !hasEmote && hasRecolor1 && !hasRecolor2) {
            when (stage) {
                104 ->
                    player.dialogueInterpreter.sendOptions(title, "Staff Orange Recolor (10c)", "Trick Emote (10c)")
                        .also { stage++ }

                105 -> when (buttonId) {
                    1 -> if (canPurchase(player, 10)) buyRecolor2(player) else npc(FaceAnim.NEUTRAL, "You can't afford that.")
                    2 -> if (canPurchase(player, 10)) buyEmote(player) else npc(FaceAnim.NEUTRAL, "You can't afford that.")
                }
            }
        } else if (hasUnlocked && !hasEmote && !hasRecolor1 && hasRecolor2) {
