...
public class UpdateCache {
   ...
   public static void main555(String[] args) throws IOException {
      ...
      for(int i$ = 0; i$ < len$; ++i$) {
         ...
         for(int i$1 = 0; i$1 < len$1; ++i$1) {
            ...
            try {
               cache.getIndexes()[37].putFile(i, i2, 2, originalCache.getIndexes()[19].getFile(i, i2), null, false, false, -1, -1);
   ...
   public static void packLogo(Store cache) throws IOException {
      ...
      for(i = 0; i < 4; ++i) {
         int realid = 3769 + i;
         int var8 = 3769 + i;
         cache.getIndexes()[8].putFile(var8, 0, (new IndexedColorImageFile(ImageIO.read(new File("bg/" + realid + ".gif")))).encodeFile());
         var8 = 3779 + i;
         cache.getIndexes()[8].putFile(var8, 0, (new IndexedColorImageFile(ImageIO.read(new File("bg/" + realid + ".gif")))).encodeFile());
         var8 = 3783 + (i >= 2?i - 2:i + 2);
         cache.getIndexes()[8].putFile(var8, 0, (new IndexedColorImageFile(ImageIO.read(new File("bg/" + realid + ".gif")))).encodeFile());
         var8 = 3769 + i;
         cache.getIndexes()[34].putFile(var8, 0, (new IndexedColorImageFile(ImageIO.read(new File("bg/" + realid + ".gif")))).encodeFile());
         var8 = 3779 + i;
         cache.getIndexes()[34].putFile(var8, 0, (new IndexedColorImageFile(ImageIO.read(new File("bg/" + realid + ".gif")))).encodeFile());
         var8 = 3783 + (i >= 2?i - 2:i + 2);
         cache.getIndexes()[34].putFile(var8, 0, (new IndexedColorImageFile(ImageIO.read(new File("bg/" + realid + ".gif")))).encodeFile());
      ...
      if(!updateJustMaps) {
         ...
         if(addOldItems) {
            ...
            for(var17 = var24; var17 < var24 + var22; ++var17) {
               ...
               cache.getIndexes()[19].putFile(var17 >>> 8, 255 & var17, 2, originalCache.getIndexes()[19].getFile(regionId >>> 8, 255 & regionId), null, false, false, -1, -1);
            }
   ...
   public static boolean addMapFile(Index index, String name, byte[] data) {
      ...
      return index.putFile(archiveId, 0, 2, data, null, false, false, Utils.getNameHash(name), -1);
   }
}
